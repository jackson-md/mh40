 ...
 *
 * Overlapping ranges should be avoided to conserve flash space 
 */
typedef enum
{
     ...
     *
     * Dump block of Apps P1 RAM with Hydra log starting four bytes
     * after P1D_P1_DM_RAM_LOWER start offset.
...
/** Debug Partition Configuration Keys for 32bit
 *
 * Overlapping ranges should be avoided to conserve flash space 
 */
typedef enum
{
     ...
     * which can change for different device chips.
     * The start value can be found with the define P1D_P1_DM_RAM_LOWER.
     * Start offset and length are encoded in two 32-bit parameters value,
     * the value1 for the start offset and the value2 for the size
     *
     * An arbitrary number of custom ranges can be configured by calling
     * the trap multiple times.
     *
     * Configuration key used for RAM bigger than 64KB
     * (start offset that can be higher than 0xFFFF) */
