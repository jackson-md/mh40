...
#include "app/message/link_status_if.h"
...
/*!
  Message to indicate the status of the priority transmission via ChargerComms Uart.
  The message content is a #MessageChargerCommsUartPriStatus.

  This message is sent to the task registered with MessageChargerCommsTask().
 */
#define MESSAGE_CHARGERCOMMS_UART_PRI_STATUS (SYSTEM_MESSAGE_BASE_ + 85)

/*!
  Message to indicate the result of starting the operator framework asynchronously.
  The message content is a #MessageOperatorFrameworkEnableResult.

  This message is sent to the task given by OperatorFrameworkEnableAsync().
 */
#define MESSAGE_OPERATOR_FRAMEWORK_ENABLE_RESULT (SYSTEM_MESSAGE_BASE_ + 86)

/*!
  Message to indicate that a new sensor data is available.
  The message content is a #MessageSensorData.

  This message is sent to the task registered with MessageSensorTask().
 */
#define MESSAGE_SENSOR_DATA (SYSTEM_MESSAGE_BASE_ + 87)

/*!
  Message to indicate that a BT link status tracking data is available.
  The message content is a #MessageLinkStatusTracking.

  This message is sent to the task registered with MessageSystemTask().
 */
#define MESSAGE_LINK_STATUS_TRACKING (SYSTEM_MESSAGE_BASE_ + 88)
...
/*!
  Message type received when ChargerCommsUart priority transmission is done
  or failed.
*/
typedef struct
{
    charger_comms_uart_tx_status status; /*!< Status */
} MessageChargerCommsUartPriStatus;
...
/*!
  Message type received when OperatorFrameworkEnableAsync has completed.
*/
typedef struct
{
    bool success; /*!< TRUE if the DSP power state changed, FALSE otherwise */
} MessageOperatorFrameworkEnableResult;

/*!
  #MESSAGE_SENSOR_DATA received to indicate more sensor data available to the application
*/
typedef struct
{
    Sensor sensor; /*!< The sensor ID of the sensor that has data available */
} MessageSensorData;

/*!
  #MESSAGE_LINK_STATUS_TRACKING received with ACL link status information. 
*/
typedef struct
{
    uint8                       num_good_channels;      /*!< Number of good channels */
    uint8                       num_acl_connections;    /*!< Number of ACL connections */
    uint8                       num_sco_connections;    /*!< Number of SCO connections */
    uint8                       num_le_connections;     /*!< Number of LE connections */
    link_status_link_quality    link_quality;           /*!< Link Quality */
    link_status_connection_type connection_type;        /*!< Connection Type*/
    int8                        average_rssi;           /*!< Average RSSI in dBm. Range = -100 to 20 */
    uint16                      eval_period;            /*!< Requested Evaluation Period */
    uint16                      sink_id;                /*!< Requested sink ID */
    uint32                      throughput;              /*!< Link throughput calculated in evaluation period in kbps */
    uint32                      nak_rate;               /*!< NAK Rate */
    uint32                      num_retransmissions;    /*!< Number of retransmissions. This is a moving average with the exponential factor used as 32 */
    uint16                      num_packets;            /*!< Number of (successful) TX packets */
    uint16                      num_coex_blocked;        /*!< Number of packets blocked by Coex */
    link_status_phy_rate        phy_rate;               /*!< Last PHY rate used on link */
    int8                        tx_power;               /*!< Last TX power used on link in dBm */
    uint16                      sniff_interval;         /*!< Not supported reserved for future use */
    uint16                      connection_event;       /*!< Not supported reserved for future use */
} MessageLinkStatusTracking;
