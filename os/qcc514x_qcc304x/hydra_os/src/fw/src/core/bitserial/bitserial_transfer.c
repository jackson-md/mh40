    ...
    if (result != BITSERIAL_RESULT_SUCCESS)
    {
        ...
        switch (result)
        {
            case BITSERIAL_RESULT_I2C_NACK:
                /* Calculate the amount of data transmitted and/or received
                 * so that we can correct the buffer pointers later.
                */
                if (instance[i]->active_op.rx_len)
                {
                    if (instance[i]->active_op.flags & BITSERIAL_ACTION_TRANSFER_RXFIRST)
                    {
                        ...
                        /* Note: Read/Write is technically illegal for I2C */
                        if (instance[i]->active_op.flags & BITSERIAL_ACTION_FLAG_ERR_PART2)
                        {
                            /* Fail on write after read */
                            rxed = instance[i]->active_op.rx_len;
                            txed = word_count;
                        }
                        else
                        {
                            /* Fail on read before write, or read-only */
                            rxed = word_count;
                    ...
                    else
                    {
                        ...
                        if (instance[i]->active_op.flags & BITSERIAL_ACTION_FLAG_ERR_PART2)
                        {
                            /* Fail on read after write */
                            rxed = word_count;
                            txed = instance[i]->active_op.tx_len;
                        }
                        else
                        {
                            /* Fail on write before read */
                            rxed = 0;
                            txed = instance[i]->active_op.tx_len;
                        }
                    }
...
static void action_engine_irq(bitserial_instance i)
{
    ...
    error = hal_bitserial_error_status_get(i);
    ...
    if (BITSERIAL_EVENT_IS_FAIL(events))
    {
        ...
        /* Indicate which stage of a combo operation failed */
        if (error & BITSERIAL_TRANSACTION_PART2_MASK)
        {
            instance[i]->active_op.flags |= BITSERIAL_ACTION_FLAG_ERR_PART2;
        }
    }
...
static void active_op_program(bitserial_instance i)
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* Set in the flags that it's a read for convenience later... */
            instance[i]->active_op.flags |= BITSERIAL_ACTION_TRANSFER_RXFIRST;
        }
        if (slave_mode)
        {
