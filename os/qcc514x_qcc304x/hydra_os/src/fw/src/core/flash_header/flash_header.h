...
typedef enum
{
    ...
    FLASH_SECTION_ID_SCRATCH_PARTITION, /* Virtual partition */
...
/**
 * Get the offset and 'real' size for a given section within the flash device.
 * This is the same as above, but uses the section mapping to determine whether
 * capacity or size should be regarded as the 'real' size.
 *
 * @param [in]  section_id  Id of the section.
 * @param [out] offset      Returns the absolute address of the section.
 * @param [out] size        Returns the size of the section from the header.
 * @return TRUE if the section was in the flash header, FALSE otherwise.
 */
bool flash_header_get_section_offset_and_realsize(FLASH_SECTION_ID section_id,
                                                  uint32 *offset, uint32 *size);
