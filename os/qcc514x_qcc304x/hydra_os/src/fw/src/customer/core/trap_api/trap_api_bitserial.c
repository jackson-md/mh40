...
static volatile bitserial_result bitserial_transfer_done[HAVE_NUMBER_OF_BITSERIALS];
...
)
{
    ...
    bitserial_transfer_done[i] = BITSERIAL_RESULT_NOT_YET;
    L5_DBG_MSG3(
    ...
    )
    {
        ...
        if (bs_flags & BITSERIAL_ACTION_FLAG_BLOCKING)
        {
            ...
            while(bitserial_transfer_done[i] == BITSERIAL_RESULT_NOT_YET)
            {
                /* Empty block. */
        ...
        return bitserial_transfer_done[i];
    }
    else
    {
...
{
    bitserial_transfer_done[i] = result;
