 ...
 *         If OperatorFrameworkEnable() is called while
 *  OperatorFrameworkEnableAsync() is pending, it will either block until the
 *         pending operation completes (if the requested state is the same), or
 *  return FALSE if the requested states conflict.
 *   @note
...
/**
 *  \brief Loads DSP operator framework / powers-off DSP asynchronously
 *         As OperatorFrameworkEnable(), but returns immediately. Once the
 *  operator framework state has changed, it returns
 *         a message to the specified task. This call is provided so that other
 *  functionality can be performed while the operator
 *         framework is loading. No audio-related operations may be performed
 *  between this call and the return message, other than
 *         OperatorFrameworkEnable().
 *         If OperatorFrameworkEnable() is called while
 *  OperatorFrameworkEnableAsync() is pending, it will either block until the
 *         pending operation completes (if the requested state is the same), or
 *  return FALSE if the requested states conflict.
 *         
 *  \param state DSP operator framework power state for specific DSP core 
 *  \param task The task which will receive the status result message. 
 *  \return TRUE if parameter is accepted, FALSE otherwise.
 * 
 * \ingroup trapset_operator
 */
bool OperatorFrameworkEnableAsync(OperatorFrameworkPowerState state, Task task);
