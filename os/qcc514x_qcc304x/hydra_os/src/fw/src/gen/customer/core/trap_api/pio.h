...
The chip will deep sleep regardless of state of PIOs by default. 
For PIOs which have been configured to wake the chip up from deep sleep 
(via PioSetDeepSleepWakeBank()) the PIO wake level has to be configured 
correctly (via PioSetWakeupStateBank()) for the chip to enter deep sleep. 
*/
#define PioSetDeepSleepEitherLevelBank(bank, mask, value) PioSetDeepSleepWakeBank(bank, mask, value)
...
/**
 *  \brief Configures PIOs to wake the chip up from deep sleep.
 *  \param bank  The bank index of the PIO lines. 
 *  \param mask  Bitmask to select the PIO lines to update within the given bank. 
 *  \param value  Each bit corresponds to a PIO line in the selected bank. 
 * Setting a bit high will configure the corresponding PIO to wake the chip from
 *  deep 
 * sleep. Setting a bit low will cause the state of the PIO to be ignored in deep
 *  sleep.
 * Note that the chip will not enter deep sleep if any of the PIOs configured to 
 * wake it up from deep sleep are in the "wake" state as configured by 
 * PioSetWakeupStateBank(). 
 * 
 * \ingroup trapset_core
 */
void PioSetDeepSleepWakeBank(uint16 bank, uint32 mask, uint32 value);
