...
/**
 *  \brief Find the RFCOMM DLCI corresponding to a sink. 
 *  \param sink The Sink to get the RFCOMM DLCI for.
 * 
 * \ingroup trapset_rfcomm
 */
uint8 SinkGetRfcommDlci(Sink sink);

/**
 *  \brief Find the Sink corresponding to an RFCOMM DLCI with remote device. 
 *  \param tpaddr Bluetooth address of remote device. 
 *  \param dlci The RFCOMM DLCI associated with remote device
 * 
 * \ingroup trapset_rfcomm
 */
Sink StreamRfcommSinkFromDlci(const tp_bdaddr * tpaddr, uint8 dlci);
...
/**
 *  \brief Enable or disable the A2DP link quality status tracking 
 *         and update the evaluation period for quality assessment. 
 *   On supported devices this will enable the periodic MESSAGE_LINK_STATUS to be
 *   sent to the task registered with MessageLinkStatusTrackingTask().
 *   
 *   The traps works for an Audio Source application taking the L2CAP stream sink
 *  ID 
 *   to identify the ACL connection for which status events are required. 
 *   
 *   Link Status Messages can only be enabled on one link at a time. A second
 *  request
 *   for a different sink ID will fail unless the initial request is disabled. 
 *   The supported periodicity for these events is 50-1000ms.
 *  \param sink L2CAP Stream sink handle used to identifies the ACL connection.
 *  \param enable Flag to enable or disable events
 *  \param period The time period for which link evaluation is to be
 *             done before sending a quality report as a message. 
 *  \return TRUE if the sink identifies an ACL and link status is
 *           supported, FALSE otherwise.
 * 
 * \ingroup trapset_bluestack
 */
bool SinkEnableLinkStatusTracking(Sink sink, bool enable, uint16 period);
