...
typedef struct IPC_STREAM_ISP_SOURCE {
    IPC_HEADER header;
    stream_isp_role role;
} IPC_STREAM_ISP_SOURCE;

typedef struct IPC_STREAM_ISP_SINK {
    IPC_HEADER header;
    stream_isp_role role;
} IPC_STREAM_ISP_SINK;
...
typedef struct IPC_SINK_GET_RFCOMM_DLCI {
    IPC_HEADER header;
    uint16 sink;
} IPC_SINK_GET_RFCOMM_DLCI;

typedef struct IPC_STREAM_RFCOMM_SINK_FROM_DLCI {
    IPC_HEADER header;
    const TP_BD_ADDR_T * tpaddr;
    uint8 dlci;
} IPC_STREAM_RFCOMM_SINK_FROM_DLCI;
...
typedef struct IPC_CHARGER_COMMS_UART_PRIORITY_TRANSMIT {
    IPC_HEADER header;
    const uint8 * data;
    uint16 length;
    charger_comms_uart_address reset_seqnum;
} IPC_CHARGER_COMMS_UART_PRIORITY_TRANSMIT;
...
typedef struct IPC_DEBUG_PARTITION_CONFIG32 {
    IPC_HEADER header;
    debug_partition_config32_key key;
    uint32 value1;
    uint32 value2;
} IPC_DEBUG_PARTITION_CONFIG32;
...
typedef struct IPC_PIO_SET_DEEP_SLEEP_WAKE_BANK {
    IPC_HEADER header;
    uint16 bank;
    uint32 mask;
    uint32 value;
} IPC_PIO_SET_DEEP_SLEEP_WAKE_BANK;
...
typedef struct IPC_FEATURE_CHALLENGE_RESPONSE {
    IPC_HEADER header;
    feature_id feature;
    const uint32 * challenge_ptr;
    uint32 * response_ptr;
} IPC_FEATURE_CHALLENGE_RESPONSE;
...
typedef struct IPC_VM_GET_B_T_RADIO_CALIBRATION_QUALITY {
    IPC_HEADER header;
    uint8 * cal_quality;
} IPC_VM_GET_B_T_RADIO_CALIBRATION_QUALITY;

typedef struct IPC_TEST2_TX_ENHANCED {
    IPC_HEADER header;
    const tx_enhanced_params_t * tx_params;
} IPC_TEST2_TX_ENHANCED;

typedef struct IPC_TEST2_RX_ENHANCED {
    IPC_HEADER header;
    const rx_enhanced_params_t * rx_params;
} IPC_TEST2_RX_ENHANCED;

typedef struct IPC_TEST2_RX_STAT_ENHANCED {
    IPC_HEADER header;
    rx_stats_enhanced_t * rx_stats_enhanced;
} IPC_TEST2_RX_STAT_ENHANCED;
...
typedef struct IPC_VM_CIS_CONN_GET_RSSI {
    IPC_HEADER header;
    const TP_BD_ADDR_T * tpbdaddr;
    uint16  handle;
} IPC_VM_CIS_CONN_GET_RSSI;
...
typedef struct IPC_VM_READ_NEXT_SNIFF_CLOCK {
    IPC_HEADER header;
    const TP_BD_ADDR_T * tpaddr;
    next_sniff_clock_t * next_sniff;
} IPC_VM_READ_NEXT_SNIFF_CLOCK;

typedef struct IPC_SINK_ENABLE_LINK_STATUS_TRACKING {
    IPC_HEADER header;
    uint16 sink;
    bool enable;
    uint16 period;
} IPC_SINK_ENABLE_LINK_STATUS_TRACKING;

typedef struct IPC_VM_CIS_CONN_GET_RSSI_RSP {
    IPC_HEADER header;
    bool ret;
    int16 rssi;
} IPC_VM_CIS_CONN_GET_RSSI_RSP;
...
typedef struct IPC_OPERATOR_FRAMEWORK_ENABLE_ASYNC {
    IPC_HEADER header;
    OperatorFrameworkPowerState state;
    Task task;
} IPC_OPERATOR_FRAMEWORK_ENABLE_ASYNC;
...
typedef struct IPC_XTAL_CLOCK_OUT_ENABLE {
    IPC_HEADER header;
    bool enable;
} IPC_XTAL_CLOCK_OUT_ENABLE;
...
#endif /* TRAPSET_CORE */
#if TRAPSET_BLUESTACK
    IPC_VM_CIS_CONN_GET_RSSI_RSP ipc_vm_cis_conn_get_rssi_rsp;
