...
models_dest_basename = 'models'
models_src_basename = 'wakeword-models'
localeToModels_filename = 'WakewordModelMapping.json'
libs_dest_basename = r'kalimba\kymera\lib_release'
headers_dest_basename = r'..\..\capabilities'
header_files_names_list=['pryon_lite.h', 'pryon_lite_error.h', 'pryon_lite_metadata.h', 'pryon_lite_vad.h', 'pryon_lite_ww.h']
...
def parse_args():
    ...
    header_files_names_list.append('pryon_lite_' + args.api_version + '.h') 
...
def add_locale_to_model_entry(locale, model):
    locale_to_model_map[locale] = model

def dump_to_json_file(file_path, data):
...
def export_file(model, locale, src_root_dir, models_dest_dir):
    ...
    if(len(model) > 1):
        print_warning("Multiple models defined for locale {}: {}".format(locale, model))
        print("Selecting only 50K models")
        # Only 50K models are supported
        model_50k = list(s for s in model if '_50k' in s)
        if model_50k:
            model = model_50k
            
    if(len(model) > 1):
        print_error_and_exit("Multiple models defined for locale {}: {}".format(locale, model))
    ...
    if(locale in model):
        ...
        add_locale_to_model_entry(locale, locale)
    else:
        ...
        add_locale_to_model_entry(locale, locale_in_model)
...
def export_header_files(dest_headers_path, header_files_path):
    ...
    for file_name in header_files_names_list:
        file_path = os.path.join(header_files_path, file_name)
        dest_file_path = os.path.join(dest_headers_path, file_name)
        if os.path.isfile(file_path):
            copyfile(file_path , dest_file_path)    
            print('%s exported as %s' % (file_path , dest_file_path))
        else:
            print_warning("{} header not found".format(file_path))
...
def extract(zip_dir, zip_file_partial_name, build_config, chip_type, operator):
    ...
    if(operator == "apva"):
        ...
        dump_to_json_file(os.path.join(dest_models_path, localeToModels_filename), locale_to_model_map)
