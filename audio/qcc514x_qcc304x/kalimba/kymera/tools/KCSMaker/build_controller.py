...
class Build_Controller(object):
    ...
    def build(self, binfo, cap_list, exe_name, linkscript_template, make_cmd, make_jobs, clean_first=True):
        ...
        cmdline = make_cmd + " -s -f " + self.makefile_name + " CONFIG=" + self.download_config + " OSTYPE=" + self.os_type + " PYTHON_VER=" + str(sys.version_info.major)
        if clean_first:
            logging.debug("Starting clean. " + cmdline + " clean")
            clean = subprocess.call(cmdline + " clean", shell=True)
            if clean != 0:
                logging.error("\nClean build failed.\nExiting...")
                sys.exit(-1)
        logging.debug("Starting make. " + cmdline)
        make = subprocess.call(cmdline, shell=True)
