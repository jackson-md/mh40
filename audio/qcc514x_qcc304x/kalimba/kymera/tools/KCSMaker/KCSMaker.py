...
# Import code appropriate to the python version
...
def proc_cmd_line(args):
    ...
    for arg in args:
        ...
        elif "-capabilitiespath" in arg:
            CAPABILITIES_PATH = os.path.abspath(arg.split("=")[1])
        elif "-bundlefilepath" in arg:
            BUNDLE_FILE_PATH = os.path.abspath(arg.split("=")[1])
    ...
    if BUNDLE_FILE_PATH == "":
        BUNDLE_FILE_PATH = os.path.abspath(os.path.join(KCSMAKER_PATH, "bundle"))
...
class KCSMaker(object):
    def __init__(self, cmd_line_args):
        ...
        self.capabilities_path = cmd_line_args["CAPABILITIES_PATH"]
        print("self.capabilities_path="+self.capabilities_path)
        self.bdl_file = os.path.abspath(
            os.path.join(
                cmd_line_args["BUNDLE_FILE_PATH"],
    ...
    def main(self, get_kcc_version):
        ...
        try:
            ...
            # Use code appropriate to the python version
            if sys.version_info < (3, 0):
                mod = imp.load_source("mod", self.bdl_file)
            else:
                mod = SourceFileLoader("mod", self.bdl_file).load_module()
...
if __name__ == "__main__":
    ...
    python_tools_path = get_python_tools.python_tools_version().get_python_tools_path(
                 cmd_line_args["OS_TYPE"])
    if python_tools_path:
