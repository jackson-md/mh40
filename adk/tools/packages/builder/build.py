...
try:
	import ubuild.maker.exceptions as bdex
except:
...
def construct_makefile_frag(proj, per_config_depend, config_name, build_output_folder=None, is_audio=False, capability=None):
    ...
    if is_audio:
        ...
        if 'CAPABILITIES_PATH' in make_vars:
            cap_path = os.path.abspath(os.path.join(proj.dirname, make_vars["CAPABILITIES_PATH"].split('=')[1], capability))
        else:
...
"""
        ...
        # Convert paths to Posix style for Make
        for var in ['INCPATHS', 'LIBPATHS']:
            if var in make_vars:
                make_vars[var] = make_vars[var].replace('\\', '/')
        ...
        for src_file in source_files:
            output += '    %s \\\n' % os.path.relpath(src_file, start=proj.dirname).replace('\\', '/')
        output += '    build_id_str.c \\\n'
        output += '    rsa_pss_constants.c \\\n'
        output += '$(DBS)\n'
        output += '\n'
...
def do_make(proj, devkit, tgt_config, build_style, is_audio, build_output_folder=None):
    ...
    if is_audio:
        ...
        if 'BUNDLE_FILE_PATH' in make_vars:
            cmd_line += [make_vars['BUNDLE_FILE_PATH']]
        if "CAPABILITIES_PATH" in make_vars:
            cmd_line += [make_vars['CAPABILITIES_PATH']]
...
def build_filesystem(proj, devkit, script_args, crypto_key, build_output_folder):
    ...
    elif filesystem_type == "customer_ra":
        import convert_to_xuv
        convert_to_xuv.run_script(project_files + ['-o','customer_ra_filesystem_dfu'])
