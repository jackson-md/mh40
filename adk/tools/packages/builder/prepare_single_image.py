...
try:
    from vm_build.elf2xuv.elf2xuv import elf2xuv
...
class PrepareSingleImage(object):
    ...
    def __init__(self, devkit_root, workspace_file, workspace, build_output_folder=None, generate_audio_image_xuv=False, device_uri=None):
        ...
        print("PrepareSingleImage: init done")
    ...
    def _search_project_for_flash_config_file(self, project):
        ...
        """
        try:
            from ubuild.maker import parse_proj_file as pproj
        except ImportError:
    ...
    def _do_elf2xuv(self, source, destination=None):
        ...
        """
        print("prepare_single_image calling elf2xuv()")
    ...
    def _add_layout(self, file_type, xuv_filename, start_tuple, encrypted):
        ...
        if file_type == "apps_p0" and encrypted == True:
            ...
            layout_dict["oem_authenticate"] = True
    ...
    def calculate_ro_fs_size(self, project, output_filename):
        ...
        """
        try:
            from vm_build.builder import parse_proj_file as pproj
        except ImportError:
            import maker.parse_proj_file as pproj
        proj_parser = pproj.Project(project, self._devkit_root, self._workspace_file)
        ...
        for srcfile in proj_parser.get_source_files():
            ...
            elif srcfile.endswith(('.pcm', '.sbc', '.aac', '.adts')):
    ...
    def process_project(self, project, outpath, build_output_folder=None):
        ...
        configuration. If a configuration is for a project type that is of interest
        ...
        for config in config_list:
            ...
            if project_type is not None:
                if project_type not in self._projects.keys():
                    ...
                    elif project_type in ("firmware_config", "device_config", "customer_ro", "user_ps", "customer_va", "customer_ra"):
    ...
    def create_flash_image(self, devkit, output_path, encrypted = False,
        ...
        ImageBuilder.partition_aliases["ra_fs"] = "ra_partition"
        ...
        xuv_filename = None
        if "customer_ra" in self._projects:
            for ptn_index, ptn in enumerate(flash0["layout"]):
                # Cannot use map to find partition with a duplicated name
                if "ra_partition" in ptn and 'id' not in ptn[1]:
                    xuv_filename = self._projects["customer_ra"]
                    flash0["layout"][ptn_index] = self._add_layout("ra_fs",
                        xuv_filename, flash0["layout"][ptn_index], None)
                    break
