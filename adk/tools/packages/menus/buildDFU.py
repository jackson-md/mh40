...
def add_layout(lines, file_type, xuv_filename, params, encrypted):
    ...
    for item, value in params.iteritems():
        if item == "id":
            ...
            lines.append(line)
    line = '            }),\n'
    if file_type == "apps_p0" and encrypted == True:
        # If encrypted then P0 image section should have oem_authenticate=True flag in the flash layout
        item = "oem_authenticate"
        value = True
        line = '            "{}": {},\n'.format(item, value)
        lines.append(line)
        line = '            }),\n'
...
def prepare_dfu_file_generator_config(devkit_root, projects, dfg_filepath,
    ...
    for i in range(len(flash_config["layout"])):
        ...
        map_params.append(flash_config["layout"][i][1])
    authenticate = None
    if encrypted == True:
        authenticate = True
    ...
    for i in range(len(map)):
        ...
        if map[i] =="curator_fs":
            ...
            lines = add_layout(lines, "curator_fs", xuv_filename, map_params[i], encrypted)
        elif map[i] == "apps_p0":
            xuv_filename = None
            if "apps0" in projects:
                xuv_filename = projects["apps0"]
            lines = add_layout(lines, "apps_p0", xuv_filename, map_params[i], encrypted)
        elif map[i] == "apps_p1":
            xuv_filename = None
            if "app/p1" in projects:
                xuv_filename = projects["app/p1"]
            lines = add_layout(lines, "apps_p1", xuv_filename, map_params[i], encrypted)
        elif map[i] == "audio":
            xuv_filename = None
            if "audio" in projects:
                xuv_filename = projects["audio"]
            lines = add_layout(lines, "audio", xuv_filename, map_params[i], False)
        elif map[i] == "device_ro_fs":
            lines = add_layout(lines, "device_ro_fs", None, map_params[i], encrypted)
        elif map[i] == "rw_config":
            xuv_filename = None
            if "user_ps" in projects:
                xuv_filename = projects["user_ps"]
            lines = add_layout(lines, "rw_config", xuv_filename, map_params[i], None)
        elif map[i] == "ra_partition":
            lines = add_layout(lines, "ra_partition", None, map_params[i], None)
        elif map[i] == "debug_partition":
            lines = add_layout(lines, "debug_partition", None, map_params[i], None)
        elif map[i] == "rw_fs":
            lines = add_layout(lines, "rw_fs", None, map_params[i], None)
        elif map[i] == "ro_cfg_fs":
            xuv_filename = None
            if "firmware_config" in projects:
                xuv_filename = projects["firmware_config"]
            lines = add_layout(lines, "ro_cfg_fs", xuv_filename, map_params[i], encrypted)
        elif map[i] == "ro_fs":
            xuv_filename = None
            if "customer_ro" in projects:
                xuv_filename = projects["customer_ro"]
            lines = add_layout(lines, "ro_fs", xuv_filename, map_params[i], encrypted)
        else:
...
def parse_args(args):
    ...
    parser.add_argument('--build_system',
                        choices=['make', 'scons'],
                        default="make",
                        help=' Specify which build system to use')
