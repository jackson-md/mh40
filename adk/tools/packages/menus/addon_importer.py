...
sbc_ro_fs_templates_project_file = 'addon_ext_ro_fs_sbc_x2p.xml'
aac_ro_fs_templates_project_file = 'addon_ext_ro_fs_aac_x2p.xml'
...
class Importer(object):
    def __init__(self, args):
        ...
        if "INCLUDE_AAC_PROMPTS" in self.addon_utils.readAppProjectProperty("DEFS", self.args.project):
        	self.ro_fs_templates_project_file = aac_ro_fs_templates_project_file
        else:
        	self.ro_fs_templates_project_file = sbc_ro_fs_templates_project_file
    ...
    def _createMakefile(self):
        ...
        if os.path.exists(dest_addon_makefile):
            print("INFO: Makefile already present in {}. Not copying template from {}".format(dest_addon_makefile, src_addon_makefile))
        else:
