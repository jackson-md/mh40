...
static const audio_source_audio_interface_t usb_audio_interface =
{
    ...
    .IsAudioRouted = usbAudio_IsAudioRouted,
...
static bool usbAudio_IsAudioRouted(audio_source_t source)
{
    ...
    if((usb_audio != NULL) && (usb_audio->headphone->source_state == source_state_connected))
    {
        is_routed = TRUE;
    }
...
static source_status_t usbAudio_SetState(audio_source_t source, source_state_t state)
{
    ...
    if(usb_audio != NULL)
    {
        DEBUG_LOG_DEBUG("usbAudio_SetState(%p) from enum:source_state_t:%d to enum:source_state_t:%d",
                        usb_audio, usb_audio->headphone->source_state, state);
        usb_audio->headphone->source_state = state;
    }
