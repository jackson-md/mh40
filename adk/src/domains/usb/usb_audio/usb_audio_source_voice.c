...
static bool usbAudio_IsVoiceRouted(voice_source_t source);
static source_status_t usbAudio_SetVoiceState(voice_source_t source, source_state_t state);
...
static const voice_source_audio_interface_t usb_voice_interface =
{
    ...
    .IsAudioRouted = usbAudio_IsVoiceRouted,
    .IsVoiceChannelAvailable = usbAudio_IsVoiceAvailable,
    .SetState = usbAudio_SetVoiceState
};
                                       ...
                                       source_defined_params_t * source_params)
{
    ...
    if(usbAudio_IsVoiceAvailable(source))
    {
        ...
        connect_params->mute_status = streaming_info->volume_status.mute_status;
                ...
                (!usb_audio->headset->spkr_active && headphone_audio && headphone_audio->headphone->spkr_active))
        {
            ...
            connect_params->spkr_frame_size = streaming_info->frame_size;
...
#ifdef USB_SUPPORT_HEADPHONE_SPKR_IN_VOICE_CHAIN
        if (usb_audio->headset->alt_spkr_connected)
        {
            usb_audio_info_t *headphone_audio = UsbAudioFd_GetHeadphoneInfo(audio_source_usb);
            disconnect_params->spkr_src = headphone_audio ? headphone_audio->headphone->spkr_src : 0;
        }
        else
#endif
        {
...
static bool usbAudio_IsVoiceRouted(voice_source_t source)
{
    ...
    if((usb_audio != NULL) && (usb_audio->headset->source_state == source_state_connected))
    {
        is_routed = TRUE;
    }
...
static source_status_t usbAudio_SetVoiceState(voice_source_t source, source_state_t state)
{
    ...
    if(usb_audio != NULL)
    {
        DEBUG_LOG_DEBUG("usbAudio_SetVoiceState(%p) from enum:source_state_t:%d to enum:source_state_t:%d",
                        usb_audio, usb_audio->headset->source_state, state);
        usb_audio->headset->source_state = state;
    }
