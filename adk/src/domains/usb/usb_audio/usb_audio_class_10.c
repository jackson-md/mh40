...
/*!  See usbAudioClass10_EnumerateStreaming() */
                                                ...
                                                uint32 sampling_rate)
{
    ...
    if (format)
    {
        ...
        for (uint8 i = 1; i < format->bSamFreqType; i++)
        {
            ...
            if(abs((int32)closest_rate - sampling_rate) > abs((int32)rate - sampling_rate))
            {
                closest_rate = rate;
            }
        }
        ...
        else if (control == UAC_FU_CONTROL_VOLUME)
        {
            ...
            if (success)
            {
                DEBUG_LOG("audioGetLevel: VOL: intf - %d, Code - %d, VAL - %d",
                           streaming_info->interface, code, (int8)value);
...
static void usbAudioClass10_HandleControlClassRequest(uac_data_t * uac_info)
{
    ...
    while ((packet_size = SourceBoundary(req)) != 0)
    {
        ...
        */
        ...
        if (streaming_info != NULL && interface == uac_info->control_interface)
        {
...
static void usbAudioClass10_HandleStreamingClassRequest(uac_data_t *uac_info, uint8 intf_index)
{
    ...
    while ((packet_size = SourceBoundary(req)) != 0)
    {
        ...
        */
        ...
        DEBUG_LOG_VERBOSE("UAC1_HandleStreamingClassRequest: intf_index %d, interface %d, desc_index %d",
                 ...
                 (usbresp.original_request.wLength == 3))
        {
            ...
            if(usbresp.original_request.wIndex == streaming_info->ep_address)
            {
                if (usbresp.original_request.bRequest == SET_CUR)
                {
                    ...
                    /* AudioStreaming requests are directed to the recipient where the Control resides
                     * See the USB Device Class Definition for Audio Devices, section 5.2 */
                    new_rate = usbAudioClass10_RoundSamplingRate(&(intf_config->streaming_config[streaming_info->streaming_desc_index]),
                                                                 new_rate);
                                               ...
                                               const usb_audio_interface_config_t *intf_config)
{
    ...
    DEBUG_LOG_DEBUG("EnumerateAudioStreaming: alt_settings_count  : %d", intf_config->alt_settings_count);
    ...
    for(uint8 index = 0; index<intf_config->alt_settings_count ; index++)
    {
        ...
        if(index == 0)
        {
            ...
            streaming_info->interface = UsbAddInterface(&usb_codes_as, UAC_CS_DESC_INTERFACE,
                                                        streaming_config->intf_descriptor,
                                                        streaming_config->size_descriptor);
        ...
        ep_info.wMaxPacketSize = streaming_config->endpoint->wMaxPacketSize ?
                      ...
                      usbAudioClass10_GetMaxPacketSize(streaming_config);
...
static void usbAudioClass10_Set_Defaults(uac_data_t *uac_info)
{
    ...
    for (uint8 i=0; i < uac_info->num_interfaces; i++)
    {
        ...
        streaming_info[i].alt_setting = INTF_ALT_SETTING_ZERO;
        /* Using streaming_desc_index 0 as default */
        format_descriptor = usbAudioClass10_GetStreamingFormatDescriptor(intf_config->streaming_config);
        PanicNull(format_descriptor);
        streaming_info[i].streaming_desc_index = 0;
        streaming_info[i].channels = format_descriptor->bNrChannels;
        streaming_info[i].frame_size = format_descriptor->bSubframeSize;

        /* set default sampling Rate */
        streaming_info[i].current_sampling_rate =
                usbAudioClass10_GetMaxSamplingRate(intf_config->streaming_config);
...
static void usbAudioClass10_SetInterface(uac_ctx_t class_ctx, uint16 interface, uint16 alt_setting)
{
    uac_data_t *uac_info = audio_class_info;
    ...
    while (uac_info)
    {
        if (uac_info == (uac_data_t *)class_ctx)
        {
            DEBUG_LOG_VERBOSE("usbAudioClass10_SetInterface %d %d ", interface, alt_setting);
            uint8 num_interfaces = uac_info->config->intf_list->num_interfaces;

            const usb_audio_interface_config_list_t *intf_list = uac_info->config->intf_list;
            usb_audio_streaming_info_t  *streaming_info = uac_info->streaming_info;

            for (uint8 index=0; index < num_interfaces; index++)
            {
                if (streaming_info[index].interface == interface)
                {
                    streaming_info[index].alt_setting = alt_setting;

                    if(alt_setting != INTF_ALT_SETTING_ZERO)
                    {
                        uac_format_descriptor_t *format_descriptor;
                        uint8 streaming_intf_desc_index = INTF_DESC_INDEX_FROM_ALT_SETTINGS(alt_setting);

                        PanicFalse(streaming_intf_desc_index < intf_list->intf[index].alt_settings_count);

                        format_descriptor = usbAudioClass10_GetStreamingFormatDescriptor(&(intf_list->intf[index].streaming_config[streaming_intf_desc_index]));
                        PanicNull(format_descriptor);
                        streaming_info[index].channels = format_descriptor->bNrChannels;
                        streaming_info[index].frame_size = format_descriptor->bSubframeSize;
                        streaming_info[index].streaming_desc_index = streaming_intf_desc_index;
                    }
...
static usb_fn_tbl_uac_if uac10_fn_tbl = {
    ...
    .SetInterface = usbAudioClass10_SetInterface,
