...
Task usb_audio_client_cb[USB_AUDIO_REGISTERED_CLIENT_COUNT] = {NULL};
...
static const usb_fn_tbl_uac_if * usbAudioGetFnTbl(usb_audio_class_rev_t rev)
{
    ...
    switch(rev)
    {
        ...
        default:
            DEBUG_LOG("Unsupported USB Class Revision 0x%x", rev);
            break;
    }
...
static void usbAudioDisconnectAudioMsg(usb_audio_info_t *usb_audio)
{
    ...
    /* Inform Status watcher */
    USB_AUDIO_MSG_SEND(usb_audio_client_cb[USB_AUDIO_REGISTERED_CLIENT_STATUS],
                       USB_AUDIO_DISCONNECTED_IND,
                       (Message)(&usb_audio->headphone->audio_source));
}
...
static void usbAudioConnectAudioMsg(usb_audio_info_t *usb_audio)
{
    ...
    /* with respect to audio context, this means host is actively playing audio */
    UsbSource_SetAudioContext(context_audio_is_playing);
...
static void usbAudioDisconnectVoiceMsg(usb_audio_info_t *usb_audio)
{
    ...
    /* Stop any ongoing silence stream */
    UsbAudioSilence_SetStreamState(usb_audio, FALSE);
...
static void usbAudioConnectVoiceMsg(usb_audio_info_t *usb_audio)
{
    ...
    /* Start streaming silence into the headset MIC sink,
     * until the application routes a chain to it */
    UsbAudioSilence_SetStreamState(usb_audio, TRUE);
...
static void usbAudioUpdateConnections(void)
{
    ...
    if (headset_audio)
    {
        if (headset_audio->headset->mic_active || headset_audio->headset->spkr_active)
        {
            ...
            if (headset_audio->headset->source_connected && (new_voice_speaker != prev_voice_speaker))
            {
...
static void usbAudioUpdateDeviceStatus(usb_audio_info_t *usb_audio, uint8 interface_index)
{
    ...
    uint16 alt_setting = streaming_info->alt_setting;
    bool headphone_alt_setting_changed = FALSE, headset_alt_setting_changed = FALSE;

    switch(intf_config->type)
    {
        case USB_AUDIO_DEVICE_TYPE_VOICE_SPEAKER:
            DEBUG_LOG_INFO("USB Voice: Spkr %x, %x", streaming_info->interface, alt_setting);
            PanicNull(usb_audio->headset);
            usb_audio->headset->spkr_active = alt_setting ? TRUE : FALSE;
            headset_alt_setting_changed = TRUE;
            break;
        case USB_AUDIO_DEVICE_TYPE_VOICE_MIC:
            DEBUG_LOG_INFO("USB Voice: Mic %x, %x",  streaming_info->interface, alt_setting);
            PanicNull(usb_audio->headset);
            usb_audio->headset->mic_active = alt_setting ? TRUE : FALSE;
            headset_alt_setting_changed = TRUE;
            break;
        case USB_AUDIO_DEVICE_TYPE_AUDIO_SPEAKER:
            DEBUG_LOG_INFO("USB Audio: Spkr %x, %x", streaming_info->interface, alt_setting);
            PanicNull(usb_audio->headphone);
            usb_audio->headphone->spkr_active = alt_setting ? TRUE : FALSE;
            headphone_alt_setting_changed = TRUE;
#ifdef USB_SUPPORT_HEADPHONE_SPKR_IN_VOICE_CHAIN
            /* If alt_setting is changed between two non-zero value,
             * audio chain need to be recreated with new parameters */
            if(alt_setting != INTF_ALT_SETTING_ZERO)
            {
                usb_audio_info_t *headset_audio = UsbAudioFd_GetHeadsetInfo(voice_source_usb);
                if(headset_audio && headset_audio->headset->source_connected &&
                        headset_audio->headset->alt_spkr_connected)
                {
                    usbAudioDisconnectVoiceMsg(headset_audio);
                }
            }
        ...
        case USB_AUDIO_DEVICE_TYPE_AUDIO_MIC:
            DEBUG_LOG_INFO("USB Audio: Mic %x, %x",  streaming_info->interface, alt_setting);
            PanicNull(usb_audio->headphone);
            usb_audio->headphone->mic_active = alt_setting ? TRUE : FALSE;
            headphone_alt_setting_changed = TRUE;
            break;
        default:
    ...
    if(alt_setting != INTF_ALT_SETTING_ZERO)
    {
        if(headset_alt_setting_changed && usb_audio->headset->source_connected &&
                usb_audio->headset->chain_active)
        {
            usbAudioDisconnectVoiceMsg(usb_audio);
        }

        if(headphone_alt_setting_changed && usb_audio->headphone->source_connected &&
...
static void usbAudioUpdateHeadsetVolume(usb_audio_info_t *usb_audio)
{
    if(usb_audio->headset != NULL && (usb_audio->headset->spkr_active || usb_audio->headset->mic_active))
    {
        ...
        USB_AUDIO_MSG_SEND(usb_audio_client_cb[USB_AUDIO_REGISTERED_CLIENT_STATUS],
                           (in_mute)?USB_AUDIO_HEADSET_MIC_MUTE_ACTIVE:USB_AUDIO_HEADSET_MIC_MUTE_INACTIVE,
                           (Message)(&usb_audio->headphone->audio_source));
    }
}
...
static void usbAudioSetDeviceSamplingRate(usb_audio_info_t *usb_audio, uint8 interface_index)
{
    ...
    if(is_headset_rate_modified && usb_audio->headset->source_connected &&
            usb_audio->headset->chain_active)
    {
        if (usb_audio_client_cb[USB_AUDIO_REGISTERED_SAMPLE_RATE] == NULL)
        {
            usbAudioDisconnectVoiceMsg(usb_audio);
        ...
        {
            USB_AUDIO_MSG_SEND(usb_audio_client_cb[USB_AUDIO_REGISTERED_SAMPLE_RATE],
    ...
    if(is_headphone_rate_modified)
    {
        if(usb_audio->headphone->source_connected)
        {
            if (usb_audio_client_cb[USB_AUDIO_REGISTERED_SAMPLE_RATE])
            {
            ...
            else if (usb_audio->headphone->chain_active)
            {
                ...
                headset_audio->headset->alt_spkr_connected)
        {
            ...
            else
            {
                USB_AUDIO_MSG_SEND(usb_audio_client_cb[USB_AUDIO_REGISTERED_SAMPLE_RATE],
                                   USB_AUDIO_SAMPLE_RATE_VOICE_IND,
                                   (Message)(&headset_audio->headset->audio_source));
            }
        }
#endif
    }
                ...
                case MESSAGE_USB_ALT_INTERFACE:
                    {
                        ...
                        usb_audio->usb_fn_uac->SetInterface(usb_audio->class_ctx, ind->interface, ind->altsetting);
                    }
                    break;
                                ...
                                uac_message_t uac_message)
{
    ...
    DEBUG_LOG_DEBUG("UsbAudio_ClassEvent intf_index %d  enum:message:%d", interface_index, uac_message);
...
bool UsbAudio_SendPlayStatusToMediaService(audio_source_t source)
{
    usb_audio_info_t *headphone_audio = UsbAudioFd_GetHeadphoneInfo(source);
    if (headphone_audio != NULL)
    {
        MessageSend(usb_audio_client_cb[USB_AUDIO_REGISTERED_CLIENT_MEDIA],
                USB_AUDIO_MEDIASERVICE_PLAY_IND, NULL);
        return TRUE;
    }
    return FALSE;
}

bool UsbAudio_SendPauseStatusToMediaService(audio_source_t source)
{
    usb_audio_info_t *headphone_audio = UsbAudioFd_GetHeadphoneInfo(source);
    if (headphone_audio != NULL)
    {
        MessageSend(usb_audio_client_cb[USB_AUDIO_REGISTERED_CLIENT_MEDIA],
                USB_AUDIO_MEDIASERVICE_PAUSE_IND, NULL);
        return TRUE;
    }
    return FALSE;
}
