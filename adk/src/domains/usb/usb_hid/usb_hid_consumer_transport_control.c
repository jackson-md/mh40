...
static bool usbHid_HandleConsumerSetReport(uint8 report_id, uint8 data_size, const uint8 *data)
{
    ...
    switch (report_id)
    {
        case USB_HID_LED_MUTE_REPORT_ID:
            ...
            break;
        case USB_HID_LED_HOLD_REPORT_ID:
            DEBUG_LOG("UsbHid:CT USB_HID_LED_HOLD_REPORT_ID 0x%X",data[1]);
            if(usb_hid_event_handler)
            {
                usb_hid_event_handler(USB_SOURCE_RX_HID_HOLD_EVT, data[1]);
            }
            break;
...
static usb_result_t usbHid_ConsumerTransporControl_SendEvent(usb_source_control_event_t event)
{
    ...
    DEBUG_LOG_INFO("UsbHid:CT send event enum:usb_source_control_event_t:%d", event);
