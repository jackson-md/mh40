...
\file
\ingroup    cc_case_channel
\brief      Public interface for the handling the Case Channel Communications
*/
/*! @{ */
...
#define EARBUD_STATUS_FLAGS_DA_MASK                 (0x1 << 2)
#define EARBUD_STATUS_FLAGS_COMMAND_REQUESTED_MASK  (0x1 << 3)
...
/*! Definition of the common fields in the #CASE_CHANNEL_MID_CASE_COMMAND_RESPONSE message. */
/*! @{ */
#define CASE_CMD_RESP_TYPE_SIZE             (1)
#define CASE_CMD_RESP_TYPE_OFFSET           (0)
#define CASE_CMD_RESP_PAYLOAD_OFFSET        (1)
/*! @} */
...
/*! Definition of the fields in the EB_CMD_HANDSET_PAIRING type CASE_CHANNEL_MID_COMMAND message. */
/*! @{ */
#define EARBUD_CMD_HANDSET_PAIRING_TOTAL_SIZE     (EARBUD_CMD_TYPE_SIZE)
/*! @} */

/*! Definition of the fields in the EB_CMD_HANDSET_PAIRING response CASE_CHANNEL_MID_COMMAND_RESPONSE message. */
/*! @{ */
#define EARBUD_CMD_RESP_HANDSET_PAIRING_PAYLOAD_SIZE      (1)
#define EARBUD_CMD_RESP_HANDSET_PAIRING_PAYLOAD_OFFSET    (1)
#define EARBUD_CMD_RESP_HANDSET_PAIRING_ACCEPTED          (0x1)
#define EARBUD_CMD_RESP_HANDSET_PAIRING_REJECTED          (0x0)
#define EARBUD_CMD_RESP_HANDSET_PAIRING_TOTAL_SIZE        (EARBUD_CMD_RESP_TYPE_SIZE + EARBUD_CMD_RESP_HANDSET_PAIRING_PAYLOAD_SIZE)
/*! @} */

/*! Definition of the fields in the EB_CMD_COMMAND_REQUEST response CASE_CHANNEL_MID_COMMAND_RESPONSE message. */
/*! @{ */
#define EARBUD_CMD_RESP_COMMAND_REQUEST_PAYLOAD_SIZE      (1)
#define EARBUD_CMD_RESP_COMMAND_REQUEST_PAYLOAD_OFFSET    (1)
#define EARBUD_CMD_RESP_COMMAND_REQUEST_TOTAL_SIZE        (EARBUD_CMD_RESP_TYPE_SIZE + EARBUD_CMD_RESP_COMMAND_REQUEST_PAYLOAD_SIZE)
...
typedef enum
{
    ...
    /*! Response to command by Case to Earbud, for requested operation.
        Not all commands require a response. */
    CASE_CHANNEL_MID_CASE_COMMAND_RESPONSE  = 7,
} case_channel_mid_t;
...
typedef enum
{
    ...
    /*! Command the Earbud to start handset pairing. */
    EB_CMD_HANDSET_PAIR       = 1,

    /*! Command the Earbud to enter shipping mode. */
    EB_CMD_SHIPPING_MODE      = 2,

    /*! Command the Earbud to send a case command. */
    EB_CMD_COMMAND_REQUEST    = 3,
} earbud_cmd_t;
...
static uint8 ccCaseChannel_EarbudStatusFlags(void)
{
    ...
    /* flag indicating if the earbud is paired with a peer. */
    if (BtDevice_IsPairedWithPeer())
    {
        info |= EARBUD_STATUS_FLAGS_PP_MASK;
    }

    /* Flag indicating that the earbud wishes to send a case command */
    if (ccWithCase_HavePendingCommand())
    {
        info |= EARBUD_STATUS_FLAGS_COMMAND_REQUESTED_MASK;
    }

#ifdef INCLUDE_DFU_CASE
    /* flag indicating if the earbud has a DFU file containing case image. */
    if(DfuCase_IsDfuNeeded())
    {
        info |= EARBUD_STATUS_FLAGS_DA_MASK;
    }
#endif
...
static void ccCaseChannel_HandleHandsetPairCmd(const uint8* msg, unsigned length, cc_dev_t source_dev)
{
    UNUSED(msg);
    UNUSED(source_dev);

    DEBUG_LOG_VERBOSE("ccCaseChannel_HandleHandsetPairCmd");

    if (length >= EARBUD_CMD_HANDSET_PAIRING_TOTAL_SIZE)
    {
        CcWithCase_HandsetPairingCmdRx();
    }
    else
    {
        DEBUG_LOG_VERBOSE("ccCaseChannel_HandleHandsetPairCmd bad cmd length %d", length);
        /* indicate to case that handset pairing will not be requested */
        CcCaseChannel_HandsetPairingCmdRespTx(FALSE);
    }
}

static void ccCaseChannel_HandleCommandRequest(const uint8* msg, unsigned length, cc_dev_t source_dev)
{
    UNUSED(msg);
    UNUSED(source_dev);
    UNUSED(length);

    DEBUG_LOG_VERBOSE("ccCaseChannel_HandleCommandRequest");
    ccWithCase_CaseCommandRequestRx();
}
...
static void ccCaseChannel_HandleCommand(const uint8* msg, unsigned length, cc_dev_t source_dev)
{
    if (length >= EARBUD_CMD_TYPE_SIZE)
    {
        ...
        switch (cmd)
        {
            ...
            case EB_CMD_HANDSET_PAIR:
                ccCaseChannel_HandleHandsetPairCmd(msg, length, source_dev);
                break;

            case EB_CMD_COMMAND_REQUEST:
                ccCaseChannel_HandleCommandRequest(msg, length, source_dev);
                break;
...
static void ccCaseChannel_HandleHandsetPairCmdResp(const uint8* msg, unsigned length, cc_dev_t source_dev)
{
    if (length >= EARBUD_CMD_RESP_HANDSET_PAIRING_TOTAL_SIZE)
    {
        bool resp = msg[EARBUD_CMD_RESP_HANDSET_PAIRING_PAYLOAD_OFFSET] == EARBUD_CMD_RESP_HANDSET_PAIRING_ACCEPTED ? TRUE : FALSE;
        CcWithEarbuds_HandsetPairingResponseRx(source_dev, resp);
    }
}
...
static void ccCaseChannel_HandleCommandResponse(const uint8* msg, unsigned length, cc_dev_t source_dev)
{
    if (length >= EARBUD_CMD_RESP_TYPE_SIZE)
    {
        ...
        switch (cmd)
        {
            ...
            case EB_CMD_HANDSET_PAIR:
                ccCaseChannel_HandleHandsetPairCmdResp(msg, length, source_dev);
                break;
...
static void ccCaseChannel_HandleCaseCommandResponse(const uint8* msg, unsigned length, cc_dev_t source_dev)
{
    UNUSED(source_dev);
    if (length >= CASE_CMD_RESP_TYPE_SIZE)
    {
        case_cmd_t cmd = msg[CASE_CMD_RESP_TYPE_OFFSET];

        switch(cmd)
        {
        case CASE_CMD_NONE:
            break;
        case CASE_CMD_RX_CASE_SERIAL_NUMBER:
            CcWithCase_CaseSerialNumberCmdRx(&msg[CASE_CMD_RESP_PAYLOAD_OFFSET],
                                             length - CASE_CMD_RESP_PAYLOAD_OFFSET);
            break;
        default:
            DEBUG_LOG_WARN("ccCaseChannel_HandleCaseCommandResponse unsupported command %u", cmd);
        }
    }
}
...
static void CcCaseChannel_HandleRxInd(unsigned mid, const uint8* msg, unsigned length, cc_dev_t source_dev)
{
    switch (mid)
    {
        ...
        case CASE_CHANNEL_MID_COMMAND_RESPONSE:
            ...
            break;
        case CASE_CHANNEL_MID_CASE_COMMAND_RESPONSE:
            ccCaseChannel_HandleCaseCommandResponse(msg, length, source_dev);
            break;
        default:
...
bool CcCaseChannel_HandsetPairingCmdTx(cc_dev_t dest)
{
    uint8 msg[EARBUD_CMD_HANDSET_PAIRING_TOTAL_SIZE];

    if (   (dest != CASECOMMS_DEVICE_LEFT_EB)
        && (dest != CASECOMMS_DEVICE_RIGHT_EB))
    {
        DEBUG_LOG_ERROR("CcCaseChannel_HandsetPairingCmdTx bad dest enum:cc_dev_t:%d", dest);
        return FALSE;
    }

    msg[EARBUD_CMD_TYPE_OFFSET] = EB_CMD_HANDSET_PAIR;

    /* response is expected, so use standard transmit */
    return CcProtocol_Transmit(dest, CASECOMMS_CID_CASE,
                               CASE_CHANNEL_MID_COMMAND,
                               msg, EARBUD_CMD_HANDSET_PAIRING_TOTAL_SIZE);
}

bool CcCaseChannel_HandsetPairingCmdRespTx(bool cmd_accepted)
{
    uint8 msg[EARBUD_CMD_RESP_HANDSET_PAIRING_TOTAL_SIZE];

    msg[EARBUD_CMD_RESP_TYPE_OFFSET] = EB_CMD_HANDSET_PAIR;
    msg[EARBUD_CMD_RESP_HANDSET_PAIRING_PAYLOAD_OFFSET] = cmd_accepted ? EARBUD_CMD_RESP_HANDSET_PAIRING_ACCEPTED
                                                                     : EARBUD_CMD_RESP_HANDSET_PAIRING_REJECTED;

    return CcProtocol_TransmitNotification(CASECOMMS_DEVICE_CASE, CASECOMMS_CID_CASE,
                                           CASE_CHANNEL_MID_COMMAND_RESPONSE,
                                           msg, EARBUD_CMD_RESP_HANDSET_PAIRING_TOTAL_SIZE);
}

bool CcCaseChannel_CaseCommandSerialNumberTx(void)
{
    uint8 msg[EARBUD_CMD_RESP_COMMAND_REQUEST_TOTAL_SIZE];

    msg[EARBUD_CMD_RESP_TYPE_OFFSET] = EB_CMD_COMMAND_REQUEST;
    msg[EARBUD_CMD_RESP_COMMAND_REQUEST_PAYLOAD_OFFSET] = CASE_CMD_RX_CASE_SERIAL_NUMBER;

    /* response is expected, so use standard transmit */
    return CcProtocol_Transmit(CASECOMMS_DEVICE_CASE, CASECOMMS_CID_CASE,
                               CASE_CHANNEL_MID_COMMAND_RESPONSE,
                               msg, EARBUD_CMD_RESP_COMMAND_REQUEST_TOTAL_SIZE);
}

bool CcCaseChannel_CaseCommandNoneTx(void)
{
    uint8 msg[EARBUD_CMD_RESP_COMMAND_REQUEST_TOTAL_SIZE];

    msg[EARBUD_CMD_RESP_TYPE_OFFSET] = EB_CMD_COMMAND_REQUEST;
    msg[EARBUD_CMD_RESP_COMMAND_REQUEST_PAYLOAD_OFFSET] = CASE_CMD_NONE;
