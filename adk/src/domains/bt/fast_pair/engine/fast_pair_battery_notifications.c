...
void fastPair_SendDeviceBattery(void)
{
    DEBUG_LOG("fastPair_SendDeviceBattery");
#if defined(INCLUDE_CASE_COMMS) || defined (INCLUDE_TWS)
    uint8 *data_battery = fastPair_BatteryGetData();

    if (NULL == data_battery)
    {
        DEBUG_LOG_WARN("fastPair_SendDeviceBattery: Battery info NULL");
        return;
    }
    DEBUG_LOG("fastPair_SendDeviceBattery: left %d, right %d,case %d",data_battery[FP_BATTERY_NTF_DATA_LEFT_STATE_OFFSET],
    ...
    fastPair_MsgStreamSendDataToAll(FASTPAIR_MESSAGESTREAM_MESSAGE_GROUP_DEVICE_INFORMATION_EVENT,
                                    FASTPAIR_MESSAGESTREAM_DEVINFO_EVENT_CODE_BATTERY_UPDATED,&data_battery[FP_BATTERY_NTF_DATA_LEFT_STATE_OFFSET],FP_BATTERY_NUM_VALUES);
#endif
}
...
void fastPair_BatteryHandleCasePowerState(const CASE_POWER_STATE_T* cps)
{
    ...
    bool is_peer_in_case = (cps->peer_battery_state != BATTERY_STATUS_UNKNOWN);
