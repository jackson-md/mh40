 ...
 */
static void fastPair_RingMuteDevice(uint8 fp_seeker_number)
{
        ...
        ((provider_device_type == multidevice_type_single) && (dev_action_data.ring_component == FASTPAIR_DEVICEACTION_RING_LEFT_MUTE_RIGHT)))
    {
      DEBUG_LOG("fastPair_RingMuteDevice: Invalid component 0x%x", dev_action_data.ring_component);
        ...
        fastPair_MsgStreamSendNAK(FASTPAIR_MESSAGESTREAM_MESSAGE_GROUP_DEVCIE_ACTION_EVENT, FASTPAIR_MESSAGESTREAM_DEVACTION_RING_EVENT,
                                  FASTPAIR_MESSAGESTREAM_NAK_REASON_NOT_SUPORTED, fp_seeker_number);
        return;
    }
...
static void fastPair_DevAction_HandleIncomingData(uint8 fp_seeker_number, const uint8* msg_data, uint16 len)
{
    ...
    switch(msg_code)
    {
        case FASTPAIR_MESSAGESTREAM_DEVACTION_RING_EVENT:
        {
          ...
          fastPair_MsgStreamSendACK(FASTPAIR_MESSAGESTREAM_MESSAGE_GROUP_DEVCIE_ACTION_EVENT,FASTPAIR_MESSAGESTREAM_DEVACTION_RING_EVENT, fp_seeker_number);
          ...
          if (fastPair_RfcommGetRFCommConnectedInstances() == FASTPAIR_RFCOMM_CONNECTIONS_MAX)
          {
              ...
              fp_seeker_number = (fp_seeker_number == 1) ? 2 : 1;
          ...
          fastPair_RingMuteDevice(fp_seeker_number);
        ...
        {
            /* Acknowledge message */
            fastPair_MsgStreamSendACK(FASTPAIR_MESSAGESTREAM_MESSAGE_GROUP_DEVCIE_ACTION_EVENT,msg_code, fp_seeker_number);
