...
/*! \brief Return device address of current connected device.
*/
bdaddr fastPair_RfcommGetDeviceAddr(uint8 instance_number);

/*! \brief Get RFCOMM connection instance number using bluetooth address
    \param addr Bluetooth address of remote device

    \return Instance number of RFCOMM connection for the given BD Address
*/
...
/*! \brief Send FastPair protocol data to the handset using RFCOMM.
    ...
    \param rfcomm_instance_number RFCOMM instance number
...
*/
typedef uint16 (*fastPair_RfcommMsgCallBack)(FASTPAIR_RFCOMM_MESSAGE_TYPE msg_type, uint8 rfcomm_instance_number, const uint8 *msg_data, uint16 len);
...
/*! Get Fast Pair RFCommm dlci connected with Bluetoot device.
    \param addr Bluetooth address of device connected over rfcomm dlci.

    \returns Fast Pair RFCommm dlci.
 */
