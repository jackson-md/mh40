...
/* Event arguments for Crypto AES CTR event */
typedef struct
{
    CL_CRYPTO_ENCRYPT_DECRYPT_AES_CTR_CFM_T* crypto_aes_ctr_cfm;
} fast_pair_state_event_crypto_aes_ctr_args_t;
...
/*! \brief Handle Account Key Characterstic Write by Fast Pair Seeker
    ...
    \param[in] enc_data_len Length of encrypted data written by FP seeker.
 */
void fastPair_DataWrite(const gatt_cid_t cid, const uint8 *enc_data, uint32 enc_data_len);
