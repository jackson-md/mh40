...
static bool fastPair_MatchProviderAddress(uint16 *decrypted_data)
{
    ...
    /* If provider BD address is not matching with device private address, check it with public address in case of RWA or device action events
       This could happen if Handset is in possession of LTK and IRK and during RWA or PName write/notify the handset will start sending EB's public
       address as part of KbP request. The BT Host on phone resolves the RPA of EB and provides only the public address to FP service */
    if(status == FALSE && (((FAST_PAIR_MSG_TYPE_KEY_BASED_PAIRING_REQ == kbp_data[0]) && (theFastPair->retroactively_writing_account_key == TRUE)) 
    || (FAST_PAIR_MSG_TYPE_DEVICE_ACTION_REQ == kbp_data[0])))
    {
        ...
        if(appDeviceGetMyBdAddr(&self_addr))
        {
            /*! Check if the provider address sent by seeker is matching with the device public address */
            if(BdaddrIsSame(&self_addr, &provider_addr))
            {
...
static bool fastPair_MatchSeekerAddress(uint16 *decrypted_data)
{
    ...
    if(BtDevice_GetIndexedDevice(pdl_index, &device))
    {
        ...
        if(BdaddrIsSame(&handset_addr, &seeker_addr))
        {
            ...
            /* Store the handset BD address in FP data structure to associate the account key index to it */
            memcpy(&theFastPair->handset_bd_addr, &seeker_addr, sizeof(bdaddr));
