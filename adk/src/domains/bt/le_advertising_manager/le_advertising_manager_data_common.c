...
void leAdvertisingManager_SetupAdvertData(le_adv_data_set_t set, Task task, uint8 adv_handle)
{
    const le_advertising_manager_data_packet_if_t* interface = leAdvertisingManager_GetDataPacketFromSet(set);
    
    if (interface != NULL)
    {
        interface->setupAdvertData(task, adv_handle);
    }
}

void leAdvertisingManager_SetupScanResponseData(le_adv_data_set_t set, Task task, uint8 adv_handle)
{
    const le_advertising_manager_data_packet_if_t* interface = leAdvertisingManager_GetDataPacketFromSet(set);
    
    if (interface != NULL)
    {
        interface->setupScanResponseData(task, adv_handle);
    }
}
...
bool leAdvertisingManager_RegisterDataClient(le_adv_data_set_t set, const le_advertising_manager_data_packet_if_t* interface)
{
    if (le_adv_mgr_size_data_if < LE_ADV_MGR_MAX_DATA_CLIENTS)
    {
        ...
        PanicNull((void *)interface->getDataPacket);
...
bool leAdvertisingManager_GetAdvertDataPacket(le_adv_data_set_t set, le_advertising_manager_data_packet_t *packet)
{
    const le_advertising_manager_data_packet_if_t* interface = leAdvertisingManager_GetDataPacketFromSet(set);

    if (interface)
    {
        return interface->getDataPacket(le_adv_manager_data_packet_advert, packet);
    }

    return FALSE;
}

bool leAdvertisingManager_GetScanResponseDataPacket(le_adv_data_set_t set, le_advertising_manager_data_packet_t *packet)
{
    const le_advertising_manager_data_packet_if_t* interface = leAdvertisingManager_GetDataPacketFromSet(set);

    if (interface)
    {
        return interface->getDataPacket(le_adv_manager_data_packet_scan_response, packet);
    }
