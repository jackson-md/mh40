...
bool leAdvertisingManager_GetDataUpdateRequired(const le_adv_data_set_t set)
{
    ...
    if (leAdvertisingManager_IsLegacySet(set))
    {
        update_required = adv_task_data->is_legacy_data_update_required;
    }

    if (leAdvertisingManager_IsExtendedSet(set))
    {
        update_required = adv_task_data->is_extended_data_update_required;
    }
...
void LeAdvertisingManager_ScheduleAdvertisingStart(const le_adv_data_set_t set)
{
    ...
    DEBUG_LOG_LEVEL_1("LeAdvertisingManager_ScheduleAdvertisingStart set enum:le_adv_data_set_t:%u", set);
