...
static void rssiPairing_HandleConManagerConnectionInd(const CON_MANAGER_CONNECTION_IND_T *msg)
{
    ...
    if (rssi_pairing_data.state == RSSI_PAIRING_STATE_ACL_CONNECTING &&
        BdaddrIsSame(&msg->bd_addr, &rssi_pairing_data.inquiry_results[0].tp_addr.taddr.addr) &&
        rssi_pairing_data.inquiry_results[0].tp_addr.transport == TRANSPORT_BREDR_ACL)
    {
...
static void rssiPairing_SendPairingCfm(bdaddr device_bd_addr, bool status)
{
    RSSI_PAIRING_PAIR_CFM_T* confirm_message = PanicUnlessNew(RSSI_PAIRING_PAIR_CFM_T);
...
static void rssiPairing_HandlePairingConfirm(const PAIRING_PAIR_CFM_T * message)
{
    ...
    switch (message->status){
        case pairingSuccess:
        {
           ...
           rssiPairing_SendPairingCfm(message->device_bd_addr, TRUE);
        }
        break;
        default:
        {
            rssiPairing_SendPairingCfm(message->device_bd_addr, FALSE);
        }
        break;
    }
...
bool RssiPairing_Start(Task client_task, const rssi_pairing_parameters_t *scan_parameters)
{
...
void RssiPairing_Stop(void)
{
    /* Don't reset data yet - still need to send a final RSSI_PAIRING_PAIR_CFM
       to the client_task. Just reset state, then request inquiry to stop. */
    rssi_pairing_data.state = RSSI_PAIRING_STATE_IDLE;
    InquiryManager_Stop();
    /* Wait for INQUIRY_MANAGER_SCAN_COMPLETE */
}
