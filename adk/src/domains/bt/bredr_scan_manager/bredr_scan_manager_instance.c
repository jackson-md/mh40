...
static void bredrScanManager_ConditionallySendPausedInd(bsm_scan_context_t *context)
{
    ...
    {
        if ((context != bredrScanManager_TruncatedPageScanContext() && bredrScanManager_IsDisabled()) ||
            (context == bredrScanManager_TruncatedPageScanContext() && bredrScanManager_IsTpsPauseRequested()))
        {
            MessageId id;
            task_list_t *clients = TaskList_GetBaseTaskList(&context->clients);
                                              ...
                                              bsm_scan_enable_state_t new_state)
{
    ...
    DEBUG_LOG_VERBOSE("bredrScanManager_InstanceSetState context_offset=%d enum:bsm_scan_enable_state_t:%d",
                      context->message_offset, new_state);

    if(new_state & (BSM_SCAN_DISABLED))
    {
        context->lock = 0;
    }
    else
    ...
    {
        if (context == bredrScanManager_TruncatedPageScanContext())
        {
            QcomConManagerEnableTruncatedScan(&bredr_scan_manager.task_data, !!(new_state & BSM_SCAN_ENABLING));
        }
        else
        {
            /* Disabling or enabling page or inquiry scanning requires a new call to the
               connection library ConnectionWriteScanEnable function, which may enable or
               disable scanning. */
    ...
    if (context == bredrScanManager_PageScanContext() ||
        context == bredrScanManager_TruncatedPageScanContext())
    {
                                                        ...
                                                        bredr_scan_manager_scan_type_t type)
{
    ...
    if (0 != memcmp(params, &context->scan_params, sizeof(*params)))
    {
        ...
        /* Synchronise regular page scan and truncated page scan parameters,
        since both use effectively only one common set on controller side */
        if (context == bredrScanManager_PageScanContext())
        {
            bredrScanManager_TruncatedPageScanContext()->scan_params = *params;

        }
        else if(context == bredrScanManager_TruncatedPageScanContext())
        {
            bredrScanManager_PageScanContext()->scan_params = *params;
        }
    }
}
...
static void bredrScanManager_InstanceSetGoal(bsm_scan_context_t *context, bool enable,
                                             bredr_scan_manager_scan_type_t type)
{
    DEBUG_LOG_VERBOSE("bredrScanManager_InstanceSetGoal enable=%d enum:bredr_scan_manager_scan_type_t:%d"
                      " enum:bsm_scan_enable_state_t:%d", enable, type, context->state);

    if (enable)
    {
        bredrScanManager_InstanceUpdateScanActivity(context, type);
        /* Save the active scan type requested by client
         * and synchronise regular page scan and truncated page scan types,
        since both use effectively only one common type on controller side */
        context->type = type;
        if (context == bredrScanManager_PageScanContext())
        {
            bredrScanManager_TruncatedPageScanContext()->type = type;

        }
        else if(context == bredrScanManager_TruncatedPageScanContext())
        {
            bredrScanManager_PageScanContext()->type = type;
        }
    }
...
void bredrScanManager_InstanceRefresh(bsm_scan_context_t *context)
{
    ...
    if (bredrScanManager_IsDisabled() || (context == bredrScanManager_TruncatedPageScanContext() && bredrScanManager_IsTpsPauseRequested())
         || (TaskList_Size(clients) == 0))
    {
        goal = FALSE;
    }
    else
    ...
    if (max_type == SCAN_MAN_PARAMS_TYPE_PENDING_RELEASE)
    {
        goal = FALSE;
        max_type = SCAN_MAN_PARAMS_TYPE_SLOW;
    }
...
void bredrScanManager_InstanceCompleteTransition(bsm_scan_context_t *context)
{
    if (context->state & BSM_SCAN_DISABLING)
    {
        ...
        MessageId id;
        task_list_t *clients = TaskList_GetBaseTaskList(&context->clients);
        id = BREDR_SCAN_MANAGER_PAGE_SCAN_RELEASED_IND + context->message_offset;
        TaskList_MessageSendId(clients, id);
