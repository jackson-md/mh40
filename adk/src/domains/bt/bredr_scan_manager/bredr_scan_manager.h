...
\version    
\file
\defgroup   bredr_scan_manager BR/EDR Sacn Manager
\ingroup    bt_domain
...
#define TRUNCATED_PAGE_SCAN_MESSAGE_OFFSET (INQ_SCAN_MESSAGE_OFFSET + 1)
...
enum bredr_scan_manager_messages
{
    ...
    /*! Indication inquiry scan has paused - another client has forced-off the scan */
    BREDR_SCAN_MANAGER_TRUNCATED_PAGE_SCAN_PAUSED_IND = BREDR_SCAN_MANAGER_PAGE_SCAN_PAUSED_IND + TRUNCATED_PAGE_SCAN_MESSAGE_OFFSET,
    ...
    /*! Indication inquiry scan has resumed - the client has released the force-off. */
    BREDR_SCAN_MANAGER_TRUNCATED_PAGE_SCAN_RESUMED_IND = BREDR_SCAN_MANAGER_PAGE_SCAN_RESUMED_IND + TRUNCATED_PAGE_SCAN_MESSAGE_OFFSET,

    /*! Indication page scan has now been terminated.*/
    BREDR_SCAN_MANAGER_PAGE_SCAN_RELEASED_IND,

    /*! Indication inquiry scan has now been terminated. */
    BREDR_SCAN_MANAGER_INQUIRY_SCAN_RELEASED_IND = BREDR_SCAN_MANAGER_PAGE_SCAN_RELEASED_IND + INQ_SCAN_MESSAGE_OFFSET,

    /*! Indication inquiry scan has now been terminated. */
    BREDR_SCAN_MANAGER_TRUNCATED_PAGE_SCAN_RELEASED_IND = BREDR_SCAN_MANAGER_PAGE_SCAN_RELEASED_IND + TRUNCATED_PAGE_SCAN_MESSAGE_OFFSET,
    ...
    /*! Confirmation truncated page scanning has been paused - sent to the client that requested
        the pause using #BredrScanManager_PauseTruncatedPageScan once scanning has been paused. */
    BREDR_SCAN_MANAGER_TPS_PAUSED_CFM,
...
/*! Message content, confirming successful pause or cancellation. */
typedef struct
{
    /*! TRUE = paused, FALSE = pause was cancelled */
    bool paused;

} BREDR_SCAN_MANAGER_TPS_PAUSED_CFM_T;
...
typedef enum
{
    /*! Indicates a client that has requested release, for which we shouldn't enable
        scan. It shall be removed once the bredr scan manager recieves indication
        that the scan has been released. */
    SCAN_MAN_PARAMS_TYPE_PENDING_RELEASE,
...
/*! @brief Request truncated page scanning for a specified client.

    @param client      Client type requesting the truncated page scan.
    @param page_type   Parameters for the requested truncated page scan.
*/
void BredrScanManager_TruncatedPageScanRequest(Task client, bredr_scan_manager_scan_type_t page_type);

/*! @brief Release truncated page scanning for a specified client.

    @param client      Client type requesting release of the truncated page scan.
*/
