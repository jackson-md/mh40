...
/*! @brief Disable truncated page scanning

    @param disabler The task requesting the disable.

    Any active truncated page scanning will be paused until
    #BredrScanManager_ResumeTruncatedPageScan is called. The disabler task will be
    sent a #BREDR_SCAN_MANAGER_PAUSE_TPS_CFM_T when scanning has been paused
    successfuly (the paused field will be set to TRUE).

    If #BredrScanManager_ResumeTruncatedPageScan is called before scanning has paused,
    a #BREDR_SCAN_MANAGER_PAUSE_TPS_CFM_T message will be sent to the disabler task
    with the disabled field set to FALSE.
    ...
    to be enabled.
*/
void BredrScanManager_PauseTruncatedPageScan(Task disabler);

/*! @brief Allow truncated page scan to resume.
*/
