...
void BtDevice_GetDeviceData(device_t device, bt_device_pdd_t *device_data)
{
    ...
    Device_GetPropertyU8(device, device_property_sass_switch_disabled, &device_data->sass_switch_disabled);
    ...
    Device_GetPropertyU32(device, device_property_v1_handset_service_config, &device_data->handset_service_config);
    Device_GetPropertyU8(device, device_property_analog_audio_volume, &device_data->analog_audio_volume);
    Device_GetPropertyU8(device, device_property_handset_account_key_index, &device_data->handset_account_key_index);
    Device_GetPropertyU8(device, device_property_gatt_service_discovery, &device_data->gsd_completed);
}

void BtDevice_SetDeviceData(device_t device, const bt_device_pdd_t *device_data)
{
    DEBUG_LOG("BtDevice_SetDeviceData device-type enum:deviceType:%d", device_data->type);
    ...
    if (device_data->type == DEVICE_TYPE_HANDSET_LE)
    {
        /* DEVICE_TYPE_HANDSET_LE is an unpaired device and expected to contain only few properties.
           Set only those properties below. */
        Device_SetPropertyU8(device, device_property_gatt_service_discovery, device_data->gsd_completed);
        return;
    }
    ...
    switch(device_data->type)
    {
        ...
        case DEVICE_TYPE_HANDSET:
            ...
            Device_SetPropertyU32(device, device_property_supported_profiles, device_data->supported_profiles);
            Device_SetPropertyU8(device, device_property_handset_account_key_index, device_data->handset_account_key_index);
            Device_SetPropertyU8(device, device_property_gatt_service_discovery, device_data->gsd_completed);
            break;

        case DEVICE_TYPE_SELF:
            Device_SetPropertyU32(device, device_property_v1_handset_service_config, device_data->handset_service_config);
            Device_SetPropertyU8(device, device_property_analog_audio_volume, device_data->analog_audio_volume);
            Device_SetPropertyU32(device, device_property_supported_profiles, device_data->supported_profiles);
            Device_SetPropertyU8(device, device_property_sass_switch_disabled, device_data->sass_switch_disabled);
            break;
