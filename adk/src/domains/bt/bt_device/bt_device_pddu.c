...
static uint16 BtDevice_SerialisePddu(bt_device_pdd_t *pdd, uint8* buf)
{
    ...
    btDevice_SerialiseVariable(pdd->handset_service_config, sizeof(pdd->handset_service_config), buf, &index);
    btDevice_SerialiseVariable(pdd->analog_audio_volume, sizeof(pdd->analog_audio_volume), buf, &index);
    btDevice_SerialiseVariable(pdd->handset_account_key_index, sizeof(pdd->handset_account_key_index), buf, &index);
    btDevice_SerialiseVariable(pdd->sass_switch_disabled, sizeof(pdd->sass_switch_disabled), buf, &index);
    btDevice_SerialiseVariable(pdd->gsd_completed, sizeof(pdd->gsd_completed), buf, &index);
...
{
    uint8 size_of_properties = sizeof(device_data->handset_service_config) + sizeof(device_data->analog_audio_volume);
    return index + size_of_properties <= data_length ? TRUE : FALSE;
}

static void btDevice_Load211Properties(bt_device_pdd_t *device_data, uint8 *buf, uint16 *index)
{
    btDevice_DeserialisePddAndIncrementIndexU32(&device_data->handset_service_config, buf, index);
...
static void btDevice_Set211Defaults(bt_device_pdd_t *device_data)
{
    ...
    if(default_value_callback_list)
    {
        ...
        {
            if(device_property_v1_handset_service_config == default_value_callback_list->callback[i].property)
            {
                default_value_callback_list->callback[i].DefaultValueCallback(&device_data->handset_service_config, sizeof(device_data->handset_service_config));
                break;
            }
        }
    }
...
static bool btDevice_IsEnoughDataFor222Properties(bt_device_pdd_t *device_data, uint16 index, uint8 data_length)
{
    uint8 size_of_properties = sizeof(device_data->handset_account_key_index) + sizeof(device_data->sass_switch_disabled) + 
                               sizeof(device_data->gsd_completed);
    return index + size_of_properties <= data_length ? TRUE : FALSE;
}

static void btDevice_Load222Properties(bt_device_pdd_t *device_data, uint8 *buf, uint16 *index)
{
    btDevice_DeserialisePddAndIncrementIndexU8(&device_data->handset_account_key_index, buf, index);
    btDevice_DeserialisePddAndIncrementIndexU8(&device_data->sass_switch_disabled, buf, index);
    btDevice_DeserialisePddAndIncrementIndexU8(&device_data->gsd_completed, buf, index);
}

static void btDevice_Set222Defaults(bt_device_pdd_t *device_data)
{
    DEBUG_LOG_ALWAYS("btDevice_Set222Defaults");
    device_data->handset_account_key_index = 0xFF;
    device_data->sass_switch_disabled = 0;
    device_data->gsd_completed = 0;
}
...
bt_device_deser_callbacks_t deser_fns[] = {
    ...
    {
        .IsEnoughDataForProperties = btDevice_IsEnoughDataFor222Properties,
        .LoadProperties = btDevice_Load222Properties,
        .SetDefaults = btDevice_Set222Defaults,
        .version_id = 0x2220
    },
};
