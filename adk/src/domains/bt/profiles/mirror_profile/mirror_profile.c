...
static bool mirrorProfile_UpdateTargetDevice(void)
{
    ...
    else
    {
        ...
        {
            /* Fall back to the first connected handset */
            device_t *connected_handsets = NULL;
            if(BtDevice_GetConnectedBredrHandsets(&connected_handsets))
            {
                target_device = connected_handsets[0];
            }
    ...
    {
        MIRROR_LOG("mirrorProfile_UpdateTargetDevice focused_src=(enum:source_type_t:%d, enum:audio_source_t:%d) target_device=0x%p",
                   routed_source.type, routed_source.u.audio, target_device);
    }
    else
    {
        MIRROR_LOG("mirrorProfile_UpdateTargetDevice focused_src=(enum:source_type_t:%d, enum:voice_source_t:%d) target_device=0x%p",
                   routed_source.type, routed_source.u.audio, target_device);
    }
