...
typedef enum
{
    ...
    /*! Le audio unicast connection indication sent by primary and secondary when its own CIS gets connected. */
    MIRROR_PROFILE_UNICAST_CIS_CONNECT_IND,

    /*! Le audio unicast connection indication sent by primary and secondary when its own CIS gets disconnected. */
    MIRROR_PROFILE_UNICAST_CIS_DISCONNECT_IND,
...
/*! \brief Test if any mirror ACL, eSCO or A2DP connection(s) are connected.
    ...
    \return TRUE if any BR/EDR mirror connection is connected; FALSE otherwise.
*/
...
/*! \brief Get address of mirrored handset device.
        BR/EDR ACL links are mirrored for audio/voice streaming.
        For LE-Audio, only CISes are mirrored/delegated but LE ACL will never be mirrored.
        Hence the returned address represent BR/EDR device which is being mirrored

    \return Address of BR/EDR device being mirrored, otherwise returned address shall be empty.
*/
bdaddr * MirrorProfile_GetMirroredDeviceAddress(void);


/*! \brief Check if the requested tpaddr of device is the same mirroring device.
    \param[in] tpaddr Pointer to address of device which shall be matched against mirroring device.

    \return TRUE, if the input address of device matches with mirroring device. FALSE otherwise.
    Note: For LE-Audio, only CISes are mirrored/delegated but LE ACL will never be mirrored.
    Hence only mirrored BR/EDR device is considered for matching.
*/
...
#define MirrorProfile_IsBredrMirroringConnected() (FALSE)
...
#define MirrorProfile_IsSameMirroredDevice(tpaddr) FALSE

#define MirrorProfile_GetLeAudioUnicastContext() ((uint16) 0x0)

#define MirrorProfile_GetCisHandle(own_handle, peer_handle) /* Nothing to do */

#define MirrorProfile_ShouldEscoAudioStartSynchronously(source) FALSE

#define MirrorProfile_GetCisMirroredDeviceAddress() ((bdaddr *)NULL)
