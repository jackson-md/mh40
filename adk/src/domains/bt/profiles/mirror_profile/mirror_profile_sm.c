...
static void mirrorProfile_EnterAclConnected(void)
{
    ...
    if (MirrorProfile_IsPrimary())
    {
        ...
        if (source != audio_source_none && MirrorProfile_StoreAudioSourceParameters(source))
        {
            ...
            MirrorProfile_SendA2dpStreamContextToSecondaryBlockUntilResponse(source);
        }
        else
        {
...
static void mirrorProfile_ExitA2dpConnected(void)
{
    ...
    if (MirrorProfile_IsPrimary())
    {
        ...
        if(MirrorProfile_GetTargetA2dpSource() != a2dp->audio_source)
        {
            if(mirrorProfile_GetAudioSyncState(a2dp->audio_source) == AUDIO_SYNC_STATE_ACTIVE)
            {
                /* Ensure we cannot resume forwarding on this source before the A2DP graph 
                   has beenn created. */
                mirrorProfile_SetAudioSyncState(a2dp->audio_source, AUDIO_SYNC_STATE_READY);
            }
        }
        MirrorProfile_StopA2dpAudioSynchronisation();
        mirrorProfile_UnregisterForMirroredSourceVolume();
...
static void mirrorProfile_EnterA2dpConnecting(void)
{
    ...
    if (MirrorProfile_IsPrimary())
    {
        MirrorProfile_MirrorL2capConnectReq(sp->acl.conn_handle, a2dp->cid);
