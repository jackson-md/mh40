...
static aghfpTaskData aghfpData;
...
static void aghfpProfileInstance_InitTaskData(aghfpInstanceTaskData* instance)
{
    ...
    AghfpProfilePort_InitInstanceData(instance);
...
static void aghfpProfileInstance_HandleInternalHfpRingRequest(AGHFP_INTERNAL_HFP_RING_REQ_T * req)
{
    ...
    case AGHFP_STATE_CONNECTED_INCOMING:
        AghfpProfileAbstract_SendRingAlert(instance, instance->bitfields.caller_id_active_host, instance->bitfields.caller_id_active_remote);
        if(instance->bitfields.caller_id_active_host && instance->bitfields.caller_id_active_remote)
        {
            AghfpProfileAbstract_SendCallerId(instance);
        }    
...
static void aghfpProfileInstance_HandleInternalCallAccept(AGHFP_INTERNAL_HFP_CALL_ACCEPT_REQ_T* req)
{
    ...
    {   
        ...
        {
            AghfpProfileAbstract_SendCallHeldIndication(req->instance);
        }

        AghfpProfileAbstract_SendCallSetupIndication(req->instance);
...
static void aghfpProfileInstance_HandleInternalCallHangup(AGHFP_INTERNAL_HFP_CALL_HANGUP_REQ_T* req)
{
    ...
    if (!AghfpProfileCallList_TerminateActiveCall(req->instance->call_list))
    {
        /* No active call, but there may be one still in the setup phase.
           Try rejecting an outgoing (but not yet active) call instead. */
        AghfpProfileCallList_OutgoingCallRejected(req->instance->call_list);
    }
    ...
    switch (state)
    {
        ...
        case AGHFP_STATE_CONNECTED_ACTIVE:
            ...
            else if (req->instance->bitfields.call_hold == aghfp_call_held_active)
            {
...
static void aghfpProfileInstance_HandleInternalToggleMicMute(AGHFP_INTERNAL_HFP_TOGGLE_MIC_MUTE_REQ_T* message)
{
    DEBUG_LOG_FN_ENTRY("aghfpProfileInstance_HandleInternalToggleMicMute");
    aghfpInstanceTaskData *instance = message->instance;

    instance->bitfields.mic_mute = !instance->bitfields.mic_mute;
    uint8 gain = instance->bitfields.mic_mute ? 0 : instance->mic_gain;
...
void AghfpProfileInstance_DeregisterVoiceSourceInterfaces(voice_source_t voice_source)
{
    VoiceSources_DeregisterTelephonyControlInterface(voice_source,
                                                     AghfpProfile_GetTelephonyControlInterface());
}
...
void AghfpProfileInstance_Destroy(aghfpInstanceTaskData *instance)
{
    ...
    if (AghfpProfile_IsDisconnected(instance))
    {
        ...
        AghfpProfilePort_DeinitInstanceData(instance);
...
aghfpInstanceTaskData * AghfpProfileInstance_GetInstanceForDevice(device_t device)
{
    ...
    DEBUG_LOG_VERBOSE("AghfpProfileInstance_GetInstanceForDevice device=%p has no device_property_aghfp_instance", device);
    return NULL;
}
...
static void aghfpProfile_InstanceHandleMessage(Task task, MessageId id, Message message)
{
   ...
   case AGHFP_INTERNAL_HFP_RELEASE_HELD_CALL_REQ:
        ...
        break;
    case AGHFP_INTERNAL_HFP_TOGGLE_MIC_MUTE_REQ:
        aghfpProfileInstance_HandleInternalToggleMicMute((AGHFP_INTERNAL_HFP_TOGGLE_MIC_MUTE_REQ_T*)message);
        break;
    }
