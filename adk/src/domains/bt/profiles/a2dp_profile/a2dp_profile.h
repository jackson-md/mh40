...
#define APP_A2DP_AUDIO_SYNC_LOCK    8U
/*!@} */
...
/*! \brief Set audio sync lock bit waiting for sync client to prepare for the interface to be destroyed */
#define appA2dpSetAudioSyncLockBit(theInst) (theInst)->a2dp.lock |= APP_A2DP_AUDIO_SYNC_LOCK
/*! \brief Clear audio sync lock bit waiting for sync client to prepare for the interface to be destroyed */
#define appA2dpClearAudioSyncLockBit(theInst) (theInst)->a2dp.lock &= ~APP_A2DP_AUDIO_SYNC_LOCK
...
/*! 
    ...
    \param[in] the_inst   AV instance to be Vetoed. refernce \ref avInstanceTaskData
    \return bool          TRUE to veto handover.
*/
...
/*! \brief Set preferred sample rate for the specified AV instance

    During media channel establishment, A2DP will attempt to negotiate the
    preferred sample rate given here, if supported by the selected codec and
    both sides of the link. Currently only the aptX Adaptive source encoder
    supports this feature.

    For codecs that do support the feature, A2DP will never negotiate a sample
    rate better than the expressed preference (it acts as a hard upper limit).
    This may result in that codec being rejected in favour of another, if there
    is no common supported rate that is equal to or lower than the preference.
    Higher/better rates can always fall back to lower rates if required, though.

    If a media channel is already established, then calling this function may
    result in an AVDTP reconfigure command being issued, renegotiating the codec
    parameters to take account of the new preference. In this case, the function
    returns TRUE. If the preference made no difference, FALSE is returned.

    \note Renegotiation may suspend/resume the media channel, as necessary.

    \param av_instance The instance for which the preferred sample rate applies.
    \param rate Preferred sample rate to negotiate, if possible. A value of 0
                represents no preference (the default), which will negotiate
                the best common rate available between source & sink.

    \return TRUE if the change in preference resulted in a reconfiguration of
            the media channel (which will now be in progress), FALSE otherwise.
*/
bool A2dpProfile_SetPreferredSampleRate(avInstanceTaskData *av_instance, uint32 rate);

/*! \brief Get preferred sample rate for the specified AV instance

    Retrieve the value previously set via A2dpProfile_SetPreferredSampleRate().

    \param av_instance The instance for which to get the preferred sample rate.

    \return The preferred sample rate. A value of 0 indicates no preference.
*/
