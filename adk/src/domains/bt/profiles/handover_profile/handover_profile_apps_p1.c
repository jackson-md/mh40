...
/*! Check if the handover client has registered for transport of device being handover */
#define handoverProfile_IsClientSupportTransport(ho_client, handover_transport) \
    (ho_client->supportedType == HANDOVER_SUPPORT_ALL || ho_client->supportedType == handover_transport)

handover_profile_status_t handoverProfile_VetoP1ClientsGlobal(void)
{
    unsigned counter = 0;
...
handover_profile_status_t handoverProfile_VetoP1ClientsPerLink(void)
{
    unsigned counter = 0;
    FOR_EACH_HANDOVER_CLIENT(ho_client)
    {
        ...
        {
            if (ho_client->pFnVetoLink &&
                (ho_client->supportedType == device->addr.transport ||
                 ho_client->supportedType == HANDOVER_SUPPORT_ALL) &&
                ho_client->pFnVetoLink(&device->addr))
            {
                DEBUG_LOG("handoverProfile_VetoP1ClientsPerLink vetoed by client %d for device lap=0x%x",
                          counter, device->addr.taddr.addr.lap);
                return HANDOVER_PROFILE_STATUS_HANDOVER_VETOED;
            }
        }
        counter++;
...
handover_profile_status_t handoverProfile_VetoP1Clients(void)
{
    ...
    status = handoverProfile_VetoP1ClientsGlobal();
    if (status == HANDOVER_PROFILE_STATUS_SUCCESS)
    {
        status = handoverProfile_VetoP1ClientsPerLink();
    }
    return status;
}
...
Source handoverProfile_MarshalP1Clients(const tp_bdaddr *bd_addr)
{
    ...
    FOR_EACH_HANDOVER_CLIENT(ho_client)
    {
        /* Marshal interface function of client is called only if registered for the transport of device */
        if (ho_client->pFnMarshal && handoverProfile_IsClientSupportTransport(ho_client, bd_addr->transport))
        {
            uint8 *client_start;
            uint16 client_len = 0;
            *write_ptr++ = client_id;
...
bool handoverProfile_UnmarshalP1Client(tp_bdaddr *addr, const uint8 *src_addr, uint16 src_len, uint16 *consumed)
{
    ...
    *consumed = header_len;
    ...
    FOR_EACH_HANDOVER_CLIENT(ho_client)
    {
        /* Unmarshal interface function of client is called only if registered for the transport of device and has received marshalled data */
        if (client_counter == client_id && handoverProfile_IsClientSupportTransport(ho_client, addr->transport))
        {
...
void handoverProfile_CommitP1Clients(const tp_bdaddr *addr, bool is_primary)
{
    FOR_EACH_HANDOVER_CLIENT(ho_client)
    {
        /* Commit interface function of client is called only if registered for the transport of device */
        if(ho_client->pFnCommit && handoverProfile_IsClientSupportTransport(ho_client, addr->transport))
        {
            HandoverPioSet();
