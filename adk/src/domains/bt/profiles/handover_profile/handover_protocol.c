...
handover_profile_status_t handoverProtocol_SendBtStackMarshalData(handover_device_t *device)
{
    Sink sink = Handover_GetTaskData()->link_sink;
...
static bool handoverProtocol_HandleMarshalData(Source source, const uint8 *src_addr, uint16 src_size)
{
    ...
    switch (src_addr[1])
    {
        ...
        {
            status = handoverProfile_AcceptorHandleBtStackData(source, src_size);
        }
        break;
        ...
        case HANDOVER_MARSHAL_APPSP1_TAG:
        {
            ...
            status = handoverProfile_AcceptorHandleAppsP1Data(source, src_size - header_size);
            if (handoverProfile_AcceptorIsAppsP1UnmarshalComplete())
            {
...
static void handoverProtocol_HandleVersionInd(const uint8 *data)
{
    ...
    if (ho_inst->state == HANDOVER_PROFILE_STATE_CONNECTED)
    {
            ...
            ind.btss_patch_version == ho_inst->btss_patch_version)
        {
            ...
            ho_inst->peer_firmware = HANDOVER_PROFILE_PEER_FIRMWARE_MATCHED;
        }
        else
        {
            DEBUG_LOG("handoverProtocol_HandleVersionInd firmware is mismatched, handover will not be allowed");
            ho_inst->peer_firmware = HANDOVER_PROFILE_PEER_FIRMWARE_MISMATCHED;
        }
    }
    else
    {
...
static handover_profile_status_t handoverProtocol_HandleStartReq(const uint8 *data)
{
    ...
    if (req)
    {
        ...
        status = handoverProfile_AcceptorStart(req);
...
static void handoverProtocol_HandleCancelInd(void)
{
    ...
    handoverProfile_AcceptorCancel();
}
