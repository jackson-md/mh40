...
/*! UUID for Qualcomm Sensor Profile Service */ 
#define UUID_SENSOR_PROFILE_SERVICE      0x00,0x00,0xeb,0x08,0xd1,0x02,0x11,0xe1,0x9b,0x23,0x00,0x02,0x5b,0x00,0xa5,0xa5
...
/*! UUID for Apple iAP2 ("Accessory") Service */
#define UUID_ACCESSORY_SERVICE 0x00, 0x00, 0x00, 0x00, 0xDE, 0xCA, 0xFA, 0xDE, 0xDE, 0xCA, 0xDE, 0xAF, 0xDE, 0xCA, 0xCA, 0xFF
#endif
...
/*! \brief Get a pointer to the service record of the Sensor Profile

    \returns A pointer to the service record of the Sensor Profile
*/
const uint8 *appSdpGetSensorProfileServiceRecord(void);

/*! \brief Get the size of the Sensor Profile service record

    \returns The size of the Sensor Profile service record
*/
uint16 appSdpGetSensorProfileServiceRecordSize(void);

/*! \brief Get the offset to PSM within the Sensor Profile service record

    \returns The offset to the PSM field within the Sensor Profile service record
*/
uint16 appSdpGetSensorProfileServiceRecordPsmOffset(void);

/*! \brief Get a pointer to an SDP search record that can be used to find Mirror Profile.

    \param[out] size  The size of the Sensor Profile search request record.

    \returns A pointer to an SDP search record that can be used to find a Sensor Profile.
*/
const uint8 *appSdpGetSensorProfileLinkAttributeSearchRequest(uint16 *size);
