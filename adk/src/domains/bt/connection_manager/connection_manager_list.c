...
void ConManagerDebugAddress(const tp_bdaddr *tpaddr)
{
    ...
    if (tpaddr->taddr.type == TYPED_BDADDR_RANDOM)
    {
        ...
        {
            DEBUG_LOG_DEBUG("ConManagerDebugAddress, transport %02x, type %02x, random addr %04x,%02x,%06lx, public addr %04x,%02x,%06lx",
                        tpaddr->transport,
                        tpaddr->taddr.type,
        ...
        {
            DEBUG_LOG_DEBUG("ConManagerDebugAddress, transport %02x, type %02x, random addr %04x,%02x,%06lx, public addr unresolved",
                        tpaddr->transport,
                        tpaddr->taddr.type,
                        tpaddr->taddr.addr.nap,
                        tpaddr->taddr.addr.uap,
    ...
    {
        DEBUG_LOG_DEBUG("ConManagerDebugAddress, transport %02x, type %02x, public addr %04x,%02x,%06lx",
                    tpaddr->transport,
                    tpaddr->taddr.type,
                    tpaddr->taddr.addr.nap,
                    tpaddr->taddr.addr.uap,
...
void ConManagerDebugAddressVerbose(const tp_bdaddr *tpaddr)
{
    ...
    if (tpaddr->taddr.type == TYPED_BDADDR_RANDOM)
    {
        if (VmGetPublicAddress(tpaddr, &resolved_tpaddr))
        {
            DEBUG_LOG_VERBOSE("ConManagerDebugAddress, transport %02x, type %02x, random addr %04x,%02x,%06lx, public addr %04x,%02x,%06lx",
                        tpaddr->transport,
                        tpaddr->taddr.type,
                        tpaddr->taddr.addr.nap,
                        tpaddr->taddr.addr.uap,
        ...
        {
            DEBUG_LOG_VERBOSE("ConManagerDebugAddress, transport %02x, type %02x, random addr %04x,%02x,%06lx, public addr unresolved",
                        tpaddr->transport,
                        tpaddr->taddr.type,
                        tpaddr->taddr.addr.nap,
                        tpaddr->taddr.addr.uap,
    ...
    {
        DEBUG_LOG_VERBOSE("ConManagerDebugAddress, transport %02x, type %02x, public addr %04x,%02x,%06lx",
                    tpaddr->transport,
                    tpaddr->taddr.type,
                    tpaddr->taddr.addr.nap,
                    tpaddr->taddr.addr.uap,
...
{
    DEBUG_LOG_DEBUG("ConManagerDebugConnection, state %u, lock %u, users %u",
...
{
    DEBUG_LOG_VERBOSE("ConManagerDebugConnection, state %u, lock %u, users %u",
                     connection->bitfields.state, connection->lock, connection->bitfields.users);
...
cm_connection_t *ConManagerFindConnectionFromBdAddr(const tp_bdaddr *tpaddr)
{
    ...
    tp_bdaddr resolved_tpaddr;
    (void) ConManagerResolveTpaddr(tpaddr, &resolved_tpaddr);

    for_all_connections(connection)
    {
        tp_bdaddr resolved_conn_tpaddr;

        (void) ConManagerResolveTpaddr(&connection->tpaddr, &resolved_conn_tpaddr);
...
/******************************************************************************/
cm_connection_t*  ConManagerFindConnectionFromBleResolvedBdaddr(const typed_bdaddr *taddr)
{
    cm_connection_t *connection;
    tp_bdaddr resolved_tpaddr;

    resolved_tpaddr.taddr = *taddr;
    resolved_tpaddr.transport = TRANSPORT_BLE_ACL;
...
uint8 ConManagerGetConnectedDeviceCount(TRANSPORT_T transport)
{
    ...
    for_all_connections(connection)
    {
        if (connection->tpaddr.transport == transport)
        {
            count++;
        }
    }
