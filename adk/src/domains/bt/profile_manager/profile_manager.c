...
/* Dummy task address used to register the Profile Manager on Profile implementation connect/disconnect
   request task lists. */
#define DUMMY_PROFILE_MANAGER_TASK_1 ((Task) 0xD001)
#define DUMMY_PROFILE_MANAGER_TASK_2 ((Task) 0xD002)
...
static bool profileManager_HandleRequest(Task client, device_t device, profile_manager_request_type_t type)
{
    ...
    if(profileManager_ClientIsValid(client, type))
    {
        ...
        else if (type == profile_manager_stop_connect)
        {
            ...
            if (Device_GetPropertyU8(device, device_property_profile_request_index, &profile_connect_index))
            {
                profile_connect_index |= STOP_CONNECT_REQ_MASK;
                PanicFalse(Device_SetPropertyU8(device, device_property_profile_request_index, profile_connect_index));
...
void ProfileManager_AddToNotifyList(task_list_t *list, device_t device)
{
    if (!ProfileManager_DeviceIsOnList(list, device))
    {
        ...
        task_list_data_t device_used = {0};
        device_used.ptr = device;
        if (!TaskList_AddTaskWithData(list, DUMMY_PROFILE_MANAGER_TASK_1, &device_used))
        {
            PanicFalse(TaskList_AddTaskWithData(list, DUMMY_PROFILE_MANAGER_TASK_2, &device_used));
        }
    }
}
