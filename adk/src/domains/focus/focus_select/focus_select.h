...
/*! \brief Configure the way focus_select prioritises A2DP audio sources for SASS.

    This function configures how focus_select resolves conflicts where there are two A2DP
    sources in the same streaming or playing state. Note that this function should only be
    used for audio source switching due to on going SASS procedure.
...
*/
void FocusSelect_EnableA2dpBargeInForSASS(bool barge_in_enable);

/*! \brief Check the way focus_select prioritises A2DP audio sources

    \return When barge_in_enable is TRUE, the most recently started A2DP stream will take the
            foreground pushing any other streams to the background. When FALSE the last 
            routed A2DP stream will keep the foreground, blocking any newly started streams.
*/
bool FocusSelect_IsA2dpBargeInEnabled(void);
