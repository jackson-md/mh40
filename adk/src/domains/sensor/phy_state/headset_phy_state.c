...
static bool appHeadsetPhyStateProvideContext(unsigned * context_data, uint8 context_data_size)
{
    ...
    switch(HeadsetPhyStateGetTaskData()->state)
    {
        case HEADSET_PHY_STATE_OFF_HEAD:
        case HEADSET_PHY_STATE_OFF_HEAD_AT_REST:
...
bool appHeadsetPhyStateInit(Task init_task)
{
    ...
    ContextFramework_RegisterContextProvider(context_physical_state, appHeadsetPhyStateProvideContext);
    return TRUE;
}
