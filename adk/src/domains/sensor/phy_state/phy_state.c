...
{
    DEBUG_LOG("appPhyStateExitOutOfEarAtRest");
...
static bool appPhyStateProvideContext(unsigned * context_data, uint8 context_data_size)
{
    PanicZero(context_data_size >= sizeof(context_physical_state_t));
    memset(context_data, 0, sizeof(context_physical_state_t));
    context_physical_state_t current_state = in_case;
    switch(PhyStateGetTaskData()->state)
    {
        case PHY_STATE_OUT_OF_EAR:
        case PHY_STATE_OUT_OF_EAR_AT_REST:
            current_state = out_of_case;
            break;
        case PHY_STATE_IN_EAR:
            current_state = on_head;
            break;
        case PHY_STATE_UNKNOWN:
        case PHY_STATE_IN_CASE:
        default:
    ...
    *((context_physical_state_t *)context_data) = current_state;
    return TRUE;
}
...
will be received. This means the in-case state can never be entered */
    ...
    ContextFramework_RegisterContextProvider(context_physical_state, appPhyStateProvideContext);
...
void appPhyStateSetState(phyStateTaskData* phy_state, phyState new_state)
{
    ...
    ContextFramework_NotifyContextUpdate(context_physical_state);
...
phyState appPhyStateGetCurrentState(void)
{
    phyStateTaskData* phy_state = PhyStateGetTaskData();
