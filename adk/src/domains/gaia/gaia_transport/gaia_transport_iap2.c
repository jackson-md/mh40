...
{
    DEBUG_LOG_INFO("gaiaTransport_Iap2HandleSessionStart, link %u, session_id %u, state %u", ind->link, ind->session_id, ti->common.state);

    switch (ti->common.state)
    {
        case GAIA_TRANSPORT_CONNECTED:
        {
            DEBUG_LOG_WARN("gaiaTransport_Iap2HandleSessionStart, new link %u, session_id %u but transport already connected", ind->link, ind->session_id);

            /* Send INTERNAL_SESSION_STOP to stop session when all Rx processing completed */
            MessageSendConditionally(&ti->common.task, INTERNAL_SESSION_STOP, NULL, &ti->rx_data_pending);

            /* Re-send SESSION_START_IND to retry when old session has been closed */
            MESSAGE_MAKE(msg_start, IAP2_EA_SESSION_START_IND_T);
            *msg_start = *ind;
            MessageSendConditionally(&ti->common.task, IAP2_EA_SESSION_START_IND, msg_start, &ti->start_lock);
        }
        break;
        ...
        case GAIA_TRANSPORT_STARTED:
        {
            ...
            /* Increment lock to block session start indications */
            ti->start_lock += 1;
        ...
        default:
        {
            DEBUG_LOG_ERROR("gaiaTransport_Iap2HandleSessionStart, rejecting link %u, session_id %u as transport in incorrect state", ind->link, ind->session_id);

            /* Close session as we can't handle it */
            if (!Accessory_SessionClose(ind->link, ind->session_id))
                DEBUG_LOG_ERROR("gaiaTransport_Iap2HandleSessionStart, failed to close session");
        }
        break;
    }
}

static void gaiaTransport_Iap2HandlerInternalSessionStop(gaia_transport_iap2_t *ti)
{
    DEBUG_LOG_INFO("gaiaTransport_Iap2HandlerInternalSessionStop, link %u, session_id %u", ti->link, ti->session_id);

    if (ti->rx_data_pending)
    {
        DEBUG_LOG_INFO("gaiaTransport_Iap2HandlerInternalSessionStop, data pending, wait");
        MessageSendConditionally(&ti->common.task, INTERNAL_SESSION_STOP, NULL, &ti->rx_data_pending);
    }
    else
    {
        /* Close current session */
        DEBUG_LOG_INFO("gaiaTransport_Iap2HandlerInternalSessionStop, closing session");
        if (!Accessory_SessionClose(ti->link, ti->session_id))
            DEBUG_LOG_ERROR("gaiaTransport_Iap2HandlerInternalSessionStop, failed to close session");
    }
}

static void gaiaTransport_Iap2HandleSessionStop(gaia_transport_iap2_t *ti, const IAP2_EA_SESSION_STOP_IND_T *ind)
{
    DEBUG_LOG_INFO("gaiaTransport_Iap2HandleSessionStop, link %u, session_id %u, state %u", ind->link, ind->session_id, ti->common.state);

    /* Check link and session ID match */
    if ((ti->common.state == GAIA_TRANSPORT_CONNECTED) &&
        (ind->link == ti->link) && (ind->session_id == ti->session_id))
    {
        ...
        if (ti->rx_data_pending)
        {
            ...
            *msg = *ind;
            MessageSendConditionally(&ti->common.task, IAP2_EA_SESSION_STOP_IND, msg, &ti->rx_data_pending);
        }
        else
        {
            /* Decrement session start lock */
            PanicFalse(ti->start_lock > 0);
            ti->start_lock -= 1;

            /* Inform framework transport has disconnected */
...
static void gaiaTransport_Iap2HandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case INTERNAL_SESSION_STOP:
            gaiaTransport_Iap2HandlerInternalSessionStop(ti);
            break;
