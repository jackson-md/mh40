...
static void gaiaCorePlugin_SendUserFeatureResponse(GAIA_TRANSPORT *t, bool next_cmd, uint8 type, uint16 offset)
{
    ...
    if (NULL == gaia_user_feature_linked_list)
    {
        uint8 response[4] = { 0x00, 0x00, 0x00, 0x00 };

        DEBUG_LOG_INFO("gaiaCorePlugin_SendUserFeatureResponse, No User Feature Lists data are set!");
        GaiaFramework_SendResponse(t, GAIA_CORE_FEATURE_ID, pdu_id, sizeof(response), &response[0]);
        DEBUG_LOG_INFO("gaiaCorePlugin_SendUserFeatureResponse, response[0-3] %02X %02X %02X %02X", response[0], response[1], response[2], response[3]);
    }
    else
    {
        ...
        payload_size = MIN(USER_FEATURE_RESPONSE_MAX_TX_PKT_SIZE, ((uint16)value));
        response = (uint8*) PanicNull(malloc(payload_size));
        PanicNull(response);
        ...
        GaiaFramework_SendResponse(t, GAIA_CORE_FEATURE_ID, pdu_id, (4 + reading_status.buf_used), response);
        DEBUG_LOG_INFO("gaiaCorePlugin_SendUserFeatureResponse, response[0-3] %02X %02X %02X %02X", response[0], response[1], response[2], response[3]);
        free(response);
    }
}
