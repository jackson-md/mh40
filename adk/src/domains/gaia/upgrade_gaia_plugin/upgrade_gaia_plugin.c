...
typedef enum
{
    ...
    UPGRADE_STATE_TRANSPORT_DISCONNECTING,
...
static void upgradeGaiaPlugin_SetState(upgrade_gaia_plugin_state_t state)
{
    ...
    switch (state)
    {
        ...
        case UPGRADE_STATE_TRANSPORT_DISCONNECTING:
...
{
    DEBUG_LOG_INFO("upgradeGaiaPlugin_TransportDisconnect, transport %p state %u", t, upgradeGaiaPlugin_task.state);
    if (upgradeGaiaPlugin_task.transport == t)
    {
        switch (upgradeGaiaPlugin_task.state)
        {
            ...
            case UPGRADE_STATE_CONNECTING:
            {
                ...
                upgradeGaiaPlugin_SetState(UPGRADE_STATE_TRANSPORT_DISCONNECTING);
            ...
            {
                /* We can arrive here if the upgrade_gaia_plugin was disconnected due to Throttling 
                 * or a handover during the Throttling. In these scenarios, the transport will have a
                 * valid value till it gets explicitly disconnected due to linkloss, handover completion etc.
                 */
            ...
            default:
                DEBUG_LOG_ERROR("upgradeGaiaPlugin_TransportDisconnect, in wrong state %u", upgradeGaiaPlugin_task.state);
                break;
        }
    }
...
static gaia_framework_command_status_t upgradeGaiaPlugin_UpgradeControl(GAIA_TRANSPORT *transport, uint16 payload_length, const uint8 *payload)
{
    ...
    if (upgradeGaiaPlugin_task.transport == transport)
    {
        ...
        /* If upgrade lib returns false then it won't be processing that command so assume it handled. */
        return UpgradeFlowControlProcessDataRequest((uint8 *)payload, payload_length)? command_pending: command_handled;
    }
    else
    {
