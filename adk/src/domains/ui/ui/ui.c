...
/*! The default UI Inpu redirector function */
redirect_ui_input ui_input_redirector_funcptr = NULL;
...
static void ui_HandleLogicalInput(unsigned logical_input, bool is_right)
{
    ...
    {
        /* Look-up to see if a Gesture ID is configured for this Logical Input in the
           end user's RAM configuration table, if it exists. */
        ui_input_t ui_input = ui_input_invalid;
#ifdef INCLUDE_UI_USER_CONFIG
        bool gesture_matched = UiUserConfig_GetUiInput(logical_input, is_right, &ui_input);

        if (!gesture_matched)
        {
            /* No RAM table, or no matching configuration, so look-up the highest priority UI Input
               for this Logical Input from the Application's const configuration table. */
            ui_input = ui_GetUiInput(logical_input);
        }
#else
        UNUSED(is_right);
        ui_input = ui_GetUiInput(logical_input);
...
static void ui_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    bool is_right_device = FALSE;
    if (Multidevice_IsPair())
    {
        is_right_device = !Multidevice_IsLeft();
    }

    ui_HandleLogicalInput(id, is_right_device);
}

static void ui_InjectRedirectableUiInputWithDelay(ui_input_t ui_input, bool is_redirectable, uint32 delay)
{
    if (is_redirectable && ui_input_redirector_funcptr != NULL)
    {
        ui_input_redirector_funcptr(ui_input);
    }
    else
    {
        PanicNull((void*)inject_ui_input_funcptr);
...
/******************************************************************************
 * UI domain scope, protected API functions
 ******************************************************************************/
const ui_config_table_content_t * Ui_GetConstUiConfigTable(unsigned * number_of_elements)
{
    *number_of_elements = ui_config_size;
    return ui_config_table;
}
...
{
    ui_HandleLogicalInput(logical_input, is_right_device);
}

void Ui_InjectUiInput(ui_input_t ui_input)
{
    Ui_InjectRedirectableUiInput(ui_input, TRUE);
}

void Ui_InjectRedirectableUiInput(ui_input_t ui_input, bool is_redirectable)
{
    ui_InjectRedirectableUiInputWithDelay(ui_input, is_redirectable, D_IMMEDIATE);
}

void Ui_InjectBackEndUiInput(ui_input_t ui_input, uint32 delay)
{
...
void Ui_RegisterUiInputsRedirector(redirect_ui_input ui_redirector_func)
{
    PanicNull((void *)ui_redirector_func);

    /* Update the active UI redirector function pointer with the supplied function */
    ui_input_redirector_funcptr = ui_redirector_func;
}
...
#ifdef INCLUDE_UI_USER_CONFIG
    case ui_indication_type_reset_user_config:
        UiUserConfig_DeleteUserConfiguration(TRUE);
        break;
#endif
...
bool Ui_IsPeerEnabled(void)
{
    return sniff_ui_event_funcptr != NULL;
}
