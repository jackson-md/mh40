...
typedef enum
{
    ...
    ui_indication_type_reset_user_config,
...
typedef void (*redirect_ui_input)(ui_input_t ui_input);
...
/*! \brief Inject a UI Input into the CAA framework, but optionally supporting
           redirection in peer-enabled Application builds.
    ...
    \param ui_input - The UI Input to be injected
    \param is_redirectable - if TRUE, the UI Input MAY be redirected to a Peer device,
                             if FALSE, the UI Input shall be injected at the local UI.
                             Note this parameter is only applicable for peer-enabled
                             Application builds.
*/
...
/*! \brief Inject a UI Input to the system, after a specified delay period.
    ...
    matching UI Provider context had been found. The UI Input will be sent to
    the registered UI Input Consumers task list directly, it shall not be
    redirected further, nor exposed to any registered function pointers.
...
/*! \brief Register a function that shall redirect injected UI Inputs to a Peer
           device, if necessary

    This mechanism allows the Peer Service to register a callback function which
    can redirect UI Inputs when they are injected on the Secondary in a Peer-enabled
    use case.

    If no function is registered, injected UI Inputs will always be handled at the
    local UI domain.
...
/*! \brief Register a function which shall indicate when logical inputs should be
    ...
    This mechanism allows the application to register a callback function which
...
/*! \brief Gets whether the UI domain is peer-enabled or not.

    \return TRUE if the UI domain is operating in a peer-enabled capacity.
*/
