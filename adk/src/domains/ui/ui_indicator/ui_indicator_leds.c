...
static void uiLeds_DoSetPattern(const ui_led_data_t *config, uint16 led_index)
{
    ...
    {
        if (!MessagePendingFirst(&the_leds.task, UI_INTERNAL_LED_FLASH_COMPLETED, &delivery_time))
        {
            indicate = TRUE;
    ...
    DEBUG_LOG_V_VERBOSE("uiLeds_DoSetPattern enum:led_priority_t:%d indicate=%d", config->priority, indicate);
