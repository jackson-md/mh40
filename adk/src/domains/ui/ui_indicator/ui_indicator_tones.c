...
#define NOT_REPEATABLE_TONE_TIMEOUT_MS D_SEC(5)
...
static void uiTones_PlayTone(uint16 tone_index, rtime_t time_to_play, const ui_tone_data_t *config)
{
    ...
    if (the_tones.tone_playback_enabled)
    {
        ...
        if(NOT_REPEATABLE_TONE_TIMEOUT_MS != 0 && config->not_repeatable_within_timeout
           /* Shouldn't ignore repeated tone if its a repeating indication */
           && !uiTones_IsRepeatingIndication(tone_index))
        {
            MessageCancelFirst(&the_tones.task, UI_INTERNAL_CLEAR_LAST_TONE);
            MessageSendLater(&the_tones.task, UI_INTERNAL_CLEAR_LAST_TONE, NULL,
                             NOT_REPEATABLE_TONE_TIMEOUT_MS);
            the_tones.last_tone_played = tone_index;
        }
    }
}
...
static inline bool uiTones_IsNotARepeatPlay(uint16 tone_index)
{
    return tone_index != the_tones.last_tone_played;
}
...
static void uiTones_SchedulePlay(uint16 tone_index)
{
    ...
    if (uiTones_IsNotARepeatPlay(tone_index) &&
        (config->queueable || (!appKymeraIsTonePlaying() && !ui_IsKymeraResourceLocked())))
    {
        ...
        {
            /* Factor in the audio latency (chain creation, configuration and data propagation through it),
               peer relay delay where it applies (may need to relay event to peer) */
            time_to_play = rtime_add(time_to_play, UI_INDICATOR_DELAY_FOR_SYNCHRONISED_TTP_IN_MICROSECONDS);
...
static void uiTones_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    // Catch any "special" message IDs first
    ...
    else if (id == UI_INTERNAL_CLEAR_LAST_TONE)
    {
        DEBUG_LOG("UI_INTERNAL_CLEAR_LAST_TONE");
        the_tones.last_tone_played = TONE_NONE;
    }
    ...
    else
    {
        ...
        // We can have the same event associated with a reminder tone as well as a "standard" tone
        ...
        else if (uiTones_GetCancelToneIndexFromReminderMappingTable(id, &tone_index))
        {
