...
\file       domain_message.h
\defgroup   domain_message Domain Message
...
/*! \brief A table of domain component names.
...
#define FOREACH_DOMAINS_MESSAGE_GROUP(X) \
    ...
    X(HIDD_PROFILE)         /* See #hidd_profile_messages */ \
    X(SENSOR_PROFILE)       /* See #sensor_profile_msg_t */ \
    X(SPATIAL_DATA)         /* See #spatial_data_messages */ \
    \
    ...
    X(DFU_CASE)             /* See #dfu_case_messages_t */\
    X(INQUIRY_MANAGER)      /* GATT LEA Discovery messages */\
    X(RSSI_PAIRING)         /* GATT LEA Discovery messages */\
    X(BTDBG_PROFILE)        /* BTDBG messages */\
    X(LE_AUDIO_CLIENT)      /* LE Audio Client messages */\
    
    

/*! \brief A table of service component names */
#define FOREACH_SERVICES_MESSAGE_GROUP(X) \
    ...
    X(SINK_SERVICE)             /* See #sink_service_msg_t */

/*! \brief A table of topology component names */
#define FOREACH_TOPOLOGY_MESSAGE_GROUP(X) \
    ...
    X(STEREO_TOPOLOGY)             /* See #stereo_topology_message_t */ \

/*! \brief A table of app/system component names */
...
/*! \brief A table of UI inputs names */
...
/*! \brief This expansion macro concatenates the component name with the text
...
/*! \brief A type to enumerate the message groups available in the system */
enum message_groups
{
...
/*! \brief The first UI inputs message group ID */
...
/*!     \name Message ID allocations for each application component
        \brief Each component in the application that sends messages is assigned
               ...
               from that base ID.
        \note There is no checking that the messages assigned by one component do
...
#ifndef HOSTED_TEST_ENVIRONMENT
/*! \brief Adding compile time assert to protect against message overflow */
...
/*! \brief Helper macro to create a message broker group regisration.
