...
void UsbSource_Play(audio_source_t source)
{
    ...
    {
        if (UsbSource_GetAudioContext(source) == context_audio_is_paused)
        {
            UsbSource_SendEvent(USB_SOURCE_PLAY);
            /* Setting the context to USB PLAY !
             * As we do not get any Acknowledgement from the Host for the send event */
            UsbSource_SetAudioContext(context_audio_is_playing);
            if (TRUE == UsbAudio_SendPlayStatusToMediaService(source))
            {
                DEBUG_LOG_VERBOSE("Usb Play Status Send Successfully");
            }
        }
    }
}
...
void UsbSource_Pause(audio_source_t source)
{
    if (source == audio_source_usb)
    {
        if (UsbSource_GetAudioContext(source) == context_audio_is_playing)
        {
            UsbSource_SendEvent(USB_SOURCE_PAUSE);
            /* Setting the context to USB PAUSE !
             * As we do not get any Acknowledgement from the Host for the send event */
            UsbSource_SetAudioContext(context_audio_is_paused);
            if (TRUE == UsbAudio_SendPauseStatusToMediaService(source))
            {
                DEBUG_LOG_VERBOSE("Usb Pause Status Send Successfully");
            }
        }
    }
}
...
device_t UsbSource_GetSelfDevice(audio_source_t source)
{
    UNUSED(source);
