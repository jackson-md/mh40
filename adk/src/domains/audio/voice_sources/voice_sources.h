 ...
 */
void VoiceSources_RegisterTelephonyControlInterface(voice_source_t source,
                                                    const voice_source_telephony_control_interface_t * interface);

/*! \brief Deregisters the provided telephony control interface for a voice source, if it was registered.

    \param source The voice source
    \param interface The voice source telephony control interface to deregister
 */
...
/*! \brief Enable or disable Caller Id notifications, if supported by the Voice Source

    \param source The voice source for which to control the Caller Id function.

    \param enable TRUE if Caller Id should be enabled, FALSE if it should be disabled,
                  for the specified voice source.
*/
void VoiceSources_EnableCallerId(voice_source_t source, bool enable);

/*! \brief Get the device for voice source

    \param source The voice source for which the device need to be extracted

    \return device_t The device associated with the source.
*/
device_t VoiceSources_GetDeviceForSource(voice_source_t source);

/*! \brief Refresh the call state for voice source

    \param source The voice source for which the call state needs to be refreshed
*/
