...
static void kymera_A2dpCreateChain(const a2dp_codec_settings *codec_settings, uint32 max_bitrate, aptx_adaptive_ttp_latencies_t nq2q_ttp)
{
    ...
    if (seid == AV_SEID_APTX_ADAPTIVE_SNK)
    {
         ...
         *    R2.1 44.1
         */
        if (split_mode_enabled == TRUE)
        {
            if ((theKymera->aptx_adaptive_r22_dec == TRUE && (rate==SAMPLE_RATE_96000 || rate==SAMPLE_RATE_44100)) ||
...
static void kymera_A2dpStartChain(int16 volume_in_db)
{
    ...
    KymeraPioClr(); /* Set before the chain is created */
}
...
void appKymeraA2dpSetSyncStartTime(uint32 clock)
{
    ...
    DEBUG_LOG("appKymeraA2dpSetSyncStartTime: enum:kymera_a2dp_state_t:%d, enum:appKymeraState:%d", appKymeraA2dpGetState(), appKymeraGetState());

    /* Cancel any pending timeout messages */
    MessageCancelAll(&theKymera->task, KYMERA_INTERNAL_A2DP_DATA_SYNC_IND_TIMEOUT);

    if (Kymera_A2dpIsStreaming() && (theKymera->sync_info.state == KYMERA_AUDIO_SYNC_STATE_INIT))
    {
        Operator op_rtp;
        rtime_t ttp_us;
        uint32 latency;
...
void appKymeraA2dpHandleAudioSyncStreamInd(MessageId id, Message msg)
{
    ...
    if (!Kymera_A2dpIsForwarding() || (theKymera->sync_info.state == KYMERA_AUDIO_SYNC_STATE_COMPLETE))
    {
        return;
    }
