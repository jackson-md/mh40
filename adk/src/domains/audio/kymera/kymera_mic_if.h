...
typedef struct
{
    ...
    *   use case can be achieved without discontinuities. */
    ...
    /*! activate_timestamps defines if a user needs timestamp information propagated through the chain. */
    const unsigned activate_timestamps:1;
    /*! mandatory_ttp_delay_us defines the required TTP latency for a user in microseconds.
     *  Each time a new user connects the required value of the new user is checked against
     *  the already programmed ttp delay. The user shouldn't change the ttp delay while being connected.
     *  There is only one user allowed to request TTP delay */
    const uint32 *mandatory_ttp_delay_us;
    /*! use_aec_ref defines if the client requires aec_ref to be connected or not.
     * This information can be changed by the client at runtime.
     */
    const bool *use_aec_ref;
