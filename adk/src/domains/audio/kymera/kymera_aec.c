...
static uint32 kymera_AecGetTtpDelay(void)
{
    uint32 ttp_delay = 0;
    if(aec_config.ttp_delay > 0)
    {
        ttp_delay = aec_config.ttp_delay;
    }
    else if(aec_config.activate_timestamps)
    {
        ttp_delay = AEC_REF_DEFAULT_MIC_TTP_LATENCY;
    }
    DEBUG_LOG("kymera_AecGetTtpDelay: %u us", ttp_delay);
    return ttp_delay;
}

void Kymera_AecSetTtpDelay(uint32 ttp_delay)
{
    DEBUG_LOG("Kymera_AecSetTtpDelay: %u", ttp_delay);
...
static void kymera_ConfigureTtpDelay(void)
{
    ...
    uint32 ttp_delay = kymera_AecGetTtpDelay();
    if (ttp_delay != 0)
    {
