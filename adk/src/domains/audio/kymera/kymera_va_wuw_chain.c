...
static va_wuw_engine_t largest_wuw_engine = va_wuw_engine_none;
...
static void kymera_CreateChains(const kymera_va_wuw_chain_params_t *params)
{
    ...
    if(creating_largest_wuw_engine || (largest_wuw_engine == va_wuw_engine_none))
    {
...
{
    return OperatorsGetApvaMetadata(kymera_GetOperatorFromChain(OPR_WUW, va_wuw_chain), size);
}
...
void Kymera_StoreLargestWuwEngine(void)
{
    ...
    DEBUG_LOG_ALWAYS("Kymera_StoreLargestWuwEngine: Largest engine is enum:va_wuw_engine_t:%d with size %u", largest_wuw_engine, largest_size);
}
