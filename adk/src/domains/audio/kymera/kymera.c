    ...
    case KYMERA_OP_MSG_ID_AHM_EVENT:
        {            
            ...
            {            
                if(op_msg->message[KYMERA_AHM_EVENT_TYPE_INDEX] == KYMERA_AHM_EVENT_TYPE_TRIGGER)
                {
                    DEBUG_LOG_ALWAYS("1MIC WIND ATTACK!");
                    appKymera_MsgRegisteredClients(KYMERA_WIND_STAGE1_DETECTED,KYMERA_OP_MSG_WORD_PAYLOAD_NA);
                }
                else
                {
            ...
            {            
                if(op_msg->message[KYMERA_AHM_EVENT_TYPE_INDEX] == KYMERA_AHM_EVENT_TYPE_TRIGGER)
                {
                    DEBUG_LOG_ALWAYS("2MIC WIND ATTACK!");
                    appKymera_MsgRegisteredClients(KYMERA_WIND_STAGE2_DETECTED,KYMERA_OP_MSG_WORD_PAYLOAD_NA);
                }
                else
                {
...
// Following APIs should not coexist with AUDIO DRIVER
#ifndef INCLUDE_AUDIO_DRIVER
...
void appKymeraProspectiveDspPowerOn(void)
{
    ...
    else
    {
        ...
        OperatorFrameworkEnableAsync(MAIN_PROCESSOR_ON, KymeraGetTask());
    }
...
static void kymera_msg_handler(Task task, MessageId id, Message msg)
{
    ...
    switch (id)
    {
        case MESSAGE_OPERATOR_FRAMEWORK_ENABLE_RESULT:
             DEBUG_LOG("appKymera MESSAGE_OPERATOR_FRAMEWORK_ENABLE_RESULT %d", ((MessageOperatorFrameworkEnableResult *)msg)->success);
