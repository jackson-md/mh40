...
static kymera_a2dp_state_t kymera_a2dp_state = kymera_a2dp_idle;
...
void appKymeraGetA2dpCodecSettingsAptxAdaptive(const a2dp_codec_settings *codec_settings, bool *split_tx, bool *r22_dec, bool *r22_enc)
{
    ...
    if (r22_dec)
    {
        *r22_dec = ((codec_settings->codecData.aptx_ad_params.features & aptx_ad_2_2_kern_enc_src) &&
                    (codec_settings->codecData.aptx_ad_params.features & aptx_ad_2_2_kern_dec_snk) &&
                    (codec_settings->rate == SAMPLE_RATE_44100));
    }
    if (r22_enc)
    {
        *r22_enc = ((codec_settings->codecData.aptx_ad_params.features & aptx_ad_2_2_kern_dec_src) &&
                  (codec_settings->codecData.aptx_ad_params.features & aptx_ad_2_2_kern_enc_snk));
    }
}
...
void Kymera_A2dpConfigureRtpDecoderStartupPeriod(Operator op, uint16 startup_period)
{
    ...
    const uint32 filter_gain = CONVERT_TO_Q1_31(0.997);
    const uint32 err_scale = CONVERT_TO_Q1_31(-0.00000001);
...
bool appKymeraSetAptxADMixerModes(Operator decoder, bool is_left, bool stereo_lr_mix)
{
    ...
    /* Force disable stereo mix for 96K if necessary, and for R2.2 as well */
    if (((appConfigEnableAptxAdaptiveStereoMix96K() == FALSE) && is_96K)
       ||(theKymera->aptx_adaptive_r22_dec == TRUE))
    {
        stereo_lr_mix = FALSE;
    }
    ...
    /* Configure the channel extract for R2.1 */
    if ((decoder != INVALID_OPERATOR) && (theKymera->aptx_adaptive_r22_dec == FALSE))
    {
...
{
    kymera_a2dp_state = state;
    // Used to trigger any functions executed upon setting/changing the kymera state
    appKymeraSetState(appKymeraGetState());
}

kymera_a2dp_state_t appKymeraA2dpGetState(void)
{
    return kymera_a2dp_state;
}
