...
static const standard_param_value_t sosy_min_period_kp_7_5_value = CONVERT_TO_Q1_31(1000.0/KICK_PERIOD_SLOW);
...
static const standard_param_value_t sosy_min_period_kp_2_0_value = CONVERT_TO_Q1_31(500.0/KICK_PERIOD_FAST);
...
static const standard_param_id_t sosy_max_latency_id = 2;
                                  ...
                                  bool is_stereo)
{
    ...
    if (GET_OP_FROM_CHAIN(op, chain, OPR_SOURCE_SYNC))
    {
        ...
        if (config->source_sync_max_latency)
        {
            appKymeraSetSourceSyncParameter(op, sosy_max_latency_id, config->source_sync_max_latency);
        }
