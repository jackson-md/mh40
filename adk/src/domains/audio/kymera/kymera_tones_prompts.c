...
static void kymera_CreatePromptChain(promptFormat prompt_format)
{
    ...
    if (config)
    {
        ...
        if (prompt_format == PROMPT_FORMAT_AAC)
        {
            Operator op;
            if (GET_OP_FROM_CHAIN(op, kymera_GetTonePromptChain(), OPR_TONE_PROMPT_DECODED_BUFFER))
            {
                OperatorsStandardSetBufferSize(op, AAC_PROMPT_DECODED_BUFFER_SIZE);
            }
        }
