...
#define USB_VOICE_MAX_NUM_OF_MICS       (2)
...
static bool kymeraUsbVoice_MicGetConnectionParameters(uint16 *mic_ids, Sink *mic_sinks, uint8 *num_of_mics, uint32 *sample_rate, Sink *aec_ref_sink);
...
static const mic_user_state_t usb_voice_mic_state =
{
    mic_user_state_non_interruptible,
};
static const bool usb_voice_use_aec_ref = TRUE;
...
static const mic_registry_per_user_t usb_voice_mic_registry =
{
    ...
    .use_aec_ref = &usb_voice_use_aec_ref,
};
...
static kymera_chain_handle_t kymeraUsbVoice_CreateChain(usb_voice_mode_t mode)
{
     ...
     * option.
     */
    if (appConfigVoiceGetNumberOfMics() > USB_VOICE_MAX_NUM_OF_MICS)
    {
        DEBUG_LOG_WARN("kymeraUsbVoice_CreateChain invalid no of mics %x", appConfigVoiceGetNumberOfMics());
        return NULL;
    }
    ...
    switch(mode)
    {
        ...
        {
            if(appConfigVoiceGetNumberOfMics() == 2)
            {
...
static void kymeraUsbVoice_ConfigureChain(KYMERA_INTERNAL_USB_VOICE_START_T *usb_voice)
{
    ...
    if(KymeraGetTaskData()->chain_config_callbacks && KymeraGetTaskData()->chain_config_callbacks->ConfigureUsbVoiceRxChain)
    {
        ...
        params.sample_size = usb_voice->spkr_frame_size;
...
static void kymeraUsbVoice_PopulateConnectParams(uint16 *mic_ids, Sink *mic_sinks, uint8 num_mics, Sink *aec_ref_sink)
{
...
void KymeraUsbVoice_Stop(KYMERA_INTERNAL_USB_VOICE_STOP_T *usb_voice)
{
    ...
    if (appKymeraGetState() != KYMERA_STATE_USB_VOICE_ACTIVE)
    {
        if (usb_voice_chain == NULL)
        {
             ...
             * This happens when the chain is destroyed, after a failed attempt to
             * start the chain in KymeraUsbVoice_Start(), and then the user calls
             * KymeraUsbVoice_Stop(). Cleaning up is already completed*/
...
void KymeraUsbVoice_MicMute(bool mute)
{
    ...
    switch (KymeraGetTaskData()->state)
    {
        ...
        /* PC mute control of the USB Microphone when HFP SCO is active */
        case KYMERA_STATE_USB_SCO_VOICE_ACTIVE:
        {
            KymeraUsbScoVoice_MicMute(mute);
        }
        break;
        default:
            break;
