...
static bool kymera_LeakthroughMicGetConnectionParameters(uint16 *mic_ids, Sink *mic_sinks, uint8 *num_of_mics, uint32 *sample_rate, Sink *aec_ref_sink);
...
static const uint16 kymera_MandatoryLeakthroughMicIds[MAX_NUM_OF_MICS_SUPPORTED] =
{
...
static const bool kymera_LeakthroughUseAecRef = FALSE;
...
static const mic_registry_per_user_t kymera_MicLeakthroughRegistry =
{
    ...
    .use_aec_ref = &kymera_LeakthroughUseAecRef,
};
...
static void kymera_PopulateLeakthroughConnectParams(uint16 *mic_ids, Sink *mic_sinks, uint8 num_mics)
{
    PanicZero(mic_ids);
    PanicZero(mic_sinks);
...
void Kymera_LeakthroughResumeChainIfSuspended(void)
{
    ...
    if(AecLeakthrough_IsLeakthroughEnabled() && !appKymeraIsBusy())
    {
...
void Kymera_EnableLeakthrough(void)
{
    ...
    if(Kymera_IsVaActive() || Kymera_A2dpIsRouted())
    {
    ...
    else
    {
        switch(appKymeraGetState())
        {
            ...
            case KYMERA_STATE_LE_AUDIO_ACTIVE:
                /* In stereo recording and gaming mode with VBC scenarios, mic is already in use. Hence, follow the same procedure as in voice calls.
                   For gaming mode without VBC, unicast music and broadcast usecases, mic is not in use. These scenarios are handled similar to A2DP streaming.
                */
                if(Kymera_MicGetActiveUsers() == mic_user_le_mic)
                {
                    Kymera_LeakthroughUpdateAecOperatorAndSidetone();
                }
                else
