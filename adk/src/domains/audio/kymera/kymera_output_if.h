...
typedef enum
{
    ...
    output_user_le_voice_mono = (1 << 9),
    output_user_le_voice_stereo = (1 << 10),
    output_user_le_srec = (1 << 11),
    output_user_loopback = (1 << 12),
    output_user_fit_test = (1 << 13),
    output_user_common_chain = (1 << 14),
} output_users_t;
...
/*! \brief Returns active/connected output users.
 */
output_users_t Kymera_OutputGetConnectedUsers(void);

/*! \brief Get the kick period used to configure the output chain with.
    \return The kick period which if zero indicates a failure (chain not prepared yet).
 */
unsigned Kymera_OutputGetKickPeriod(void);
