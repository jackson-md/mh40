...
#ifdef INCLUDE_TWS
#define FBC_FEATURE_ID TDFBC_MONO
#define VAD_FEATURE_ID VAD_MONO
#else
#define FBC_FEATURE_ID TDFBC
#define VAD_FEATURE_ID VAD
#endif

typedef struct
{
    /*! Assert on a license check failure */
    unsigned assert:1;
} license_check_params_t;
...
static void kymera_CheckVadLicense(capability_id_t vad_id, const void *params);
static void kymera_CheckCvcLicense(capability_id_t cvc_id, const void *params);

static const capability_check_map_t license_check_map[] =
{
    {OPR_VAD, kymera_CheckVadLicense},
    {OPR_CVC_SEND, kymera_CheckCvcLicense},
};

static const uint16 mic_map[MAX_NUM_OF_MICS_SUPPORTED] =
{
    appConfigVaMic1(),
    appConfigVaMic2(),
};
...
static void kymera_CheckLicense(feature_id feature, const license_check_params_t *params)
{
    if (!FeatureVerifyLicense(feature))
    {
        DEBUG_LOG_WARN("kymera_CheckLicense: WARNING enum:feature_id:%d is not licensed", feature);
...
static void kymera_CheckVadLicense(capability_id_t vad_id, const void *params)
{
    ...
    {
        case CAP_ID_VAD:
        case CAP_ID_DOWNLOAD_VAD:
            kymera_CheckLicense(VAD_FEATURE_ID, params);
        break;
        default:
        break;
    }
...
static void kymera_CheckCvcLicense(capability_id_t cvc_id, const void *params)
{
    ...
    {
        case CAP_ID_CVC_FBC:
        case CAP_ID_DOWNLOAD_CVC_FBC:
            kymera_CheckLicense(FBC_FEATURE_ID, params);
        break;
        default:
        break;
    }
...
static void kymera_LicenseCheck(const chain_config_t *chain_config, bool assert)
{
    license_check_params_t params = {0};
    params.assert = assert;
    Kymera_CheckChainCapabilities(chain_config, license_check_map, ARRAY_DIM(license_check_map), &params);
}

static void kymera_CreateChain(const chain_config_t *chain_config)
{
    ...
    {
        DEBUG_LOG_PANIC("kymera_CreateChain: No compatible chain configuration found!");
    }
}
...
void Kymera_CreateVaMicChain(const va_mic_chain_create_params_t *params)
{
    ...
    const chain_config_t *chain_config = kymera_GetChainConfig(&params->chain_params);
    kymera_LicenseCheck(chain_config, TRUE);
    kymera_CreateChain(chain_config);
...
bool Kymera_GetVaMicChainMicConnectionParams(uint16 *mic_ids, Sink *mic_sinks, uint8 *num_of_mics, uint32 *sample_rate, Sink *aec_ref_sink)
{
    Sink sink;
