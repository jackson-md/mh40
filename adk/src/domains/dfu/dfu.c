...
static void dfu_HandleUpgradeOperationInd(const UPGRADE_OPERATION_IND_T *operation)
{
    ...
    switch (operation->ops)
    {
        ...
        case upgrade_ops_notify_early_erase:
        {
             ...
             * can be cancelled to avoid false DFU timeouts and connection 
             * retentions can be enabled owing to actual DFU start indication 
             * being deferred as DFU erase was ongoing.
             */
            dfu_NotifyPreStart();
        ...
        case upgrade_ops_save_peer_pskeys:
        {
            ...
            case upgrade_ops_check_peer_connection_pre_reboot:
            {
#ifdef INCLUDE_DFU_PEER
                if(!UPGRADE_PEER_IS_CONNECTED && UPGRADE_PEER_IS_PRIMARY)
                {
                   DEBUG_LOG_INFO("dfu_HandleUpgradeOperationInd connect peer before send COMPLETE_RES");
                   UpgradePeerReconnectForReboot();
                }
                else
#endif
                {
                    DEBUG_LOG_INFO("dfu_HandleUpgradeOperationInd complete reboot process");
