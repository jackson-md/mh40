...
typedef struct 
{
    ...
    /*! List of tasks registered for peer find role start */
    task_list_t                 start_tasks;
...
/*! Check if any client has registered for PEER_FIND_ROLE_STARTED message.

    \return TRUE if any client is registered, FALSE otherwise.
*/
#define peer_find_role_start_client_registered() (TaskList_Size(&peer_find_role.start_tasks) != 0)
