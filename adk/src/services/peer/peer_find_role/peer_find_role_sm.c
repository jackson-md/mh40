...
{
    DEBUG_LOG("peer_find_role_enter_discover_connectable. AdvertBusy:%d", PeerFindRoleGetAdvertBusy());
    ...
    {
        DEBUG_LOG("peer_find_role_enter_discover_connectable. Queue PEER_FIND_ROLE_INTERNAL_ATTEMPT_ADVERTISING");
    ...
    {
        DEBUG_LOG("peer_find_role_enter_discover_connectable. Enable Advertising");
...
static void peer_find_role_enter_connecting_to_discovered(void)
{
    ...
    ConManagerRequestDeviceQos(&tp_addr, cm_qos_short_while_streaming);
...
static void peer_find_role_exit_advertising_state(void)
{
    ...
    PeerFindRole_RemovePeerFromAcceptorList();
...
#else
    LeAdvertisingManager_UpdateAdvertisingItem(PeerFindRole_GetAdvertisingItemHandle());
#endif
}
...
void peer_find_role_set_state(PEER_FIND_ROLE_STATE new_state)
{
    ...
    DEBUG_LOG_STATE("peer_find_role_set_state. enum:PEER_FIND_ROLE_STATE:%d -> enum:PEER_FIND_ROLE_STATE:%d Busy flags were:0x%x",
                old_state, new_state,
