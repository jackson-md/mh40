...
/*! Marshal type descriptor for #anc_sm_world_volume_gain_balance_info_t. */
const marshal_type_descriptor_t marshal_type_descriptor_anc_sm_world_volume_gain_balance_info_t =
    MAKE_MARSHAL_TYPE_DEFINITION_BASIC(anc_sm_world_volume_gain_balance_info_t);
...
/*! #STATE_PROXY_CIS_RSSI_T message member descriptor. */
const marshal_member_descriptor_t state_proxy_cis_rssi_quality_member_descriptors[] = 
{
    MAKE_MARSHAL_MEMBER(STATE_PROXY_CIS_RSSI_T, int8, cis_rssi),
    MAKE_MARSHAL_MEMBER(STATE_PROXY_CIS_RSSI_T, tp_bdaddr, device),
    MAKE_MARSHAL_MEMBER(STATE_PROXY_CIS_RSSI_T, uint16, cis_handle),
};

/*! #STATE_PROXY_LINK_QUALITY_T marshal type descriptor. */
const marshal_type_descriptor_t marshal_type_descriptor_STATE_PROXY_CIS_RSSI_T =
    MAKE_MARSHAL_TYPE_DEFINITION(STATE_PROXY_CIS_RSSI_T, state_proxy_cis_rssi_quality_member_descriptors);
...
const marshal_member_descriptor_t state_proxy_data_member_descriptors[] = 
{
    ...
    MAKE_MARSHAL_MEMBER(state_proxy_data_t, uint16, previous_config),
    MAKE_MARSHAL_MEMBER(state_proxy_data_t, uint16, previous_mode),
    ...
    MAKE_MARSHAL_MEMBER(state_proxy_data_t, int8, world_volume_gain_dB),
    MAKE_MARSHAL_MEMBER(state_proxy_data_t, anc_sm_world_volume_gain_balance_info_t, balance_info),
};
...
/*----------------------------------------------------------------------------*/
/*! #STATE_PROXY_AANC_FB_GAIN_LOGGING_T marshal type descriptor. */
const marshal_type_descriptor_t marshal_type_descriptor_STATE_PROXY_AANC_FB_GAIN_LOGGING_T =
        MAKE_MARSHAL_TYPE_DEFINITION_BASIC(STATE_PROXY_AANC_FB_GAIN_LOGGING_T);
...
{
    /* Normally, state proxy is only sent in the direction secondary to primary, the exception is for
       physical state transition messages, where it is useful for the secondary to track the primary's
       phy state for application rule execution decision making. */
    bool is_send_allowed_for_topology_role = stateProxy_IsSecondary() || (stateProxy_IsPrimary() && marshal_type == MARSHAL_TYPE(PHY_STATE_CHANGED_IND_T));

    bool send = !stateProxy_Paused() && appPeerSigIsConnected() && is_send_allowed_for_topology_role;
    DEBUG_LOG("stateProxy_MarshalToConnectedPeer stateProxy_Paused=%u, appPeerSigIsConnected=%u, is_allowed_for_topology_role=%u",
            stateProxy_Paused(), appPeerSigIsConnected(), is_send_allowed_for_topology_role);
