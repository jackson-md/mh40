...
typedef struct
{
    ...
    /*! ANC previous config used when concurrency is over*/
    uint16 previous_config;
    
    /*! ANC previous mode when ANC was OFF */
    ...
    /* ANC configuration during standalone, playback, SCO, VA & LE Stereo recording */
    ...
    /* World volume gain in dB */
    int8 world_volume_gain_dB;

    /* World volume balance */
...
typedef struct
{
    ...
    /*! TRUE when CIS quality measurements are enabled. */
    bool measuring_cis_rssi:1;
...
#define stateProxy_IsMeasuringCisRssi()           (stateProxy_GetTaskData()->measuring_cis_rssi)
#define stateProxy_SetMeasuringCisRssi(value)     (stateProxy_GetTaskData()->measuring_cis_rssi = (value))
...
enum state_proxy_internal_messages
{
    ...
    STATE_PROXY_INTERNAL_TIMER_CIS_RSSI_QUALITY,
