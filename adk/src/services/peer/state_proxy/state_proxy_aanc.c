...
/*! \brief Handle local events for AANC FB gain update. */
static void stateProxy_HandleAancFBGainUpdateInd(const STATE_PROXY_AANC_FB_GAIN_LOGGING_T* aanc_data)
{
    state_proxy_task_data_t *proxy = stateProxy_GetTaskData();
    proxy->local_state->aanc_fb_gain = aanc_data->aanc_fb_gain;

    if(!stateProxy_Paused() && appPeerSigIsConnected())
    {
        void* copy;
        marshal_type_t marshal_type = MARSHAL_TYPE(STATE_PROXY_AANC_FB_GAIN_LOGGING_T);
...
void stateProxy_HandleAancLoggingUpdate(MessageId id, Message aanc_data)
{
    switch(id)
    {
       ...
       case AANC_FB_GAIN_UPDATE_IND:
           stateProxy_HandleAancFBGainUpdateInd((const STATE_PROXY_AANC_FB_GAIN_LOGGING_T*) aanc_data);
       break;
...
void stateProxy_HandleRemoteAancFBGainLoggingUpdate(const STATE_PROXY_AANC_FB_GAIN_LOGGING_T* remote_aanc_data)
{
    state_proxy_task_data_t *proxy = stateProxy_GetTaskData();
    proxy->remote_state->aanc_fb_gain = remote_aanc_data->aanc_fb_gain;

    /* Update remote device data if local device is a primary; else ignored */
    if(StateProxy_IsPrimary())
    {
       STATE_PROXY_AANC_FB_GAIN_LOGGING_T aanc_fb_gain_logging;

       aanc_fb_gain_logging.aanc_fb_gain = remote_aanc_data->aanc_fb_gain;

       /* Updating ANC SM with secondary FB gain*/
        stateProxy_MsgStateProxyEventClients(state_proxy_source_remote,
                                             state_proxy_event_type_aanc_fb_gain_logging,
                                             &aanc_fb_gain_logging);
    }
}
