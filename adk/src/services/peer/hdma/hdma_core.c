...
{
    HDMA_DEBUG_LOG_VERBOSE("Local bud state - In Case: %d, In Ear: %d    Remote bud state - In Case: %d, In Ear: %d ",
...
static void hdma_SetHandoverEvent(hdma_core_handover_result_t newResult)
{
    ...
    {
        DEBUG_LOG_DEBUG("hdma_SetHandoverEvent: Handover is false [%d]\n", newResult.handover);

        if(hdma_core_data->hdma_result.handover != newResult.handover)
        {
            DEBUG_LOG_ERROR("hdma_SetHandoverEvent: Old Handover [%d] New handover [%d]", hdma_core_data->hdma_result.handover, newResult.handover);
        }
        #ifndef DEBUG_HDMA_UT
    ...
    {
        DEBUG_LOG_ERROR("hdma_SetHandoverEvent: Timestamp [%u]: Suppress Handover: reason [%d] due to recently generated handover event, urgency [%d]", hdma_core_data->timestamp, hdma_core_data->hdma_result.reason, hdma_core_data->hdma_result.urgency);
    ...
    HDMA_DEBUG_LOG_INFO("hdma_SetHandoverEvent: Timestamp [%u]: Handover: reason = %d , urgency = %d.", hdma_core_data->timestamp, hdma_core_data->hdma_result.reason, hdma_core_data->hdma_result.urgency);
...
static uint8 hdma_CheckRSSILevel( hdma_timestamp timestamp, int16 rssiHalfLife_ms,int16 rssiMaxAge_ms, int16 absRSSIThreshold,int16 relRSSIThreshold)
{
    ...
    HDMA_DEBUG_LOG_DEBUG("otherRSSI = %d, thisRSSI = %d", otherRSSI, thisRSSI);
...
static void hdma_CheckLinkQualityLevel( hdma_timestamp timestamp, int16 lqHalfLife,int16 lqMaxAge, uint16 absLQ,uint16 relLQ, uint8 *otherIsBetter)
{
    ...
    HDMA_DEBUG_LOG_DEBUG("hdma_CheckLinkQualityLevel: otherLQ = %u, thisLQ = %u, otherIsBetter [%u]", otherLQ, thisLQ, *otherIsBetter);
}
