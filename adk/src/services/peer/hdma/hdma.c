...
#endif
    HDMA_DEBUG_LOG_INFO("INCLUDE_HDMA_CIS_RSSI_EVENT = DISABLED");
...
static void hdma_HandleStateProxyEvent(const STATE_PROXY_EVENT_T* sp_event)
{
    ...
    switch(sp_event->type)
    {
        case state_proxy_event_type_phystate:
            HDMA_DEBUG_LOG_VERBOSE("hdma_HandleStateProxyEvent: Timestamp [%u]: Phy State [%u] source [%d]", sp_event->timestamp, sp_event->event.phystate.event, is_this_bud);
            hdma_HandlePhyState(is_this_bud, sp_event->timestamp, sp_event->event.phystate.event);
            break;
#ifdef INCLUDE_HDMA_MIC_QUALITY_EVENT
       case state_proxy_event_type_mic_quality:
            HDMA_DEBUG_LOG_VERBOSE("hdma_HandleStateProxyEvent: Timestamp [%u] Mic Quality [%u] source [%d]", sp_event->timestamp, sp_event->event.mic_quality.mic_quality, is_this_bud);
        ...
        case state_proxy_event_type_battery_state:
            HDMA_DEBUG_LOG_VERBOSE("hdma_HandleStateProxyEvent: Timestamp [%u] Battery State [%u] source [%d]", sp_event->timestamp, (MESSAGE_BATTERY_REGION_UPDATE_STATE_T *)(&(sp_event->event)), is_this_bud);
        ...
        case state_proxy_event_type_link_quality:
            HDMA_DEBUG_LOG_VERBOSE("hdma_HandleStateProxyEvent: Timestamp [%u] Link Quality [%d] source [%d]", sp_event->timestamp, sp_event->event.link_quality.rssi, is_this_bud);
        ...
        default:
            DEBUG_LOG_INFO("hdma_HandleStateProxyEvent: Event not handled: type [%u]", sp_event->type);
            break;
    }
}
