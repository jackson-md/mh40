...
static bool peer_pair_le_is_advertising_state(PEER_PAIR_LE_STATE state)
{
    switch (state)
    {
...
{
    return peer_pair_le_is_advertising_state(peer_pair_le_get_state());
}
...
static void peer_pair_le_cancel_advertising(void)
{
    ...
    PeerPairLe_UpdateLeAdvertisingData();
#else
...
bool peer_pair_le_enable_advertising(void)
{
    ...
    if(!PeerPairLe_UpdateLeAdvertisingData())
    {
        return FALSE;
    }
#else
...
#ifdef PEER_PAIR_ADJACENT_DEVICE_ONLY
    /* Test mode that will only pair with a single device
       Even will only pair with odd
       The last bit of the LAP must differ
       e.g. 0x00FFFE will only pair with 0x00FFFF */
    ppl->expected_device = ppl->local_addr.addr;
    ppl->expected_device.lap ^= 0x1;
#endif
