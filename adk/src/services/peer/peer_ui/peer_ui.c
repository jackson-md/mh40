...
{
    DEBUG_LOG_ALWAYS("peerUi_UpdateUiInputData ui input is %d", ui_input);
    switch(ui_input)
    {
    ...
    case ui_input_anc_wind_released:
        DEBUG_LOG_ALWAYS("ui input is %d data is %d",ui_input,data);
        WindDetect_SetRemoteAttackStatus(data);
        break;
    ...
    case ui_input_anc_set_world_volume_gain:
        if(!AncStateManager_StoreWorldVolumeGain((int8)data))
        {
            DEBUG_LOG_WARN("peerUi_UpdateUiInputData, unable to store world volume gain %d", data);
        }
        break;

    case ui_input_anc_set_world_volume_balance:
        AncStateManager_StoreEncodedWorldVolumeBalanceInfo(data);
        break;
...
static void peerUi_HandleUiInputRxInd(PEER_SIG_MARSHALLED_MSG_CHANNEL_RX_IND_T* ind)
{
    ...
    DEBUG_LOG_ALWAYS("peerUi_HandleUiInputRxInd");
    ...
    if(rtime_gt(delta, 0))
    {
        ...
        Ui_InjectBackEndUiInput(ui_input, US_TO_MS(delta));
    }
    else
    {
        DEBUG_LOG("peerUi_HandleUiInputRxInd send ui_input(0x%x) in %d ms", ui_input, US_TO_MS(delta));
        Ui_InjectBackEndUiInput(ui_input, 1);
    }
}
...
static uint32 peerUi_ForwardAncUiInputToPeer(ui_input_t ui_input)
{
    ...
    switch(ui_input)
    {
        ...
        case ui_input_anc_wind_detected:
            peerUi_ForwardUiInputWithDataToPeer(ui_input,PEER_ANC_UI_INPUT_DELAY_US, WindDetect_GetLocalAttackStatus());
            break;
        ...
        case ui_input_anc_set_world_volume_gain:
            peerUi_ForwardUiInputWithDataToPeer(ui_input, PEER_ANC_UI_INPUT_DELAY_US, (uint8)AncStateManager_GetRequestedWorldVolumeGain());
            break;

        case ui_input_anc_set_world_volume_balance:
            peerUi_ForwardUiInputWithDataToPeer(ui_input, PEER_ANC_UI_INPUT_DELAY_US, AncStateManager_GetEncodedWorldVolumeBalanceInfo());
            break;
            
        case ui_input_anc_auto_transparency_release_time:
            peerUi_ForwardUiInputWithDataToPeer(ui_input, PEER_ANC_UI_INPUT_DELAY_US, (uint8)AncAutoAmbient_GetReleaseTimeConfig());
            break;
