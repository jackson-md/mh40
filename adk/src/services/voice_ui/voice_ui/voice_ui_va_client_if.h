...
typedef struct
{
    /*! \brief Called once data arrives at the source.
        ...
        \return TRUE is capture data consumed successfully, otherwise FALSE.
     */
    bool (*CaptureDataReceived)(Source source);
...
typedef struct
{
    ...
    void (*SetWakeUpWordDetectionEnable)(bool enable);
    void (*SetPrivacyModeEnable)(bool enable);
...
/*! \brief Start capturing mic data
    ...
    \return voice_ui_audio_success on success, else the type of failure.
            voice_ui_audio_suspended if capture is blocked, this is not considered a failure.
            voice_ui_audio_not_active if the VA handle is not that of the active assistant.
            voice_ui_audio_failed if capture fails to start.
*/
...
/*! \brief Start Wake-Up-Word detection
    ...
    \return voice_ui_audio_success if a new detection chain is started,
            ...
            voice_ui_audio_suspended if detection is blocked and will resume later when it is able.
            voice_ui_audio_not_active if the VA handle is not that of the active assistant.
            voice_ui_audio_failed if detection fails to start.
*/
...
/*! \brief Set the privacy mode flag
    \param enable TRUE if enabling privacy mode, FALSE if disabling privacy mode
 */
void VoiceUi_SetPrivacyModeEnable(bool enable);

/*! \brief Check if privacy mode is enabled
    \return TRUE if privacy mode is enabled, otherwise FALSE
 */
bool VoiceUi_IsPrivacyModeEnabled(void);

/*! \brief Set the wake word detection flag
    \param enable TRUE if enabling wake word detection, FALSE if disabling wake word detection
 */
void VoiceUi_SetWakeUpWordDetectionEnable(bool enable);

/*! \brief Check if wake word detection is enabled
    \return TRUE if wake word detection is enabled, otherwise FALSE
 */
bool VoiceUi_IsWakeUpWordDetectionEnabled(void);
...
/*! \brief Check if Voice Assistant audio has been suspended.
    \param va_handle Handle provided at registration.
    \return If TRUE, you will be unable to start an audio capture or WuW detection.
