...
#define voiceUi_IsVaHandleFunctionPointerValid(va_handle, function_name) \
        (va_handle && va_handle->voice_assistant && va_handle->voice_assistant->function_name)
...
void VoiceUi_DeselectCurrentAssistant(void)
{
    voice_ui_handle_t * va_handle = voiceUi_GetActiveVa();

    if (va_handle && va_handle->voice_assistant && va_handle->voice_assistant->DeselectVoiceAssistant)
    {
        DEBUG_LOG_DEBUG("voiceUi_DeselectCurrentAssistant");
        va_handle->voice_assistant->DeselectVoiceAssistant();
...
void VoiceUi_SetDeviceFlag(device_va_flag_t flag, bool value)
{
    ...
    if (device)
    {
        uint8 flags = device_va_flag_wuw_enabled;
...
void VoiceUi_SetPrivacyModeEnable(bool enable)
{
    ...
    VoiceUi_SetDeviceFlag(device_va_flag_privacy_mode_enabled, enable);

    if(voiceUi_IsVaHandleFunctionPointerValid(va_handle, SetPrivacyModeEnable))
    {
        voiceUi_GetActiveVa()->voice_assistant->SetPrivacyModeEnable(enable);
    }
}

bool VoiceUi_IsPrivacyModeEnabled(void)
{
    return VoiceUi_GetDeviceFlag(device_va_flag_privacy_mode_enabled);
}
...
void VoiceUi_SetWakeUpWordDetectionEnable(bool enable)
{
    if(VoiceUi_IsWakeUpWordFeatureIncluded())
    {
        ...
        if (voiceUi_IsVaHandleFunctionPointerValid(va_handle, SetWakeUpWordDetectionEnable))
        {
            voiceUi_GetActiveVa()->voice_assistant->SetWakeUpWordDetectionEnable(enable);
        }
    }
}
