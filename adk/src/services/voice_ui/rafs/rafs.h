...
typedef enum {
    NAMED_PARTITION,
    UNNAMED_PARTITION,
} rafs_partition_type_t;        /*!< The partition type id */
...
/*! \brief  Switch task data structure between unnamed and named partition access
    \param[in] partition type: unnamed or named.
 */
void Rafs_SwitchTaskData(rafs_partition_type_t type);
