...
#define LENGTH_OF_UNNAMED_PARTITION_NAME 1
...
rafs_errors_t Rafs_PartitionOpen(const char *partition_name)
{
    ...
    if(strlen(partition_name) > LENGTH_OF_UNNAMED_PARTITION_NAME)
    {
        /* Named partition */
        result = Rafs_GetPartitionName(partition_name,
                                       rafs_self->partition->partition_name,
                                       sizeof(rafs_self->partition->partition_name));
        if( result == RAFS_OK )
        {
            ra_result = RaPartitionNamedOpen(rafs_self->partition->partition_name,
                                             &rafs_self->partition->part_handle,
                                             &rafs_self->partition->part_info);
        }
    }
    else
    {
        /* Unnamed partition */
        ra_result = RaPartitionOpen(&rafs_self->partition->part_handle,
                                    &rafs_self->partition->part_info);
    }
