...
#define ANC_CONFIG_KEY_LEN_PER_INSTANCE (212)  /*In QCC517x, key length per instance is 212*/
#else
...
#if defined(__QCC517X__)
#define PSKEY_MAX_LENGTH_SUPPORTED  230
#else
                            ...
                            const struct DeviceTestServiceCommand_HandleAncSetPsKey *set)
{
    ...
    key_to_store = (uint16*)PanicUnlessMalloc(PSKEY_MAX_LENGTH_SUPPORTED);
