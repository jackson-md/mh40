...
/*! Claim buffer space for a response to be populated by the caller

    This can be used to send raw data (binary) without using the
    Device Test Service built in response handling.

    The task passed should match that sent in the function call.

    The function should be followed by a call to 
    DeviceTestService_CommandResponseFlushRaw() to send the data.

   \param task The task information sent for the incoming command
   \param size The amount of buffer requested in octets
   \param[out] raw_area Address of location to hold pointer to allocated buffer

   \return TRUE If buffer of requested size was available
*/
bool DeviceTestService_CommandResponseClaimRaw(Task task, uint16 size, uint8 **raw_area);


/*! Write previously requested raw buffer to the connected device

    The task passed should match that sent in the function call.

    \param task The task information sent for the incoming command
    \param final_response The date being flushed completes response to the
                incoming command
*/
void DeviceTestService_CommandResponseFlushRaw(Task task, bool final_response);
...
#define DeviceTestService_CommandResponseClaimRaw(_task, _size, _raw_area) (UNUSED(_task), UNUSED(_size), UNUSED(_raw_area), FALSE)
#define DeviceTestService_CommandResponseFlushRaw(_task, _response) (UNUSED(_task), UNUSED(_response))
