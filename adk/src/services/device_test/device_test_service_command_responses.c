...
bool DeviceTestService_CommandResponseClaimRaw(Task task, uint16 size, uint8 **raw_area)
{
    ...
    uint16 offset = SinkClaim(dts->rfc_sink, size);
    uint8* base = SinkMap(dts->rfc_sink);

    if (offset != 0xFFFF && base)
    {
        *raw_area = base + offset;
        dts->raw_size = size;
        return TRUE;
    }

    DEBUG_LOG_WARN("DeviceTestService_CommandResponseClaimRaw. Failed to claim %d octets", size);
...
void DeviceTestService_CommandResponseFlushRaw(Task task, bool final_response)
{
    ...
    UNUSED(task);
    UNUSED(final_response);

    if (dts->raw_size)
    {
        SinkFlush(dts->rfc_sink, dts->raw_size);
    }
    else
    {
        DEBUG_LOG_WARN("DeviceTestService_CommandResponseFlushRaw. Called with size variable unset.");
    }
