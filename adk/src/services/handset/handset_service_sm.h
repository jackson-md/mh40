...
typedef enum
{
    ...
    HANDSET_SERVICE_STATE_STOP_CONNECTING_BREDR_PROFILES = 6 + HANDSET_SERVICE_CONNECTING_BREDR_STATE_MASK,
...
typedef struct
{
    ...
    handset_service_ble_state_machine_t ble_sm;
...
#define HandsetServiceSm_GetLeTpBdaddr(sm) sm->ble_sm.le_addr
...
#define BREDR_AND_BLE FALSE
#define BREDR_ONLY    TRUE

/*! \brief Check if all connections have been disconnected for a state machine, 
           including BR/EDR ACL, BR/EDR profiles and optionally BLE ACL

    \param sm State machine to check
    \param bredr_only BREDR_AND_BLE to check BLE ACL connection, BREDR_ONLY to 
           check only BR/EDR ACL and profiles
           
    \return TRUE if all the connections checked are disconnected, otherwise FALSE
*/
...
void HandsetService_SmHandleDisconnection(handset_service_state_machine_t *sm);
...
/*! \brief Have max supported BR/EDR ACL connections been established?

    \return TRUE if max BR/EDR ACL connections reached, otherwise FALSE
*/
bool HandsetServiceSm_MaxBredrAclConnectionsReached(void);

/*! \brief Have max fully(ACL + supported profiles) connected BR/EDR Handset connections been established?

    \return TRUE if max fully(ACL + supported profiles) connected BR/EDR connections reached, otherwise FALSE
*/
...
/*! \brief Count the number of active fully(ACL + supported profiles) connected BR/EDR handset connections.
    \return Number of active BR/EDR fully(ACL + supported profiles) connected BR/EDR handset connections.
*/
...
/*! \brief Get the number of handsets connected by BR/EDR, LE, or both.

    Handsets that are currently in the process of connecting are included in
    the total.

    \return Number of handsets connected by BR/EDR and / or LE.
*/
unsigned HandsetServiceSm_GetBredrOrLeAclConnectionCount(void);

/*! \brief Find out if any handsets could pair

    This function checks if any handset could pair if BLE is connected for a BLE state machine and
    requires an update to the resolving list in the controller.
    If BLE is disconnected for a BLE state machine this function returns FALSE.

    \return TRUE if any handset might pair, FALSE otherwise.
*/
...
/*! \brief Query if any handset service SMs are connecting.
    \return TRUE if any SMs are connecting.
*/
bool HandsetServiceSm_IsConnecting(void);

/*! \brief Query if any handset service SMs are disconnecting.
    \return TRUE if any SMs are disconnecting.
*/
bool HandsetServiceSm_IsDisconnecting(void);

/*! \brief Get the handset_service_state_machine_t associated with a 
           handset_service_ble_state_machine_t

    \param ble_sm The BLE state machine to check
           
    \return The handset_service_state_machine_t associated with the BLE
            state machine. This function will panic if the ble_sm is invalid
            and should never not return a handset_service_state_machine_t
*/
