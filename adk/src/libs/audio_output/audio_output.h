...
/******************************************************************************
NAME:
    AudioOutputEnableDacUltraQuietMode

DESCRIPTION:
    Enable ultra-quiet mode at both DAC outputs (primary_left, primary_right), and
    returns whether requested mode actually set.
    <Note> This feature is available only in QCC517x audio firmware and afterwards.

RETURNS:
    Whether a DAC quiet mode enabled (TRUE = enabled, FALSE = ignored)
	In case of pre-mora2.0 audio firmware, return FALSE.
*/
bool AudioOutputEnableDacUltraQuietMode(void);

/******************************************************************************
NAME:
    AudioOutputDisableDacUltraQuietMode

DESCRIPTION:
    Disable ultra-quiet mode at DAC output (primary_left, primary_right), and
    returns whether requested mode actually set.
    <Note> This feature is available only in QCC517x audio firmware and afterwards.

RETURNS:
    Whether a DAC quiet mode disabled (TRUE = disabled, FALSE = ignored)
	In case of pre-mora2.0 audio firmware, return FALSE.
*/
bool AudioOutputDisableDacUltraQuietMode(void);
