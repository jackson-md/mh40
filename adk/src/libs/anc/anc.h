...
/*!
    @brief Enable the ANC functionality with provided config.
           ANC will start operating in the last set ANC mode filter config.

    @param update_filter_coefficients True to update filter coefficients (IIR, LPF, SmLPF, DC) to ANC hardware.

    @param mute_path_gains True to mute path gains (FFA, FFB, FB) in ANC hardware.
...
/*!
    @brief Set the ANC operating mode. It reads the filter coefficients and path gains from persistent memory
           and configure them into ANC hadrware based on the provided config

    @param anc_mode_t The ANC mode to set.

    @param update_filter True to update filter coefficients (IIR, LPF, SmLPF, DC) to ANC hardware.

    @param enable_gains True to set path gains (FFA, FFB, FB) to ANC hardware.
...
/*!
    ...
    ADVISORY : THIS API IS RECOMMENDED TO BE USED DURING PRODUCTION TUNING/CALIBRATION FOR
    HEADSET CONFIGURATION ONLY. There is a risk to the product in detuning pre-set values if
...
/*!
    @brief Read the ANC fine gain after adding/subracting the Delta gain value
    for specificed path from the particular ANC HW instance
...
/*!
    @brief Read LPF Coefficients

    @param inst             The ANC Hardware instance to get the coefficients from
    @param path             The path to get the coefficients
    @param lpf_shift_1      Output variable to hold the lpf shift 1
    @param lpf_shift_2      Output variable to hold the lpf shift 2
...
/*!
    @brief Read DC Filter Coefficients

    @param inst             The ANC Hardware instance to get the coefficients from
    @param path             The path to get the coefficients
    @param filter_shift     Output variable to hold the filter shift
    @param filter_enable    Output variable to hold the filter enable
...
/*!
    @brief Read Small LPF Coefficients
...
/*!
    @brief Set LPF Coefficients for the specified instance and path. If the ANC functionality is enabled then
           the LPF coefficients will be applied immediately otherwise it will be ignored.

    @param inst             The ANC Hardware instance to set the coefficients to
    @param path             The path to set the coefficients
    @param lpf_shift_1      lpf shift 1 value
    @param lpf_shift_2      lpf shift 2 value

    @return TRUE indicating the LPF coefficients were successfully configured
            otherwise FALSE.
*/
bool AncConfigureLPFCoefficients(audio_anc_instance instance, audio_anc_path_id path, uint8 lpf_shift_1, uint8 lpf_shift_2);

/*!
    @brief Set DC Filter Coefficients for the specified instance and path. If the ANC functionality is enabled then
           the DC Filter coefficients will be applied immediately otherwise it will be ignored.

    @param inst             The ANC Hardware instance to set the coefficients to
    @param path             The path to set the coefficients
    @param filter_shift     Filter shift value
    @param filter_enable    Filter enable value

    @return TRUE indicating the DC Filter coefficients were successfully configured
            otherwise FALSE.
*/
...
/*!
    @brief Set Small LPF Coefficients for the specified instance. If the ANC functionality is enabled then
           the Small LPF coefficients will be applied immediately otherwise it will be ignored.

    @param inst             The ANC Hardware instance to set the coefficients to
    @param filter_shift     Filter shift value
    @param filter_enable    Filter enable value

    @return TRUE indicating the Small LPF coefficients were successfully configured
            otherwise FALSE.
*/
