...
static anc_instance_config_t * getAncInstanceConfig(anc_instance_mask_t instance_mask)
{
    anc_instance_config_t *inst_config;
...
{
    anc_instance_config_t * inst_config = getAncInstanceConfig(inst_mask);
...
{
    return AudioAncFilterLpfSet(instance, path, lpf_shift_1, lpf_shift_2);
}
...
bool ancConfigureDcFilterCoefficients(audio_anc_instance instance, audio_anc_path_id path, uint8 filter_shift, uint8 filter_enable)
{
    ...
    {
        status = SourceConfigure(mic_source, STREAM_ANC_FFA_DC_FILTER_SHIFT, filter_shift) &&
                    SourceConfigure(mic_source, STREAM_ANC_FFA_DC_FILTER_ENABLE, filter_enable);
        break;
    }

    case AUDIO_ANC_PATH_ID_FFB:
    {
        status = SourceConfigure(mic_source, STREAM_ANC_FFB_DC_FILTER_SHIFT, filter_shift) &&
                    SourceConfigure(mic_source, STREAM_ANC_FFB_DC_FILTER_ENABLE, filter_enable);
        break;
    }
...
bool ancConfigureSmLPFCoefficients(audio_anc_instance instance, uint8 filter_shift, uint8 filter_enable)
{
    ...
    return SourceConfigure(mic_source, STREAM_ANC_SM_LPF_FILTER_SHIFT, filter_shift) &&
            SourceConfigure(mic_source, STREAM_ANC_SM_LPF_FILTER_ENABLE, filter_enable);
}
