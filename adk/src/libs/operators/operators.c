...
typedef anc_gain_msg_t atr_vad_msg_t;
...
typedef struct
{
    ...
    uint16 qhs;
    uint16 twm;
} aptxad_96k_encoder_config_params_msg_t;
...
typedef struct
{
    ...
    uint16 quality;
    uint16 cqddr;
} aptxad_96K_encoder_rf_signal_params_msg_t;
...
{
    uint16 id;
    uint16 gains[ahm_static_gain_max];
} ahm_set_static_gain_msg_t;
...
typedef struct
{
    ...
    uint16 filter;
    uint16 target_gain;
} ahm_set_fine_target_gain_msg_t;

typedef ahm_set_static_gain_msg_t ahm_get_static_gain_msg_t;

typedef struct
{
    uint16 id;
    uint16 release_duration;
} atr_vad_set_release_duration_msg_t;

typedef atr_vad_set_release_duration_msg_t atr_vad_get_release_duration_msg_t;

typedef struct
{
    uint16 id;
    uint16 sensitivity;
} atr_vad_set_sensitivity_msg_t;

typedef atr_vad_set_sensitivity_msg_t atr_vad_get_sensitivity_msg_t;
...
typedef struct
{
    ...
    uint16 op_id_lsw;
    uint16 op_id_msw;
} anc_link_hw_mgr_msg_t;

typedef struct
{
    uint16 id;
    uint16 channel;
    uint16 target_op_id_lsw;
    uint16 target_op_id_msw;
    uint16 filter;
    uint16 coarse;
} anc_link_target_gain_msg_t;

typedef struct
{
    uint16 id;
    uint16 frame_type;
} aac_decoder_set_frame_type_msg_t;
...
typedef struct
{
    ...
    uint16 num_numerator;
    value_32bit_t value[];  /*Denominator array followed by Numerator array*/
} ahm_iir_coefficients_msg_t;

typedef struct
{
    uint16 id;
    uint16 current_adaptive_gain;
} ancv2_get_adaptive_gain_msg_t;

typedef struct
{
    uint16 id;
} compander_gain_msg_t;

typedef struct
{
    uint16 id;
    uint16 adjusted_gain;
} compander_get_adjusted_gain_msg_t;
...
}lc3_dec_sco_iso_frame_duration_t;

typedef struct
{
    uint16 msg_id;
    uint16 num_of_channels;
}lc3_dec_sco_iso_set_num_of_channels_t;

typedef struct
{
    uint16 msg_id;
    uint16 channel_id;
}lc3_dec_sco_iso_set_channel_decode_t;

typedef struct
{
    uint16 msg_id;
    uint16 blocks_per_sdu;
} lc3_dec_sco_iso_blocks_per_sdu_t;
...
typedef struct
{
    uint16 msg_id;
    uint16 blocks_per_sdu;
} lc3_enc_sco_iso_blocks_per_sdu_t;

typedef struct
{
    uint16 msg_id;
    uint16 excluded_op_bitmask;
} va_gm_set_mode_t;
...
typedef struct
{
    uint16 excluded_op_bitmask;
    Operator op_list[4];
    uint8 num_of_op_to_delegate;
} va_gm_delegate_op_info_t;
...
static void updateGraphManagerDelegateOpInfo(va_gm_delegate_op_info_t *info, Operator op, uint16 op_not_present_bit)
{
    if (op == INVALID_OPERATOR)
    {
        info->excluded_op_bitmask |= op_not_present_bit;
    }
    else
    {
        PanicFalse(info->num_of_op_to_delegate < ARRAY_DIM(info->op_list));
        info->op_list[info->num_of_op_to_delegate] = op;
        info->num_of_op_to_delegate++;
    }
}

static va_gm_delegate_op_info_t getGraphManagerDelegateOpInfo(const graph_manager_delegate_op_t *op_list)
{
    va_gm_delegate_op_info_t info = {0};
    updateGraphManagerDelegateOpInfo(&info, op_list->vad, 1 << 0);
    updateGraphManagerDelegateOpInfo(&info, op_list->wuw_engine, 1 << 1);
    updateGraphManagerDelegateOpInfo(&info, op_list->cvc, 1 << 2);
    updateGraphManagerDelegateOpInfo(&info, op_list->splitter, 1 << 3);
    return info;
}

static inline bool isGraphManagerThatSupportsExcludedOperators(Operator graph_manager)
{
    capablity_version_t version = OperatorGetCapabilityVersion(graph_manager);
    return (version.version_lsb >= 0x1) ||
           (version.version_msb > 0);
}

static void setGraphManagerExcludedOperatorsBitmask(Operator graph_manager, uint16 excluded_op_bitmask)
{
    va_gm_set_mode_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);
    msg.msg_id = VA_GM_SET_MODE;
    msg.excluded_op_bitmask = excluded_op_bitmask;
    PanicFalse(VmalOperatorMessage(graph_manager, &msg, size_msg, NULL, 0));
}
...
static void delegateOperatorsToGraphManager(Operator graph_manager, const graph_manager_delegate_op_t *op_list, bool delegate)
{
    ...
    va_gm_delegate_op_info_t info = getGraphManagerDelegateOpInfo(op_list);

    if (delegate)
    {
        if (isGraphManagerThatSupportsExcludedOperators(graph_manager))
        {
            /* If GM supports excluded operators always set the bitmask (there is no guarantee it wasn't set to a non-default value previously) */
            setGraphManagerExcludedOperatorsBitmask(graph_manager, info.excluded_op_bitmask);
        }
        else
        {
            /* If any operators are excluded the bitmask is not zero (GM should have supported this!) */
            PanicFalse(info.excluded_op_bitmask == 0);
        }
    }

    PanicFalse(OperatorDelegateMultiple(delegate_op, info.num_of_op_to_delegate, info.op_list));
}
...
void OperatorsAptxAdEncoderSetEncodingParams(Operator op, aptxad_encoder_params_t *params)
{
    ...
    msg.sample_rate =  (uint16)(params->sample_rate / 25);  /* divide by 25 to fit into uint16 */
...
{
    aptxad_96k_encoder_config_params_msg_t msg;

    PanicNull(params);

    msg.id = APTXAD_96K_ENCODER_CONFIG_PARAMS;
    msg.quality_mode    = (uint16)params->quality_mode;
    msg.sample_rate     = (uint16)(params->sample_rate / 25);   /* divide by 25 to fit into uint16 */
    msg.compatibility   = (uint16)params->compatibility;
    msg.mtu             = (uint16)params->mtu;
    msg.split_tx        = (uint16)params->split_tx;
    msg.qhs             = (uint16)params->qhs;
    msg.twm             = (uint16)params->twm;
...
{
    aptxad_96K_encoder_rf_signal_params_msg_t msg;

    PanicNull(params);

    msg.id      = APTXAD_96K_ENCODER_RF_SIGNAL_PARAMS;
    msg.rssi    = (uint16)params->rssi;
    msg.cqddr   = (uint16)params->cqddr;
    msg.quality = (uint16)params->quality;
...
static unsigned getAhmIirCoefficientMessageSize(uint16 num_denominator_coefficients, uint16 num_numerator_coefficients)
{
    unsigned message_size = (unsigned) (sizeof(ahm_iir_coefficients_msg_t) +
                                             ((unsigned) sizeof(value_32bit_t) * num_denominator_coefficients) +
                                             ((unsigned) sizeof(value_32bit_t) * num_numerator_coefficients));
...
{
    lc3_dec_sco_iso_frame_duration_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);
...
{
    lc3_dec_sco_iso_set_channel_decode_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = LC3_DEC_SCO_ISO_SET_MONO_DECODE;
...
{
    lc3_dec_sco_iso_set_num_of_channels_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = LC3_DEC_SCO_ISO_SET_NUM_OF_CHANNELS;
    msg.num_of_channels = num_of_channels;
...
{
    lc3_dec_sco_iso_blocks_per_sdu_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = LC3_DEC_SCO_ISO_SET_BLOCKS_PER_SDU;
    msg.blocks_per_sdu = blocks_per_sdu;
...
void OperatorsLc3EncoderScoIsoSetFrameDuration(Operator op, uint16 frame_duration)
{
    ...
    msg.msg_id = LC3_ENC_SCO_ISO_SET_FRAME_DURATION;
    msg.frame_duration = frame_duration;
...
{
    lc3_enc_sco_iso_blocks_per_sdu_t msg;
    uint16 size_msg = SIZEOF_OPERATOR_MESSAGE(msg);

    msg.msg_id = LC3_ENC_SCO_ISO_SET_BLOCKS_PER_SDU;
    msg.blocks_per_sdu = blocks_per_sdu;
...
{
    OperatorsStandardSetControl(op, ADAPTIVE_ANCV2_MODE_OVERRIDE_CTRL, (unsigned) sysmode);
}

void OperatorsAdaptiveAncV2SetParallelTopology(Operator op, adaptive_anc_filter_config_t filter_config)
{
    OperatorsStandardSetControl(op,ADAPTIVE_ANCV2_FILTER_CONFIG_CTRL,(unsigned)filter_config);
}

void OperatorsAdaptiveAncV2SetSampleRate(Operator op, adaptive_anc_sample_rate_config_t sample_rate)
{
    OperatorsStandardSetControl(op, ADAPTIVE_ANCV2_SET_SAMPLE_RATE_CTRL, (unsigned) sample_rate);
}

uint16 OperatorsAdaptiveAncV2GetAdaptiveGain(Operator op)
{   
    ancv2_get_adaptive_gain_msg_t get_adaptive_gain;
    anc_gain_msg_t gain_msg;

    gain_msg.id = ADAPTIVE_ANC_GET_ADAPTIVE_GAIN;
    PanicFalse(VmalOperatorMessage(op,&gain_msg,SIZEOF_OPERATOR_MESSAGE(gain_msg), &get_adaptive_gain, SIZEOF_OPERATOR_MESSAGE(get_adaptive_gain)));
    return get_adaptive_gain.current_adaptive_gain;
}

void OperatorsAhmSetStaticGain(Operator op, uint16 *gains)
{
    ahm_set_static_gain_msg_t gain_msg;
    gain_msg.id = AHM_SET_STATIC_GAIN;

    memcpy(&gain_msg.gains[0], gains, sizeof(uint16) * ahm_static_gain_max);
...
{
    ahm_set_fine_target_gain_msg_t gain_msg;
    gain_msg.id = AHM_SET_FINE_TARGET_GAIN;
    gain_msg.filter = AHM_FF_FILTER;
    gain_msg.target_gain = target_gain;
                                                                                                    ...
                                                                                                    const iir_coefficients_t *denominator_coefficients)
{
    ...
    unsigned message_size = getAhmIirCoefficientMessageSize(num_denominator, num_numerator);
    ahm_iir_coefficients_msg_t* set_iir_msg = PanicUnlessMalloc(message_size);
    uint16 id=0;

    memset(set_iir_msg, 0, message_size);

    set_iir_msg->msg_id = AHM_SET_IIR_FILTER_COEFFS;
    set_iir_msg->instance = inst;
    set_iir_msg->path = path;

    if (denominator_coefficients)
    {
        set_iir_msg->num_denominator = num_denominator;
        for (id=0; id < num_denominator; id++)
        {
            set_iir_msg->value[id] = split32BitWordTo16Bits(denominator_coefficients->coefficients[id]);
        }
    }

    if (numerator_coefficients)
    {
        set_iir_msg->num_numerator = num_numerator;
        for (id=0; id < num_numerator; id++)
        {
            set_iir_msg->value[num_denominator+id] = split32BitWordTo16Bits(numerator_coefficients->coefficients[id]);
        }
    }

    PanicFalse(VmalOperatorMessage(op, set_iir_msg, SIZEOF_OPERATOR_MESSAGE_ARRAY(message_size), NULL, 0));
    free(set_iir_msg);
}

void OperatorsAhmSetTriggerTransitionCtrl(Operator op, ahm_trigger_transition_ctrl_t transition)
{
    OperatorsStandardSetControl(op, AHM_TRIGGER_TRANSITION_CTRL, (unsigned)transition);
}

void OperatorsAhmSetTaskPeriod(Operator op, uint16 period, uint16 decim_factor)
{
    ahm_set_task_period_msg_t ahm_set_task_period_msg;

    ahm_set_task_period_msg.id = AHM_SET_TASK_PERIOD;
    ahm_set_task_period_msg.task_period = period;
    ahm_set_task_period_msg.decim_factor = decim_factor;

    PanicFalse(VmalOperatorMessage(op, &ahm_set_task_period_msg, SIZEOF_OPERATOR_MESSAGE(ahm_set_task_period_msg), NULL, 0));
}

uint16 OperatorsAhmGetFeedForwardFineGain(Operator op)
{   
    ahm_get_static_gain_msg_t get_gains;
    anc_gain_msg_t gain_msg;

    gain_msg.id = AHM_GET_GAIN;
    PanicFalse(VmalOperatorMessage(op,&gain_msg,SIZEOF_OPERATOR_MESSAGE(gain_msg), &get_gains, SIZEOF_OPERATOR_MESSAGE(get_gains)));
    return (uint16) get_gains.gains[ahm_static_gain_ff_fine];
}
...
uint16 OperatorsAhmGetFeedBackFineGain(Operator op)
{
    ...
    gain_msg.id = AHM_GET_GAIN;
    PanicFalse(VmalOperatorMessage(op,&gain_msg,SIZEOF_OPERATOR_MESSAGE(gain_msg), &get_gains, SIZEOF_OPERATOR_MESSAGE(get_gains)));
    return (uint16) get_gains.gains[ahm_static_gain_fb_fine];
}
...
uint32 OperatorsAhmGetConfig(Operator op)
{
    uint32 ahm_config=0;
    get_params_data_t* get_params_data = OperatorsCreateGetParamsData(1);
    get_params_data->standard_params->id = AHM_CONFIG_PARAM_INDEX;
    OperatorsStandardGetParameters(op, get_params_data);
    ahm_config = (uint32)(get_params_data->standard_params->value);
...
{
    standard_param_t parameter = {AHM_CONFIG_PARAM_INDEX, (standard_param_value_t)ahm_config};
    operatorsSetSingleParameter(op, &parameter);
}

void OperatorsAhmSetSysmodeCtrl(Operator op, ahm_sysmode_t mode)
{
    OperatorsStandardSetControl(op, AHM_SYSMODE_CTRL, (unsigned) mode);
}

void OperatorsAhmSetChannelCtrl(Operator op, ahm_channel_t channel)
{
    OperatorsStandardSetControl(op, AHM_CHANNEL_CTRL, (unsigned) channel);
}

void OperatorsAhmSetFeedforwardPathCtrl(Operator op, ahm_ffpath_t path)
{
    OperatorsStandardSetControl(op, AHM_FFPATH_CTRL, (unsigned) path);
}

void OperatorsAhmSetAmbientCtrl(Operator op, ahm_ambient_ctrl_t ambient_ctrl)
{
    OperatorsStandardSetControl(op, AHM_AMBIENT_CTRL, (unsigned) ambient_ctrl);
}

void OperatorsAhmSetInEarCtrl(Operator op, bool enable)
{
    OperatorsStandardSetControl(op, AHM_INEAR_CTRL, (unsigned)enable);
}

void OperatorsAhmSetFfPathFineGainrCtrl(Operator op, uint8 ff_fine_gain)
{
    OperatorsStandardSetControl(op, AHM_FFPATH_FINE_GAIN_CTRL, (unsigned)ff_fine_gain);
}

void OperatorsAncCompanderSetSysmodeCtrl(Operator op, anc_compander_sysmode_t mode)
{
    OperatorsStandardSetControl(op, ANC_COMPANDER_SYSMODE_CTRL, (unsigned) mode);
}

void OperatorsHowlingControlSetSysmodeCtrl(Operator op, hc_sysmode_t mode)
{
    OperatorsStandardSetControl(op, HCGR_ANC_SYSMODE_CTRL, (unsigned) mode);
}
...
uint32 OperatorsHowlingControlGetConfig(Operator op)
{
    ...
    get_params_data_t* get_params_data = OperatorsCreateGetParamsData(1);
    get_params_data->standard_params->id = HCGR_CONFIG_PARAM_INDEX;
    OperatorsStandardGetParameters(op, get_params_data);
    hcgr_config = (uint32)(get_params_data->standard_params->value);
    free(get_params_data);
    return hcgr_config;
}
...
void OperatorsAncCompanderSetMakeUpGain(Operator op, int32 gain)
{
    set_params_data_t* set_params_data = OperatorsCreateSetParamsData(1);
    set_params_data->number_of_params = 1;
    set_params_data->standard_params[0].id = ANC_COMPANDER_MAKEUP_GAIN_PARAM_INDEX;
    set_params_data->standard_params[0].value = (standard_param_value_t) gain;
    OperatorsStandardSetParameters(op, set_params_data);
    free(set_params_data);
}
...
{
    standard_param_value_t makeUpGain=0;
    get_params_data_t* get_params_data = OperatorsCreateGetParamsData(1);
    get_params_data->standard_params->id = ANC_COMPANDER_MAKEUP_GAIN_PARAM_INDEX;
    OperatorsStandardGetParameters(op, get_params_data);
    makeUpGain = get_params_data->standard_params->value;
    free(get_params_data);
    return(makeUpGain);
}
...
{
    standard_param_t parameter = {AHM_FF_RAMP_PRIORITY, (standard_param_value_t)value};
    operatorsSetSingleParameter(op, &parameter);
}

void OperatorsAncSetCompanderPriority(Operator op, unsigned value)
{
    standard_param_t parameter = {ANC_COMPANDER_PRIORITY, (standard_param_value_t)value};
    operatorsSetSingleParameter(op, &parameter);
}

uint16 OperatorsCompanderGetAdjustedGain(Operator op)
{   
    compander_get_adjusted_gain_msg_t get_adjusted_gain;
    compander_gain_msg_t gain_msg;

    gain_msg.id = COMPANDER_GET_ADJUSTED_GAIN;
    
    PanicFalse(VmalOperatorMessage(op,&gain_msg,SIZEOF_OPERATOR_MESSAGE(gain_msg), &get_adjusted_gain, SIZEOF_OPERATOR_MESSAGE(get_adjusted_gain)));
    return get_adjusted_gain.adjusted_gain;
}

void OperatorsAncLinkHwManager(Operator op, unsigned link, uint32 ahm_op_id)
{
    anc_link_hw_mgr_msg_t lnk_msg;

    lnk_msg.id = ANC_LINK_HW_MANAGER;
    lnk_msg.link = (uint16) link;

    lnk_msg.op_id_lsw = (uint16)(ahm_op_id & 0xffff);
    lnk_msg.op_id_msw = (uint16)(ahm_op_id >> 16);

    PanicFalse(VmalOperatorMessage(op, &lnk_msg, SIZEOF_OPERATOR_MESSAGE(lnk_msg), NULL, 0));
}

void OperatorsAncLinkTargetFFFineGain(Operator op, uint32 target_op_id)
{
    anc_link_target_gain_msg_t msg;

    msg.id = ANC_HCGR_LINK_TARGET_GAIN;
    msg.channel = HCGR_ANC_CHANNEL;

    msg.target_op_id_lsw = (uint16)(target_op_id & 0xffff);
    msg.target_op_id_msw = (uint16)(target_op_id >> 16);
    msg.filter = (uint16) HCGR_ANC_FF_FILTER;
    msg.coarse = (uint16) HCGR_ANC_FINE_GAIN;
...
{
    OperatorsStandardSetControl(op, WIND_DETECT_SYSMODE_CTRL, (unsigned) mode);
}

void OperatorsAahSetSysmodeCtrl(Operator op, aah_sysmode_t mode)
{
	OperatorsStandardSetControl(op, AAH_SYSMODE_CTRL, (unsigned) mode);
}

void OperatorsAacDecoderSetFormat(Operator op, aac_frame_format_t format)
{
    aac_decoder_set_frame_type_msg_t msg;
    msg.id = AAC_DECODER_SET_FRAME_TYPE;
    msg.frame_type = format;
    PanicFalse(VmalOperatorMessage(op , &msg, SIZEOF_OPERATOR_MESSAGE(msg), NULL, 0));
}

void OperatorsAtrVadSetSysmodeCtrl(Operator op, atr_vad_sysmode_t mode)
{
    OperatorsStandardSetControl(op, ATR_VAD_SYSMODE_CTRL, (unsigned) mode);
}

void OperatorsAtrVadSetReleaseDuration(Operator op, atr_vad_release_duration_t release_duration)
{
    atr_vad_set_release_duration_msg_t msg;
    msg.id = ATR_VAD_SET_RELEASE_DURATION;
    msg.release_duration = (uint16) release_duration;
...
{   
    atr_vad_get_release_duration_msg_t info;
    atr_vad_msg_t get_msg;

    get_msg.id = ATR_VAD_GET_RELEASE_DURATION;
    PanicFalse(VmalOperatorMessage(op, &get_msg, SIZEOF_OPERATOR_MESSAGE(get_msg), &info, SIZEOF_OPERATOR_MESSAGE(info)));
    return (atr_vad_release_duration_t) info.release_duration;
}

void OperatorsAtrVadSetSensitivity(Operator op, atr_vad_sensitivity_t sensitivity)
{
    atr_vad_set_sensitivity_msg_t msg;
    msg.id = ATR_VAD_SET_SENSITIVITY;
    msg.sensitivity = (uint16) sensitivity;
...
{   
    atr_vad_get_sensitivity_msg_t info;
    atr_vad_msg_t get_msg;

    get_msg.id = ATR_VAD_GET_SENSITIVITY;
    PanicFalse(VmalOperatorMessage(op, &get_msg, SIZEOF_OPERATOR_MESSAGE(get_msg), &info, SIZEOF_OPERATOR_MESSAGE(info)));
    return (atr_vad_sensitivity_t) info.sensitivity;
}

void * OperatorsGetApvaMetadata(Operator op, size_t size)
{
    uint16 recv_msg[64];
    apva_metadata_msg_t metadata_msg = {.id = GET_APVA_METADATA, .part_number = 1};
    size_t part_size;
    void *metadata = PanicUnlessMalloc(size);
    while (size > 0)
    {
        PanicFalse(VmalOperatorMessage(op, &metadata_msg, SIZEOF_OPERATOR_MESSAGE(metadata_msg), recv_msg, SIZEOF_OPERATOR_MESSAGE(recv_msg)));
        /* uint16 response[]: message_id, metadata part_number, part_size_in_bytes, 1st uint16 of part requested, ... */
        PanicFalse(recv_msg[1] == metadata_msg.part_number);
        part_size = recv_msg[2];
        DEBUG_LOG("OperatorGetApvaMetadata: Received part_number %d, part_size %d", metadata_msg.part_number, part_size);
        memcpy(metadata, &recv_msg[3], part_size);
        size -= part_size;
        metadata_msg.part_number++;
    }
    return metadata;
}
