...
static void device_KeyValueAddedCallback(key_value_list_t list, key_value_key_t key, const void *value, size_t size)
{
    if (onPropertySetHandler)
    {
        onPropertySetHandler(list, key, value, size);
    }
}
...
void Device_RegisterOnPropertySetHandler(Device_OnPropertySet handler)
{
    ...
    /* Register callback with key value list */
    KeyValueList_RegisterAddCallback(device_KeyValueAddedCallback);
}

bool Device_Merge(device_t source_device, device_t target_device, device_merge_resolve_callback_t resolve_callback)
{
    return KeyValueList_Merge(source_device, target_device, resolve_callback);
}
