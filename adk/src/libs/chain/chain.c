...
kymera_chain_handle_t ChainCreateWithFilter(const chain_config_t *config, const operator_filters_t* filter)
{
    ...
    DEBUG_LOG("ChainCreateWithFilter: config %p, filter %p, chain %p", config, filter, chain);
...
{
    kymera_chain_t *chain = handle;
    DEBUG_LOG("ChainDestroy: chain %p", chain);
...
Sink ChainGetInput(kymera_chain_handle_t handle, unsigned input_role)
{
    ...
    DEBUG_LOG("ChainGetInput: chain %p, input_role %d, sink %p", chain, input_role, sink);
    return sink;
}
...
Source ChainGetOutput(kymera_chain_handle_t handle, unsigned output_role)
{
    ...
    DEBUG_LOG("ChainGetOutput: chain %p, output_role %d, source %p", chain, output_role, source);
    return source;
}
...
bool ChainStartAttempt(kymera_chain_handle_t handle)
{
    ...
    DEBUG_LOG("ChainStartAttempt: %p", chain);

    if (!runFunctionOnMultipleOperators(startOperators, chain, NULL))
    {
        DEBUG_LOG_WARN("ChainStartAttempt: Unable to start all operators. Stopping started operators.");
        ChainStop(chain);
        return FALSE;
    }
    return TRUE;
