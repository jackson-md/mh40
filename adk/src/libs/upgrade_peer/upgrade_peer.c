 ...
 * Second part maintains this gap even after reading a new packet. */
#define READ_WRITE_OFFSET_GAP 1024 + UPGRADE_MAX_FORWARD_PACKET_SIZE

/* Zero offset to process/request data. */
#define NO_OFFSET 0
...
static upgrade_partition_reader_handle_t peer_partition_reader_handle = NULL;
...
static upgrade_peer_status_t upgradePeer_SendData(void)
{
    ...
    const uint16 req_data_bytes = MIN(remaining_size, UPGRADE_MAX_FORWARD_PACKET_SIZE);
    ...
    status = (upgrade_peer_status_t)UpgradePartitionReader_ReadData(peer_partition_reader_handle,
                                                                    &payload[UPGRADE_PEER_PACKET_HEADER + UPGRADE_DATA_MIN_DATA_LENGTH],
...
static void upgradePeer_ReceiveDataBytesREQ(UPGRADE_PEER_START_DATA_BYTES_REQ_T *data)
{
    ...
    if (data->common.length == UPGRADE_DATA_BYTES_REQ_DATA_LENGTH)
    {
        ...
        UpgradePartitionReader_SetPartitionOffset(peer_partition_reader_handle, 0);

        if(data->start_offset)
        {
            /* peer has requested the file from certain offset. Figure out the partition state and other 
             * counter variables and set the state properly to resume data transfer from this offset.
             */
            upgrade_partition_state_t expected_state;
            UpgradeGetPartitionStateFromDfuFileOffset(data->start_offset, &expected_state);
        
            UpgradePartitionReader_SetPskey(peer_partition_reader_handle, expected_state.pskey);
            /* Convert pskey offset from bytes to words. */
            UpgradePartitionReader_SetReadOffset(peer_partition_reader_handle, (expected_state.pskey_offset/2));
            UpgradePartitionReader_SetState(peer_partition_reader_handle, expected_state.state);
        
            UpgradePartitionReader_SetPartitionNumber(peer_partition_reader_handle, expected_state.part_num);
            if(expected_state.state == UPGRADE_PARTITION_DATA_STATE_FOOTER)
            {
                UpgradePartitionReader_SetFooterLength(peer_partition_reader_handle, expected_state.partition_len);
            }
            else
            {
                UpgradePartitionReader_SetPartitionDataLength(peer_partition_reader_handle, expected_state.partition_len);
            }
            UpgradePartitionReader_SetPartitionOffset(peer_partition_reader_handle, expected_state.part_offset);

            /*
             * Set the offset from where we want to read the data in the current partition.
             */
            upgradePeerInfo->SmCtx->req_start_offset = expected_state.part_offset;
        }
 ...
 */
static bool upgradePeer_DelayParallelReadRequest(uint32 req_offset)
{
    uint32 prim_file_offset = UpgradeGetDfuFileOffset();

    uint32 prim_offset    = UpgradeCtxGetPartitionDataOffset();
    uint32 sec_offset     = UpgradePartitionReader_GetPartitionOffset(peer_partition_reader_handle);
    ...
    uint16 sec_partn      = UpgradePartitionReader_GetPartitionNumber(peer_partition_reader_handle);

    DEBUG_LOG_VERBOSE("upgradePeer_DelayParallelReadRequest Primary partition %d Written so far %u, Secondary partition %d Read so far %u, total file offset %u and req_offset %u", 
        prim_partn, written_so_far, sec_partn, read_so_far, prim_file_offset, req_offset);

    /* If Peer has requested data from an offset from start of the DFU file then Verify if we have received that much data. */
    if(!UpgradeCtxIsPartitionDataStateFooter() && prim_file_offset <= (READ_WRITE_OFFSET_GAP + req_offset))
    {
        DEBUG_LOG_INFO("upgradePeer_DelayParallelReadRequest Requested data not available yet");
        return TRUE;
    }
    ...
    {
        DEBUG_LOG_INFO("upgradePeer_DelayParallelReadRequest : as secondary partiton > primary partition");
        return TRUE;
    }
    ...
    if(prim_partn == sec_partn )
    {
        ...
        {
            DEBUG_LOG_INFO("upgradePeer_DelayParallelReadRequest: Footer state, Is DataTransfer Complete = %d",UpgradeIsDataTransferComplete());
            return UpgradeIsDataTransferComplete() ? FALSE: TRUE;
        }
        else
        {
            ...
            {
                DEBUG_LOG_INFO("upgradePeer_DelayParallelReadRequest: Secondary trying to read within %d bytes of write offset", READ_WRITE_OFFSET_GAP);
                return TRUE;
            }
        }
...
static void upgradePeer_SelfKickNextDataBlock(void)
{
    ...
    if (upgradePeerInfo->SmCtx->total_sent_size < upgradePeerInfo->SmCtx->total_req_size)
    {
        ...
        * of primary for the same partition, if so delay the read*/
        if(upgradePeer_DelayParallelReadRequest(NO_OFFSET))
        {
...
static void upgradePeer_HandlePeerAppMsg(uint8 *data)
{
    ...
    switch(msgId)
    {
        ...
        case UPGRADE_PEER_DATA_BYTES_REQ:
            ...
            DEBUG_LOG("UpgradePeer_HandlePeerAppMsg UPGRADE_PEER_DATA_BYTES_REQ_T start_offset %lu, last closed partition %d, peer read partition %d",
                (local_msg->start_offset), UpgradeCtxGetPSKeysLastClosedPartition() - 1, UpgradePartitionReader_GetPartitionNumber(peer_partition_reader_handle));

            if(upgradePeer_DelayParallelReadRequest(local_msg->start_offset))
            {
...
void UpgradePeerCtxInit(void)
{
    if(upgradePeerInfo != NULL && !UPGRADE_PEER_IS_STARTED)
    {
        ...
        upgradePeer_CtxSet();
        peer_partition_reader_handle = UpgradePartitionReader_CreateInstance();
    }
}
    ...
    switch(id)
    {
        case UPGRADE_PEER_CONNECT_CFM:
            ...
            if(state == UPGRADE_PEER_CONNECT_SUCCESS)
            {
                switch(upgradePeerInfo->SmCtx->peerState)
                {
                    ...
                    {
                        /* Once the upgrade peer connection is established, initiate the upgrade reboot process */
        ...
        case UPGRADE_PEER_DISCONNECT_IND:
            ...
            else if(upgradePeerInfo->SmCtx->peerState == UPGRADE_PEER_STATE_DATA_TRANSFER)
            {
                 ...
                 */
                UpgradePartitionReader_DestroyInstance(peer_partition_reader_handle);
                UpgradePeerDeInit();
...
/****************************************************************************
NAME
    UpgradePeerReconnectForReboot

DESCRIPTION
    Check the resume point for UPGRADE_RESUME_POINT_PRE_REBOOT, and then start peer connect
*/

void UpgradePeerReconnectForReboot(void)
{
    if(UpgradeIsResumePointPreReboot())
    {
        DEBUG_LOG_INFO("UpgradePeerReconnectForReboot : Start Peer Connect");
        UpgradePeerCtxInit();
