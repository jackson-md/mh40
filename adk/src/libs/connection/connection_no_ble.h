...
typedef enum
{
    ...
    CL_DM_ULP_PERIODIC_ADV_ENABLE_CFM,
    CL_DM_BLE_SET_DATA_RELATED_ADDRESS_CHANGES_CFM,
...
typedef struct
{
    ...
    bdaddr           bd_addr;
    /*! Specifies whether SCO or eSCO packet type is requested. */
    sync_link_type   link_type;
...
typedef struct
{
    ...
    bool  sm_over_bredr;
    /*! A link key already exists locally */
    bool link_key_exists;
...
/*!
    ...
    Trusted Device List (TDL) size. This has to be between a minimum of 1 device
    and a maximum of 12 devices.

    Note: Do not attempt to increase the TDL size while performing a DFU upgrade,
    this may lead to device inoperability due to list indexing changes for sizes
    in the range 9-12.
...
/*! @brief This function is called to initialise the Multipoint Connection
    ...
    in the Trusted Device List, with a minimum of 1 and a maximum of 12, the
...
typedef struct
{
    /*! Indicates the success or failure of change address Req. */
    connection_lib_status   status;
} CL_INTERNAL_DM_ULP_PERIODIC_ADV_ENABLE_CFM_T;
