...
static media_channel* findMediaFromConnectionId (remote_device *device, uint16 connection_id)
{
    ...
    if (device != NULL)
    {
        for (i=0; i<A2DP_MAX_MEDIA_CHANNELS; i++)
        {
            ...
            DEBUG_LOG_INFO("findMediaFromConnectionId state=0x%x inbound_cid=0x%d outbound_cid=0x%d sink=0x%x",
                           media->status.conn_info.connection_state,
                           media->connection.setup.inbound_cid,
                           media->connection.setup.outbound_cid,
                           (uintptr_t)media->connection.active.sink);
            
            switch (media->status.conn_info.connection_state)
            {
            case avdtp_connection_idle:
                /* No media channel in this state */
                break;
            ...
            case avdtp_connection_disconnect_pending:
                /* Possible when disconnect_pending that there may or may not be an active sink so check
                 * connection.setup cids or connection.active.sink */
                if ( (connection_id == media->connection.setup.inbound_cid) ||
                     (connection_id == media->connection.setup.outbound_cid) ||
                     (AVDTP_OUTGOING_CONNECTION_ID == media->connection.setup.outbound_cid) ||
                     connection_id == SinkGetL2capCid(media->connection.active.sink))
                {
                    return media;
                }
                break;
