...
UpgradeHostErrorCode upgradeHandleFooterState(const uint8 *data, uint16 len, bool reqComplete)
{
    ...
    DEBUG_LOG_DEBUG("HandleFooterState isDataXferComplete %d", UpgradeCtxGet()->isDataXferComplete);

    /* Process the data if the data transfer is not over yet. Otherwise, we could have requested 
     * the footer again on resuming to avoid request with 0 bytes so, ignore it */
    if(!UpgradeCtxGet()->isDataXferComplete)
    {
