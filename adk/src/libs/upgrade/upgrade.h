...
#define UPGRADE_SEND_STATUS_IND_WITH_DELAY                           300

#define UPGRADE_SEND_ABORT_CFM_WITH_DELAY                            200

#define UPGRADE_SEND_END_DATA_IND_WITHOUT_DELAY                      0

/*!<
 * If any other component wants to forward the data received by the upgrade 
 * library to other devices, it can send it in the chunks of size defined by
 * this macro. This size is optimally decided to work in all use-cases without
 * exhausting the memory.
 */
#define UPGRADE_MAX_FORWARD_PACKET_SIZE 240
...
/*!
    @brief Enumeration of DFU file partition states.
*/
typedef enum
{
...
typedef enum
{
    ...
    /*! Dfu file for case has been detected and the first part of the header has been received. */
    UPGRADE_CASE_DFU_CASE_IMAGE_IND,

    /*! Dfu file for earbud has been detected. */
    UPGRADE_CASE_DFU_EARBUD_IMAGE_IND,

    /*! New Dfu file for case has been detected. Abort the previous DFU */
    UPGRADE_CASE_DFU_CASE_ABORT,

    /*! Transport has been reconnected during case DFU and upgrade lib has handled 
     *  UPGRADE_HOST_START_REQ. 
     */
    UPGRADE_CASE_DFU_RESUME_IND,
    ...
    /*! Message sent to DFU domain to sync revert reboot between devices
    */
    UPGRADE_SYNC_REVERT_REBOOT,
...
typedef enum
{
    ...
    upgrade_ops_check_peer_connection_pre_reboot
} upgrade_ops_t;
...
#define UPGRADE_IS_CASE_SUPPORTED           UpgradeGetCaseDfuSupport()
...
/*!
    @brief DFU partition state variables used to process(read or write) DFU file data from a certain.offset.
*/
typedef struct
{
...
/*!
    @brief To check if the upgrade state has moved on from Data transfer.

    Returns TRUE if the Upgrade state is in hash checking or higher.
*/
...
/*!
    @brief Set the clientTask in upgradeCtx.
    @param clientTask task to set
*/
void UpgradeClientConnect(Task clientTask);

/*!
    @brief Reset the upgrade lib task as clientTask.
*/
void UpgradeClientReconnectLib(void);

/*!
    @brief UpgradeTransportDataCfm
    Send all the scheduled data confirmation messages to the transport task.
*/
void UpgradeTransportDataCfm(void);

/*!
    @brief Schedule a data confirmation message to be sent to the transport task.

        All scheduled Messages can be sent by calling UpgradeTransportDataCfm.
*/
...
/*!
    @brief UpgradeSetCaseDfuSupport

    Set the flag to indicate if case dfu is supported

    @param is_case_dfu_supported Used to assign required value (0 or 1)
    @return None
*/
void UpgradeSetCaseDfuSupport(bool is_case_dfu_supported);

/*!
    @brief UpgradeGetCaseDfuSupport

    Find out if case dfu is supported.

    @return TRUE if case dfu is supported
*/
bool UpgradeGetCaseDfuSupport(void);

/*!
    @brief UpgradeIsCaseDfuInProgress

    @return TRUE if Case DFU file has been detected and Upgrade is in progress
*/
bool UpgradeIsCaseDfuInProgress(void);

/*!
    @brief UpgradeSetResumePoint

    @param resume_point new resume-point to set in pskey
*/
void UpgradeSetResumePoint(UpdateResumePoint resume_point);

/*!
    @brief UpgradeGetCaseDfuHeaderLength

    Get the stored header length if we have received the header first part.

    @return length
*/
uint32 UpgradeGetCaseDfuHeaderLength(void);

/*! @brief Clean-up the upgrade library context after the case DFU ends.
...
/*! @brief Given the offset from the start of DFU file, calculate partition state and other related info to 
            start processing data from this offset.

    @param req_offset offset from start of DFU file
    @param state pointer to a struct which will be filled if state is calculated without error
    @return TRUE if success
*/
bool UpgradeGetPartitionStateFromDfuFileOffset(uint32 req_offset, upgrade_partition_state_t* state);

/*! @brief Wrapper to internal message sent to upgrade lib to delay reboot due to revert by a stipulated time

    @param None
    @return None
*/
void Upgrade_DelayRevertReboot(void);

/*! @brief Get isDfuAbortDueToSyncIdMismatch info

    @param None
    @return TRUE if the DFU has been aborted due to sync ID mismatch while resuming it
*/

bool UpgradeIsAbortDfuDueToSyncIdMismatch(void);

/*! @brief Handle Upgrade Reboot Process
...
/*! @brief Match the resume point with UPGRADE_RESUME_POINT_PRE_REBOOT

    @param None
    @return TRUE if resume point is UPGRADE_RESUME_POINT_PRE_REBOOT, else FALSE
*/

bool UpgradeIsResumePointPreReboot(void);

/*! @brief Match the resume point with UPGRADE_RESUME_POINT_START

    @param None
    @return TRUE if resume point is UPGRADE_RESUME_POINT_START, else FALSE
*/
