    ...
    *is_data_cfm_needed = TRUE;
    ...
    switch (msgId)
    {
        ...
        case UPGRADE_HOST_DATA:
        {
            ...
            {
                if(UpgradeCtxGet()->isTransportCfmDelayed)
                {
                    /* If the data confirmation needs to be delayed then the client will schedule the confirmation 
                     * for UPGRADE_HOST_DATA messages. */
                    MESSAGE_MAKE(data_cfm, UPGRADE_TRANSPORT_DATA_CFM_T);
                    data_cfm->packet_type = data[0];
                    data_cfm->status = upgrade_status_success;
                    data_cfm->data = data;
                    data_cfm->size_data = dataSize;

                    MESSAGE_MAKE(msg, UPGRADE_HOST_DATA_WITH_ACK_T);
                    msg->length = msgLen - 1;
                    msg->lastPacket = ByteUtilsGet1ByteFromStream(msgPayload);
                    msg->data = msgPayload + 1;
                    msg->dataCfm = data_cfm;
                    msg->transportTask = UpgradeCtxGet()->transportTask;

                    DEBUG_LOG_VERBOSE("UpgradeHostIFDataBuildIncomingMsg, UPGRADE_HOST_DATA, with ack length %u, last_packet %u", msg->length, msg->lastPacket);
                    MessageSend(clientTask, msgId, msg);
                    *is_data_cfm_needed = FALSE;
                }
                else
                {
