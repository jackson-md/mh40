    ...
    uint16 init_config_version)
{
    ...
    if(UpgradeInProgressId() && UpgradeCtxGetPSKeys()->upgrade_in_progress_key == UPGRADE_RESUME_POINT_START)
    {
        UpgradeRebootHandlerCalculateDfuOffset();
    }
...
{
    if(UpgradeSMGetState() >= UPGRADE_STATE_DATA_HASH_CHECKING)
        return TRUE;
    else
        return FALSE;
}
...
{
    return UpgradeRebootHandlerGetPartitionStateFromDfuFileOffset(req_offset, state);
}
