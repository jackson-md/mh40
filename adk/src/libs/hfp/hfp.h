...
typedef struct
{
    ...
    /*! Indicates whether a SCO or eSCO link is requested. */
...
/*!
    ...
    @param packet_type The packet type to use specifically for this SCO/eSCO connection
    
    @param audio_params The audio parameters to use, if NULL defaults will be used
    
    @param disable_override Stop the lib setting WBS compliant eSCO 
    params/packet type (for test only)
    
    It is the application's responsibility to ensure that the remote device supports the
    specified packet_type and audio_params parameters.
