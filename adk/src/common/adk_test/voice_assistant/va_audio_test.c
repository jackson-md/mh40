...
static bool vaTestPopulateVaEncodeConfig(va_audio_codec_t encoder, va_audio_encode_config_t *config)
{
    ...
    DEBUG_LOG_WARN("vaTestPopulateVaEncodeConfig: No configuration found for encoder option enum:va_audio_codec_t:%d", encoder);
    return FALSE;
}
...
static bool vaTestPopulateVaMicConfig(unsigned num_of_mics, va_audio_mic_config_t *config)
{
    ...
    if (config->max_number_of_mics >= 1)
    {
    	return TRUE;
    }
    else
    {
    	DEBUG_LOG_WARN("vaTestPopulateVaMicConfig: Number of mics has to be greater than zero");
    	return FALSE;
    }
}
...
static bool vaTestPopulateWuwDetectionParams(va_wuw_engine_t engine, unsigned num_of_mics, va_audio_wuw_detection_params_t *params)
{
    ...
    for(i = 0; i < ARRAY_DIM(wuw_detection_start_table); i++)
    {
        if (wuw_detection_start_table[i].engine == engine)
        {
            ...
            {
            	DEBUG_LOG_WARN("vaTestPopulateWuwDetectionParams: Model %s not in RO filesystem", wuw_detection_start_table[i].model);
            }
        }
    }

    DEBUG_LOG_WARN("vaTestPopulateWuwDetectionParams: No valid configuration found for engine option enum:va_wuw_engine_t:%d", engine);
    return FALSE;
}
