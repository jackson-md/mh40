...
#ifdef INCLUDE_BTDBG
    if (td->profiles_status & DEVICE_PROFILE_BTDBG)
    {
        DEBUG_LOG("twsTopology_ProcedurePriConnectPeerProfilesConnectProfile BTDBG_PROFILE");
        BtdbgPeerProfile_Connect(TwsTopProcPriConnectPeerProfilesGetTask(), &secondary_addr);
        return;
    }
#endif

#ifdef INCLUDE_SENSOR_PROFILE
    if (td->profiles_status & DEVICE_PROFILE_SENSOR)
    {
        DEBUG_LOG("twsTopology_ProcedurePriConnectPeerProfilesConnectProfile SENSOR");
        SensorProfile_Connect(TwsTopProcPriConnectPeerProfilesGetTask(), &secondary_addr);
    }
#endif /* INCLUDE_SENSOR_PROFILE */
                                                   ...
                                                   Message goal_data)
{
    ...
    td->profiles_status = TwsTopologyConfig_PeerProfiles();

    DEBUG_LOG("TwsTopology_ProcedurePriConnectPeerProfilesStart (profiles:0x%x)", td->profiles_status);
...
static void twsTopology_ProcPriConnectPeerProfilesHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
...
#ifdef INCLUDE_BTDBG
        case BTDBG_PROFILE_PEER_CONNECT_CFM:
        {
            DEBUG_LOG("twsTopology_ProcPriConnectPeerProfilesHandleMessage BTDBG_PROFILE");
            twsTopology_ProcPriConnectPeerProfilesStatus(DEVICE_PROFILE_BTDBG, TRUE);
        }
        break;
#endif
        ...
        {
            SENSOR_PROFILE_CONNECTED_T* cfm = (SENSOR_PROFILE_CONNECTED_T*)message;
            DEBUG_LOG("twsTopology_ProcPriConnectPeerProfilesHandleMessage SENSOR status %d", cfm->status);
            twsTopology_ProcPriConnectPeerProfilesStatus(DEVICE_PROFILE_SENSOR, cfm->status == sensor_profile_status_peer_connected);
        }
        break;
#endif /* INCLUDE_SENSOR_PROFILE */
