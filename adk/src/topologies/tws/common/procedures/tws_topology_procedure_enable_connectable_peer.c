                                        ...
                                        Message goal_data)
{
    ...
    if (td->params.enable && !appDeviceIsPeerConnected())
    {
        ...
        BredrScanManager_PauseTruncatedPageScan(TwsTopProcEnableConnectablePeerGetTask());
    }
    else
    {
...
static void twsTopology_ProcEnableConnectablePeerHandleTpsPausedCfm(BREDR_SCAN_MANAGER_TPS_PAUSED_CFM_T * cfm)
{
    twsTopProcEnableConnectablePeerTaskData* td = TwsTopProcEnableConnectablePeerGetTaskData();

    DEBUG_LOG("twsTopology_ProcEnableConnectablePeerHandleTpsPausedCfm");

    if (cfm->paused && td->params.enable)
    {
        ...
        MessageSendLater(TwsTopProcEnableConnectablePeerGetTask(), TWS_TOP_PROC_ENABLE_CONNECTABLE_PEER_INTERNAL_CONNECT_TIMEOUT,
                         NULL, TwsTopologyConfig_PrimaryPeerConnectTimeoutMs());
    }
}

static void twsTopology_ProcEnableConnectablePeerHandlePageScanReleaseInd(void)
{
    DEBUG_LOG("twsTopology_ProcEnableConnectablePeerHandlePageScanReleaseInd");

    BredrScanManager_ResumeTruncatedPageScan();
}
...
static void twsTopology_ProcEnableConnectablePeerHandleMessage(Task task, MessageId id, Message message)
{
    ...
    if (!td->active)
    {
        if (id == BREDR_SCAN_MANAGER_PAGE_SCAN_RELEASED_IND)
        {
            twsTopology_ProcEnableConnectablePeerHandlePageScanReleaseInd();
        }
        return;
    }
    ...
    switch (id)
    {
        ...
        case BREDR_SCAN_MANAGER_TPS_PAUSED_CFM:
            twsTopology_ProcEnableConnectablePeerHandleTpsPausedCfm((BREDR_SCAN_MANAGER_TPS_PAUSED_CFM_T *) message);
            break;

        case BREDR_SCAN_MANAGER_PAGE_SCAN_RELEASED_IND:
            twsTopology_ProcEnableConnectablePeerHandlePageScanReleaseInd();
            break;
